...
import { getPayment, REFUNDABLE_RAILS, refundPayment } from "@/lib/api/endpoints"
import { formatMicros, formatUnix, microsFromInput, shortId } from "@/lib/format"
import { toastApiError } from "@/lib/toast"

export function PaymentDetailPage() {
  const { id = "" } = useParams()
  const navigate = useNavigate()
  const { data: payment, loading, error, reload } = useApiData(() => getPayment(id), [id])
  React.useEffect(() => {
    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center gap-3">
        ...
        <div className="ml-auto">
          <RefundDialog payment={payment} disabled={!refundable} disabledNote={railRefundNote} onDone={reload} />
        </div>
      </div>

      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <Fact label="Amount">{formatMicros(payment.amount, payment.currency)}</Fact>
        <Fact label="Refunded">
          {payment.amount_refunded ? formatMicros(payment.amount_refunded, payment.currency) : "—"}
        </Fact>
        <Fact label="Customer">
          <Link
        ...
        {payment.failure_message && <Fact label="Failure">{payment.failure_message}</Fact>}
      </div>
            ...
            <Table>
              ...
              <TableBody>
                {payment.refunds.data.map((r) => (
                  <TableRow key={r.id}>
                    <TableCell className="font-mono text-xs">{shortId(r.id, 16)}</TableCell>
...
function RefundDialog({
  ...
  payment: { id: string; amount: number; amount_refunded: number; currency: string; rail: string }
  disabled: boolean
  disabledNote?: string
  onDone: () => void
}) {
  ...
  return (
    <>
      ...
      >
        <Undo2Icon className="size-4" /> Refund
      </Button>
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Executes the refund at the {payment.rail} rail. Remaining refundable:{" "}
              {formatMicros(remaining, payment.currency)}.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-3">
            ...
            <div className="flex items-center gap-2">
              <Switch id="refund-revoke" checked={revokeAccess} onCheckedChange={setRevokeAccess} />
