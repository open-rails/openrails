...
import { client, render, server } from "@/test/harness"
...
const singleCurrency: MetricsResult = {
  range,
  columns: [money],
  rows: [["100000000"]],
...
describe("comparisons", () => {
  ...
  it("invents no comparison from duplicate, missing or absent rows", () => {
    ...
    expect(
      statDelta({ ...moneyStats, compare_rows: [["JPY", 0]] }, moneyStats.rows[1], 1)
    ).toBeNull()
...
describe("units", () => {
    ...
    for (const viz of ["line", "area", "bar", "donut"] as const) {
      const html = render(
        <WidgetVizView
          ...
          result={
            viz === "donut"
              ? { ...moneyStats, rows: [["EUR", "100000000"], ["JPY", 200_000_000]] }
              : moneyChart
          }
        />
      )
  ...
  it("keeps a filtered-out currency and leaves non-money measures alone", () => {
    ...
    for (const viz of ["stat", "table"] as const) {
      const html = render(
        <WidgetVizView viz={viz} result={singleCurrency} query={query} />
      )
    ...
    expect(groupSeries(mixed.series).map((group) => group.series[0].unit)).toEqual(
      ["money", "count", "money"]
    )
    expect(indexColumns(singleCurrency.columns).currency).toBe(-1)
  })
  ...
  it("does not relabel a stale result when the currency filter changes", async () => {
    ...
    queries.setQueryData(adminQueries.widgetMetrics(eur).queryKey, singleCurrency)
    ...
    try {
      const yen = { ...singleCurrency, rows: [[200_000_000]] }
      const options = {
...
describe("money beyond Number precision", () => {
  it("keeps the exact wire string in the stat, table and chart tooltip", () => {
    ...
    expect(formatMeasure(UNSAFE, "money", "USD")).toBe(
    ...
    ).toBe(formatNativeAmount(UNSAFE, "USD"))
  })
