...
describe("displaying an exact amount", () => {
  ...
  it.each([[Number.MAX_SAFE_INTEGER + 1, 6], ["12.5", 6], ["9223372036854775808", 6]])(
    "refuses to display %s rather than round it",
    (units, scale) => {
      expect(formatUnits(units, "USD", scale)).toContain("exceeds the exact display range")
    }
  )

  it("keeps every digit an amount carries, at any scale", () => {
    expect(digits(formatUnits("1", "shop/points", 18))).toBe("0000000000000000001")
  ...
  it("never prefills a rounded amount from an unsafe JSON number", () => {
    ...
    for (const unsafe of [Number(UNSAFE), -Number(UNSAFE), Number(MAX), 1e21, 1.5, Number.NaN]) {
      expect(nativeAmountToInput(unsafe, "USD")).toBe("")
      expect(nativeAmountFromInput(nativeAmountToInput(unsafe, "USD"), "USD")).toBeNull()
    }
    expect(nativeAmountToInput(Number.MAX_SAFE_INTEGER, "USD")).toBe("9007199254.740991")
    // The exact string form of the same value prefills and round-trips.
    expect(nativeAmountFromInput(nativeAmountToInput(UNSAFE, "USD"), "USD")).toBe(UNSAFE)
  })

  it.each(Object.entries(currencyUnits))(
    "%s round-trips every int64 edge exactly at scale %i",
    (currency, scale) => {
      for (const units of ["0", "1", "-1", String(10n ** BigInt(scale)), String(Number.MAX_SAFE_INTEGER), MAX, MIN]) {
...
describe("engines without exact decimal-string Intl formatting", () => {
  ...
  it("is detected, and still formats everything a Number holds exactly", async () => {
    ...
    expect(legacy.formatUnits("999999999999999", "USD", 6)).toBe("$999,999,999.999999")
    expect(legacy.formatUnits("-999999999999999", "JPY", 4)).toBe("-¥99,999,999,999.9999")
  ...
  it("refuses rather than misformats amounts beyond that range", async () => {
    ...
    expect(new LegacyNumberFormat("en", { useGrouping: false }).format(UNSAFE)).toBe("9007199254740992")
    for (const [units, currency, scale] of [
      [UNSAFE, "shop/points", 0], [UNSAFE, "USD", 6],
      ["1000000000000000", "USD", 6], ["-1000000000000000", "JPY", 4],
    ] as const)
      expect(legacy.formatUnits(units, currency, scale)).toContain("this browser's exact display range")
    expect(legacy.formatNativeAmount(MAX, "USD")).toContain("this browser's exact display range")
    // Not an int64 at all: the range refusal is unchanged.
    expect(legacy.formatUnits("9223372036854775808", "USD", 6)).toBe("USD amount exceeds the exact display range")
  })
})

describe("money submitted by console forms", () => {
  const credit = {
    amount: "1.000001", decimals: 6, currency: "USD", expires: "",
  ...
  it("grants credit at the server unit scale, keeping the caller's key", () => {
    ...
    expect(amount({ amount: "1.2345", currency: "JPY", decimals: 4 })).toBe("12345")
    expect(amount({ amount: "12", currency: "shop/points", decimals: 0 })).toBe("12")
    expect(() => amount({ amount: "1.23456", currency: "JPY", decimals: 4 })).toThrow("4 decimal places")
  })

  it.each(["", "0", "-1", "NaN", "Infinity", "1e3", "1.0000001", "9223372036854.775808"])(
    "refuses the invalid or unrepresentable grant amount %s",
    (invalid) => {
      expect(() => amount({ amount: invalid })).toThrow("positive amount")
    }
  )
  ...
  it("remits invoices at the invoice's own scale, never beyond the balance", () => {
    ...
    for (const over of ["0.00001", "13", "0", "9223372036854.775808", UNSAFE])
      expect(() => invoicePaymentAmount(over, "120000", 4)).toThrow()
  })
...
describe("canonical wire fixtures in the browser", () => {
  ...
  const fixture = (name: string) =>
    JSON.parse(Object.entries(fixtures).find(([path]) => path.endsWith(`/${name}`))![1])
  ...
  it("contains only JSON numbers JavaScript represents exactly", () => {
    ...
    for (const [path, raw] of Object.entries(fixtures))
      for (const token of raw.replace(/"(?:[^"\\]|\\.)*"/g, '""').match(/-?\d[\d.eE+-]*/g) ?? [])
        expect(
  ...
  it("round-trips int64 boundary money through the console's own parsing", () => {
    ...
    expect(fixture("merchant_settings.json").billing_policies[1].spend_windows[0].limit).toBe(MAX)
    expect(fixture("error_envelope.json").error.metadata.committed_amount).toBe(MAX)
    expect(digits(formatUnits(fixture("subscription.json").price.unit_amount, "USD", 6))).toBe(digits(MAX))
  })
})
