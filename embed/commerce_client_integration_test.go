...
func TestCommerceClientCheckoutAndTier(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = h.Pool().Exec(ctx, `INSERT INTO openrails.prices(id,merchant_id,product_id,key,amount,currency,access_duration_hours,auto_renew,psp_links) VALUES($1,$2,$3,$4,10000000,'USD',720,true,jsonb_build_object('ccbill',jsonb_build_object('rail','ccbill','flex_id',$5::text,'form_name','test-form')))`, priceID, dbtest.TestMerchantID.UUID(), productID, priceKey, uuid.NewString())
	require.NoError(t, err)
