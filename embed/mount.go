...
const (
	...
	// RouteSetMerchantAPI mounts the service/API-key surface (/billing/v1/merchant/*);
	// most embedded hosts use Client() instead.
...
var (
	...
	// customer, merchant_admin, catalog and webhooks.
	EmbeddedDefaultRouteSets = append([]RouteSet(nil), embedhttp.EmbeddedDefaultRouteSets...)
	// StandaloneDefaultRouteSets is the full standalone surface.
...
// MountOptions configures the combined embedded surface (Handler).
...
func (r *Runtime) Handler(opts MountOptions) (http.Handler, error) {
	if r == nil || r.app == nil {
		return nil, fmt.Errorf("openrails embed: runtime is not initialized")
	}
	...
	active := r.mountRouteSets(opts.RouteSets)
	...
	if routeSetSelected(active, RouteSetCustomer) {
		var err error
		self, err = r.selfHandler(opts.DelegatedAuthenticator, providerRoutes)
		if err != nil {
			return nil, err
		}
	}
...
// to the base handler. Most hosts should use Handler, which does that
// routing itself.
...
func (r *Runtime) SelfHandler(authn billingauth.DelegatedAuthenticator) (http.Handler, error) {
	return r.selfHandler(authn, nil)
}
