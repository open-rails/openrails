...
func TestRecoveryClientAcrossTransports(t *testing.T) {
	...
	for _, d := range deployments {
		...
		t.Run(d.name, func(t *testing.T) {
			...
			_, err := client.DeletePaymentMethod(ctx, openrails.CustomerID(customer), openrails.PaymentMethodID(method))
			require.ErrorIs(t, err, openrails.ErrConflict, "active subscription blocks deleting its card before any provider mutation")
			_, err = client.DeletePaymentMethod(ctx, openrails.CustomerID(uuid.New()), openrails.PaymentMethodID(method))
			...
			page, err := client.ListSubscriptions(ctx, openrails.SubscriptionFilter{CustomerID: openrails.CustomerID(customer), Status: "cancelled", PageOptions: openrails.PageOptions{Limit: 1}})
			require.NoError(t, err)
			...
			methods, err := client.ListPaymentMethods(ctx, openrails.CustomerID(customer), openrails.PageOptions{Limit: 1})
			require.NoError(t, err)
			...
			none, err := client.ListPaymentMethods(ctx, openrails.CustomerID(customer), openrails.PageOptions{Limit: 1, Offset: 1})
			require.NoError(t, err)
	...
	_, err = reader.DeletePaymentMethod(ctx, openrails.CustomerID(uuid.New()), openrails.PaymentMethodID(uuid.New()))
