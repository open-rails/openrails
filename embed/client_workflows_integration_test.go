...
func checkClientDeclaredAccess(t *testing.T, ctx context.Context, h *integrationharness.Harness, d clientWorkflowDeployment) {
	...
	hasPremium, err := client.HasEntitlement(ctx, comped, "premium", time.Time{})
	require.NoError(t, err)
	...
	permanent, err := client.GrantEntitlement(ctx, customer, openrails.GrantEntitlementRequest{Entitlement: "staff"})
	require.NoError(t, err)
	require.Equal(t, "admin", permanent.SourceType)
	require.Nil(t, permanent.EndAt)
	ghost := openrails.CustomerID(uuid.New())
	batch, err := client.ListActiveEntitlements(ctx, []openrails.CustomerID{customer, customer, comped, ghost}, time.Time{})
	require.NoError(t, err)
	require.Len(t, batch, 3)
	require.Contains(t, batch, ghost)
	require.Empty(t, batch[ghost])
	require.Len(t, batch[customer], 1)
	require.Equal(t, customer, batch[customer][0].CustomerID)
	require.Equal(t, "staff", batch[customer][0].Entitlement)
	require.Equal(t, "admin", batch[customer][0].SourceType)
	require.Nil(t, batch[customer][0].EndAt)
	records, err := client.ListEntitlements(ctx, customer, time.Time{})
	require.NoError(t, err)
	require.Equal(t, batch[customer], records)
	for _, entitlement := range []string{"staff", "missing"} {
		has, err := client.HasEntitlement(ctx, customer, entitlement, time.Time{})
		require.NoError(t, err)
		require.Equal(t, entitlement == "staff", has)
	}
	require.ErrorIs(t, client.RevokeEntitlement(ctx, ghost, permanent.ID), openrails.ErrNotFound)
	require.NoError(t, client.RevokeEntitlement(ctx, customer, permanent.ID))
	has, err := client.HasEntitlement(ctx, customer, "staff", time.Time{})
	require.NoError(t, err)
	require.False(t, has)
	...
	require.Equal(t, product.ID, grants[0].ProductID.String())
	...
	has, err = client.HasProductAccess(ctx, comped, sdkProductID(t, product.ID))
	require.NoError(t, err)
	require.True(t, has)
