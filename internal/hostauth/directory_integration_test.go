...
func TestDirectoryPublicClient(t *testing.T) {
	...
	require.NoError(t, err)
	defer core.Close()
	directory := billingauthkit.NewDirectory(core.Client())
	user, err := core.Client().CreateUser(ctx, "billing@example.test", "billing-user")
	require.NoError(t, err)
	...
	deleted, err := core.Client().SoftDeleteUsers(ctx, []string{user.ID})
	require.NoError(t, err)
