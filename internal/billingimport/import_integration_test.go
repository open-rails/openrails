...
func TestImportBilling_DeclaredBookClassifiesAtAsOf(t *testing.T) {
	...
	defaultNMIPSP := billingimport.PSPRef{Key: "nmi"}
	ccbillPSPRef := billingimport.PSPRef{Key: "ccbill"}
	t.Cleanup(func() {
	...
	twin := billingimport.DeclaredBilling{
		AsOf:       asOf,
	...
	// still-dunning at the provider (billingimport.DunningEvidence.ScheduleLive, no explicit
	...
	restalled := billingimport.DeclaredBilling{
		AsOf:       asOf2,
		DefaultPSP: defaultNMIPSP,
		Subscriptions: []billingimport.DeclaredSubscription{{
	...
	// dead subscription.
	asOf3 := asOf2.Add(day)
	terminated := billingimport.DeclaredBilling{
		AsOf:       asOf3,
		DefaultPSP: defaultNMIPSP,
		Subscriptions: []billingimport.DeclaredSubscription{{
...
func TestImportBilling_RefusesPaymentMethodOwnerChange(t *testing.T) {
	...
	method := func(customer uuid.UUID) billingimport.DeclaredBilling {
