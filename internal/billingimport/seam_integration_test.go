...
// admin comp over as a declared admin grant, then ConvergeMerchant — and
...
func TestDeclaredAdminGrantThenConverge(t *testing.T) {
	...
	_, err = hosttools.ConvergeMerchant(context.Background(), hosttools.ConvergeMerchantOptions{
		PGXPool:    pool,
		MerchantID: dbtest.TestMerchantID,
	})
	require.NoError(t, err)
