...
func TestReconciliationUsesOnlySelectedPSPPriceAndPaymentBindings(t *testing.T) {
	...
	for _, pspID := range accounts {
		_, err := database.Qx(ctx).Exec(ctx, `INSERT INTO billing.psps(id,merchant_id,rail,environment,account_id,key) VALUES($1,$2,'nmi','test',$3,$3)`, pspID, dbtest.TestMerchantID.UUID(), uuid.NewString())
		...
		_, err = database.Qx(ctx).Exec(ctx, `INSERT INTO billing.products(id,merchant_id,key,display_name) VALUES($1,$2,$3,'Reconcile')`, productID, dbtest.TestMerchantID.UUID(), uuid.NewString())
		require.NoError(t, err)
		...
		require.NoError(t, err)
		_, err = database.Qx(ctx).Exec(ctx, `INSERT INTO billing.price_psp_bindings(merchant_id,price_id,psp_id,plan_id) VALUES($1,$2,$3,$4)`, dbtest.TestMerchantID.UUID(), priceID, pspID, planRef)
		require.NoError(t, err)
