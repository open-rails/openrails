...
func TestReconcileUnknownCohort_StaleDeclineQueuesDeferredDelete(t *testing.T) {
	...
	require.NoError(t, appDB.RunInMerchantConn(baseCtx, func(ctx context.Context) error {
		...
		mk := func(id uuid.UUID, railSub string) {
			...
			prices[id] = price
			exec(`INSERT INTO billing.products (id,key,display_name,entitlements_spec,merchant_id) VALUES ($1,$2,$2,'{}'::jsonb,$3)`, prod, "ud-"+railSub, merchantID)
	...
	t.Cleanup(func() {
		_ = appDB.RunInMerchantConn(baseCtx, func(ctx context.Context) error {
			for id, price := range prices {
				_, _ = appDB.Qx(ctx).Exec(ctx, `DELETE FROM billing.rail_intents WHERE subscription_id=$1`, id)
			...
			_, _ = appDB.Qx(ctx).Exec(ctx, `DELETE FROM billing.products WHERE key LIKE 'ud-rs-%'||$1`, sfx)
			return nil
		})
	})
	...
	require.NoError(t, appDB.RunInMerchantConn(baseCtx, func(ctx context.Context) error {
		...
		require.NoError(t, appDB.Qx(ctx).QueryRow(ctx,
			`SELECT status, deletion_scheduled_at, cancel_feedback FROM billing.subscriptions WHERE id=$1`, subStale).
		...
		require.NoError(t, appDB.Qx(ctx).QueryRow(ctx,
			`SELECT status, deletion_scheduled_at FROM billing.subscriptions WHERE id=$1`, subGone).
			Scan(&status, &marker))
		require.Equal(t, "cancelled", status)
		require.Nil(t, marker, "roster-gone cancel must not schedule a delete")
		var n int
		require.NoError(t, appDB.Qx(ctx).QueryRow(ctx,
			`SELECT count(*) FROM billing.rail_intents WHERE subscription_id=$1`, subGone).Scan(&n))
