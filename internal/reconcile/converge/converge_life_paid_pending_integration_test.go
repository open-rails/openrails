...
func TestConverge_LifePaidPendingSubscriptionIsSurfacedNotCancelled(t *testing.T) {
	...
	require.NoError(t, appDB.RunInMerchantConn(baseCtx, func(ctx context.Context) error {
		...
		exec(`INSERT INTO billing.products (id, key, display_name, tier_group, entitlements_spec, merchant_id) VALUES ($1,$2,$2,$3,'{"premium": null}'::jsonb,$4)`,
		...
		exec(`INSERT INTO billing.payments (id, customer_id, price_id, subscription_id, rail, psp_id, transaction_id, amount, list_amount, currency, status, money_movement, purchased_at, merchant_id)
		      VALUES ($1,$2,$3,$4,'nmi',$5,$6,9990000,9990000,'USD','completed','rail',$7,$8)`,
			paymentID, customer, priceID, subID, pspID, txnID, old, merchantID)
		exec(`INSERT INTO billing.reconciliation_state (merchant_id, source_domain, fully_reconciled)
	...
	t.Cleanup(func() {
		_ = appDB.RunInMerchantConn(baseCtx, func(ctx context.Context) error {
			_, _ = appDB.Qx(ctx).Exec(ctx, `DELETE FROM billing.reconciliation_findings WHERE merchant_id=$1 AND subject_key IN ($2, $3)`, merchantID, "subscription:"+subID.String(), "payment:"+paymentID.String())
	...
	require.NoError(t, appDB.RunInMerchantConn(baseCtx, func(ctx context.Context) error {
		...
		rows, err := appDB.Qx(ctx).Query(ctx,
			`SELECT finding_type, status FROM billing.reconciliation_findings WHERE merchant_id=$1 AND subject_key=$2 ORDER BY finding_type`,
