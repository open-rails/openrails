...
func TestConverge_PeriodOverdue_WatermarkEvidence(t *testing.T) {
	...
	require.NoError(t, appDB.RunInMerchantConn(baseCtx, func(ctx context.Context) error {
		...
		exec(`INSERT INTO openrails.provider_refresh_watermarks (id,merchant_id,provider,event_domain,watermark_at) VALUES ($1,$2,'nmi','events',$3)`,
			wm, merchantID, time.Now().UTC().Add(-time.Hour))
		return nil
	}))
	t.Cleanup(func() {
		_ = appDB.RunInMerchantConn(baseCtx, func(ctx context.Context) error {
			_, _ = appDB.Qx(ctx).Exec(ctx, `DELETE FROM openrails.provider_refresh_watermarks WHERE id=$1`, wm)
