...
CREATE TABLE four ENGINE = ReplicatedMergeTree('/clickhouse/tables/{database}/{table}', '{replica}') ORDER BY id;
`,
	}}
	...
	if strings.Contains(content, "ReplicatedReplacingMergeTree") || strings.Contains(content, "ReplicatedMergeTree") || strings.Contains(content, "{replica}") {
		t.Fatalf("replicated engine was not fully rewritten:\n%s", content)
	}
	for _, want := range []string{"ReplacingMergeTree(version)", "ReplacingMergeTree(last_updated)", "ReplacingMergeTree()", "MergeTree()"} {
