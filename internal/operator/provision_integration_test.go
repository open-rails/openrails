...
func hostedTestConfig(t *testing.T, dsn, issuer string) *hostconfig.Config {
	t.Helper()
	return &hostconfig.Config{Config: &config.Config{ProviderWriteMode: config.ProviderWriteModeReadOnly, Encryption: &config.EncryptionConfig{MasterKey: "AAECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8="},
