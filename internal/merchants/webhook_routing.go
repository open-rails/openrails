...
// boundary — after resolution OpenRails loads that merchant's signing secret and
...
type WebhookResolver interface {
	// ResolveBySlug maps an explicit merchant slug (e.g. from a /t/:tenant/ path)
	...
	// webhook_host. Returns ErrMerchantRouteUnresolved when no active merchant claims
	// the host.
...
// registered webhook_host. The default merchant is never matched by host (it has no
...
func (s *Service) ResolveByHost(ctx context.Context, host string) (WebhookRoute, error) {
	...
	err := s.pool.QueryRow(ctx, `
		SELECT id::text, slug, status FROM openrails.merchants
