...
func TestStripeArchive_RelevanceFlipsWhenObjectJoinsCatalog(t *testing.T) {
	...
	t.Run("price: linked by id", func(t *testing.T) {
		...
		linked := &models.Price{
			ID: uuid.New(), ProductID: productID, Amount: 900, Currency: "usd", BillingCycleDays: &cycle,
			Processors: map[string]map[string]string{
				"stripe": {models.ProcessorKeyStripePriceID: "price_x"},
			},
		}
	...
	t.Run("price: content key joins", func(t *testing.T) {
		...
		price := &models.Price{ID: uuid.New(), ProductID: productID, Amount: 900, Currency: "usd", BillingCycleDays: &cycle}
