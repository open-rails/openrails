...
func TestFailMembershipLimitedModeQueuesDeleteIntent(t *testing.T) {
	...
	t.Cleanup(func() {
		_, _ = pool.Exec(ctx, `DELETE FROM openrails.rail_mutation_logs
			WHERE provider_intent_id IN (SELECT id FROM openrails.rail_intents WHERE subscription_id = $1)`, subID)
	...
	lifecycle.SetDeferredDeleteScheduler(NewNMIDeleteScheduler(dbi, OriginSystem, "test: dunning exhaustion"))
