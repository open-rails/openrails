...
func TestNMIRefundUnknownResolvesOnlyFromExactReceipt(t *testing.T) {
	...
	_, err = runner.Resolve(ctx, row.ID, Resolution{Step: "source", RequalifyAccount: "account-proof", Actor: "ops", Reason: "cutover-only requalification"})
	require.ErrorIs(t, err, ErrResolutionUnsupported, "account requalification must not reach refund handlers")
...
func TestNMIRefundUnknownNonExecutionPreservesReservation(t *testing.T) {
	for _, visible := range []bool{false, true} {
		t.Run(fmt.Sprint(visible), func(t *testing.T) {
			...
			status, _, _ := fx.reservation(t)
			require.Equal(t, "pending", status)
			_, err = runner.RunExecuteOnce(dbtest.WithTestMerchant(context.Background()))
			require.NoError(t, err)
...
func TestNMIRefundReceiptSurvivesLocalFailureAcrossRestart(t *testing.T) {
	...
	_, err := ddl.Exec(dbtest.WithTestMerchant(context.Background()), fmt.Sprintf(`ALTER TABLE billing.payments ADD CONSTRAINT %s CHECK (id <> '%s'::uuid OR NOT (coalesce(metadata,'{}') ? 'provider_refund_id'))`, constraint, fx.reservationID))
	require.NoError(t, err)
	remove := func() {
		_, err := ddl.Exec(dbtest.WithTestMerchant(context.Background()), `ALTER TABLE billing.payments DROP CONSTRAINT IF EXISTS `+constraint)
		require.NoError(t, err)
	}
	t.Cleanup(remove)
...
func TestManualRebillOperatorResolutionRequiresQualifiedReceipt(t *testing.T) {
	...
	row, err := runner.EnqueueAndExecute(dbtest.WithTestMerchant(context.Background()), fx.enqueueParams(1))
	require.NoError(t, err)
	require.Equal(t, StatusUnknownNeedsVerify, row.Status)
	ctx := dbtest.WithTestMerchant(context.Background())
