...
func (s *Store) RecordProgress(ctx context.Context, id uuid.UUID, keys map[string]any) error {
	if _, ok := keys["enrollment_submitted"]; ok {
...
func (s *Store) RecordProgressIfAbsent(ctx context.Context, id uuid.UUID, key string, value any) (bool, error) {
	...
	if key == "enrollment_submitted" && value != true {
 ...
 AND ($2::text <> 'enrollment_submitted' OR NOT (coalesce(result_evidence,'{}'::jsonb) ? 'qualified_initial_refusal'))`, id, key, b, mid.UUID())
	if err != nil {
		return false, err
	}
