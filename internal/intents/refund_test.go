...
func TestRefundIntentForRoutesByProcessor(t *testing.T) {
	stripePayment := &models.Payment{Processor: models.ProcessorStripe}
	...
	nmiPayment := &models.Payment{Processor: models.ProcessorMobius}
...
func stripeTestConfig() *config.Config {
	return &config.Config{Processors: map[string]*config.ProcessorConfig{
...
func TestStripeRefundExecuteClassification(t *testing.T) {
	...
	t.Run("unconfigured stripe parks", func(t *testing.T) {
		h := NewStripeRefundHandler(nil, &config.Config{}, nil)
	...
	t.Run("sends the intent idempotency key", func(t *testing.T) {
		...
		h := NewStripeRefundHandler(nil, stripeTestConfig(), nil)
		h.Stripe = fake
