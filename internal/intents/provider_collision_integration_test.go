...
func TestRefundIntentKeepsArchivedAccountWithCollidingTransactions(t *testing.T) {
	...
	_, err := fx.db.Qx(ctx).Exec(ctx, `UPDATE billing.psps SET account_id=$2,key=$2 WHERE id=$1`, fx.pspID, accountA)
	require.NoError(t, err)
	_, err = fx.db.Qx(ctx).Exec(ctx, `INSERT INTO billing.psps(id,merchant_id,rail,environment,account_id,key) VALUES($1,$2,'nmi','test',$3,$3)`, sibling, dbtest.TestMerchantID.UUID(), accountB)
	require.NoError(t, err)
	siblingPayment := uuid.New()
	_, err = fx.db.Qx(ctx).Exec(ctx, `INSERT INTO billing.payments(id,price_id,rail,psp_id,transaction_id,amount,list_amount,currency,status,customer_id,merchant_id,money_movement) SELECT $2,price_id,rail,$3,transaction_id,amount,list_amount,currency,status,customer_id,merchant_id,money_movement FROM billing.payments WHERE id=$1`, fx.paymentID, siblingPayment, sibling)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	_, err = fx.db.Qx(ctx).Exec(ctx, `UPDATE billing.psps SET key=$2,archived=true WHERE id=$1`, fx.pspID, "renamed-"+accountA)
	require.NoError(t, err)
	...
	require.NoError(t, fx.db.Qx(ctx).QueryRow(ctx, `SELECT count(*) FROM billing.payments WHERE refunded_payment_id=$1`, siblingPayment).Scan(&siblingRefunds))
