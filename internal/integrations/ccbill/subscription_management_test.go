...
func TestSubscriptionManagementMinusSevenIsIndeterminate(t *testing.T) {
	for _, body := range []string{`-7`, `<results>-7</results>`} {
		t.Run(body, func(t *testing.T) {
			cap, c := newSMSServer(t)
			cap.answer.Store(body)
			_, err := c.CancelSubscription(context.Background(), "0123456789")
			require.ErrorIs(t, err, ErrDataLinkIndeterminate)
			require.NotErrorIs(t, err, ErrDataLinkAuth, "internal errors cannot authorize clean retries")
			_, err = c.ViewSubscriptionStatus(context.Background(), "0123456789")
			require.ErrorIs(t, err, ErrDataLinkIndeterminate)
		})
	}
