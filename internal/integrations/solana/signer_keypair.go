...
// from a TenantSecretGetter, parses it, and signs in-process. Works with any
...
type keypairSigner struct {
	secrets TenantSecretGetter
	ttl     time.Duration
	now     func() time.Time

	mu    sync.Mutex
	cache map[string]cachedKey // keyed by tenantID.String()
}
...
// NewKeypairSigner returns a per-tenant in-process signer backed by secrets. A
// zero ttl uses DefaultSignerCacheTTL.
func NewKeypairSigner(secrets TenantSecretGetter, ttl time.Duration) Signer {
	if ttl <= 0 {
...
func (k *keypairSigner) load(ctx context.Context, tenantID tenant.ID) (solanago.PrivateKey, error) {
	...
	if tenantID.IsZero() {
		return nil, fmt.Errorf("solana: signing requires a tenant id")
	}
	...
	raw, err := k.secrets.GetSecret(ctx, tenantID, SecretSolanaPrivateKey)
	if err != nil {
		return nil, fmt.Errorf("solana: load tenant signing key: %w", err)
	}
	key, err := solanago.PrivateKeyFromBase58(strings.TrimSpace(raw))
	if err != nil {
		return nil, fmt.Errorf("solana: parse tenant signing key: %w", err)
	}
