...
func TestCustomerRecoverySelfRoutes(t *testing.T) {
	...
	for name, s := range surfaces {
		t.Run(name, func(t *testing.T) {
			...
			custodied := h.SeedPastDueSubscription(s.runtime, merchant.ID(mid), SubscriptionForCustomer(mine.Customer))
			h.FlipCustody(custodied)
			require.Equal(t, openrails.RecoveryBlockedNoPaymentMethod, blocked(custodied).BlockedReason)
