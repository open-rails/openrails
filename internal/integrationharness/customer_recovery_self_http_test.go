...
func TestCustomerRecoverySelfRoutes(t *testing.T) {
	...
	sandbox(cfg)
	rt, err := embed.New(ctx, embed.Options{Options: embedded.Options{Config: cfg, Redis: h.Redis, River: embedded.RiverManagedByOpenRails()}})
	...
	handler, err := rt.Handler(embedded.MountOptions{RouteSets: []embed.RouteSet{embed.RouteSetCustomer}, Gate: httproutes.NewGate(httproutes.GateOptions{DelegatedAuthenticator: authn}), DelegatedAuthenticator: authn})
	require.NoError(t, err)
	...
	for name, s := range surfaces {
		t.Run(name, func(t *testing.T) {
			...
			gateway.RegisterPlan(sub.RailSubscriptionID, "12.00", "USD")
			...
			require.Contains(t, invoice.Recovery.CompatiblePaymentMethodIDs, uuid.UUID(mine.Method))
			require.Contains(t, invoice.Recovery.CompatiblePaymentMethodIDs, uuid.UUID(sub.Method), "every vaulted NMI method of the payer")
			...
			require.Equal(t, []uuid.UUID{uuid.UUID(sub.Method)}, subscription.Recovery.CompatiblePaymentMethodIDs)
			...
			gateway.RegisterPlan(fresh.RailSubscriptionID, "12.00", "USD")
