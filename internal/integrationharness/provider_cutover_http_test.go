...
func assertCutoverCommitted(t *testing.T, h *Harness, g *cutoverGateway, p cutoverHTTPFixture, result *openrails.ProviderCutover) {
	...
	require.Equal(t, api.FormatSubscriptionID(p.Sub), result.SubscriptionID)
...
func TestNMIProviderCutoverHTTP(t *testing.T) {
	...
	for _, mode := range []string{"success", "lost_cancel", "lost_activation", "source_dark", "lost_create", "wrong_receipt", "wrong_tombstone"} {
		t.Run(mode, func(t *testing.T) {
			...
			if mode == "lost_create" || mode == "wrong_receipt" || mode == "wrong_tombstone" {
				...
				again, e := client.CutoverProvider(ctx, api.FormatSubscriptionID(p.Sub), key, p.Req)
				require.NoError(t, e)
				require.Equal(t, result, again)
			}
			changed := p.Req
			changed.ExpectedSourcePSPID = uuid.New()
			_, e = client.CutoverProvider(ctx, api.FormatSubscriptionID(p.Sub), key, changed)
			require.Error(t, e)
			got, e := client.GetProviderCutover(ctx, api.FormatSubscriptionID(p.Sub), key)
			require.NoError(t, e)
	...
	t.Run("concurrent_replay", func(t *testing.T) {
		...
		for i := 0; i < 2; i++ {
			...
			go func() {
				defer wg.Done()
				_, e := client.CutoverProvider(ctx, api.FormatSubscriptionID(p.Sub), key, p.Req)
				errs <- e
			}()
		}
		wg.Wait()
	...
	t.Run("customer_ownership", func(t *testing.T) {
		...
		_, e := client.PreviewProviderCutover(ctx, api.FormatSubscriptionID(p.Sub), bad)
		require.Error(t, e)
	})
	t.Run("host_campaign", func(t *testing.T) {
		...
		result, e := client.GetProviderCutover(ctx, api.FormatSubscriptionID(p.Sub), strings.TrimPrefix(key, intents.TypeNMIProviderCutover+":"))
		require.NoError(t, e)
		assertCutoverCommitted(t, h, g, p, result)
	})
}
