...
func TestStandaloneMerchantAdmitAcceptsUserSessionByPermissionHTTP(t *testing.T) {
	...
	mintUserToken := func(_ string, perms []string) string {
		...
		require.NoError(t, core.Genesis().AssignGroupRole(ctx, controlplane.MerchantGroup(dbtest.TestMerchantSlug), authkit.UserSubject(user.ID), authkit.Role(roleForPerms(perms))), "assign merchant role")
...
func TestCoreDoesNotMountPlatformAdminRoutesHTTP(t *testing.T) {
	...
	require.NoError(t, core.Genesis().AssignGroupRole(ctx, controlplane.MerchantGroup(dbtest.TestMerchantSlug), authkit.UserSubject(user.ID), controlplane.MerchantRoleOwner), "assign merchant admin role")
	token, _, err := core.MintAccessToken(ctx, user.ID, nil)
