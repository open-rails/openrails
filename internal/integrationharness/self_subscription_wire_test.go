...
func TestSelfSubscriptionWireParity(t *testing.T) {
	...
	handler, err := httptesthost.Handler(rt, httptesthost.Options{HTTP: embed.HTTPConfig{CustomerRoutes: []embed.CustomerRoutesConfig{{Treasury:true}}, Gate: httproutes.NewGate(httproutes.GateOptions{DelegatedAuthenticator: authn})}, DelegatedAuthenticator: authn})
	require.NoError(t, err)
