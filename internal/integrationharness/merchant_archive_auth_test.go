...
func archiveEmbeddedClient(t *testing.T, dsn, schema string, mid merchant.ID) *openrails.Client {
	...
	rt, err := embed.New(t.Context(), embed.Options{
		Config: &config.Config{Encryption: &config.EncryptionConfig{MasterKey: "AAECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8="}, TestMode: config.CredentialPostureSandbox, ProviderWriteMode: config.ProviderWriteModeReadOnly, SecretBackend: config.SecretBackendDB, DB: &config.DBConfig{URL: dsn, Schema: schema}},
