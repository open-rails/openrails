...
func TestQualifiedOneTimeSaleClientAcrossEmbeddedAndHTTP(t *testing.T) {
	...
	cfg := &config.Config{TestMode: config.CredentialPostureSandbox, MerchantConfigSource: config.MerchantConfigSourceAPI, AllowCatalogUpdates: true, SecretBackend: config.SecretBackendDB, ProviderWriteMode: config.ProviderWriteModeFull, DB: &config.DBConfig{URL: h.DSN}}
	configure(cfg)
	...
	for name, client := range map[string]*openrails.Client{"embedded": embedded, "http": remote} {
		for _, token := range []bool{false, true} {
			...
			t.Run(name+"/"+mode, func(t *testing.T) {
				...
				request := openrails.CreateCheckoutSessionRequest{Customer: openrails.CheckoutCustomerIdentity{ID: openrails.CustomerID(customer).String(), VerifiedEmail: "sale@example.test", Username: "sale-buyer"}, PriceID: price.ID, IdempotencyKey: uuid.NewString(), PaymentOptions: openrails.CheckoutPaymentOptions{Rail: "loopback", PaymentMethodID: openrails.PaymentMethodID(method).String()}}
