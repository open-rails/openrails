...
func TestCustomerPayNowAcrossDeployments(t *testing.T) {
	...
	runMoneyDeployments(t, h, gateway, func(t *testing.T, d moneyDeployment) {
		...
		payNow := func(f CollectionFixture, key string, method uuid.UUID) (*openrails.InvoicePayNowResult, error) {
			return client.PayInvoiceNow(ctx, openrails.PayInvoiceNowRequest{CustomerID: openrails.CustomerID(f.Customer), InvoiceID: f.Invoice, PaymentMethodID: uuid.UUID(method), IdempotencyKey: key})
		}
		...
		t.Run("success", func(t *testing.T) {
			...
			require.Equal(t, uuid.UUID(fixture.Method), *result.Attempt.PaymentMethodID)
		...
		t.Run("idempotency_key_bound_to_request", func(t *testing.T) {
			...
			_, err = client.PayInvoiceNow(ctx, openrails.PayInvoiceNowRequest{CustomerID: openrails.CustomerID(fixture.Customer), InvoiceID: second, PaymentMethodID: uuid.UUID(fixture.Method), IdempotencyKey: key})
			...
			paid, err := client.PayInvoiceNow(ctx, openrails.PayInvoiceNowRequest{CustomerID: openrails.CustomerID(fixture.Customer), InvoiceID: second, PaymentMethodID: uuid.UUID(fixture.Method), IdempotencyKey: "second-" + uuid.NewString()[:8]})
			require.NoError(t, err)
			require.Equal(t, "settled", paid.Attempt.Status)
		...
		t.Run("other_customer_refused", func(t *testing.T) {
			...
			_, err := client.PayInvoiceNow(ctx, openrails.PayInvoiceNowRequest{CustomerID: openrails.CustomerID(other.Customer), InvoiceID: fixture.Invoice, PaymentMethodID: uuid.UUID(other.Method), IdempotencyKey: "cross-" + uuid.NewString()[:8]})
		...
		t.Run("lost_response_converges_after_restart", func(t *testing.T) {
			...
			d.start()
			client = d.client()
			_, err = client.PayInvoiceNow(ctx, openrails.PayInvoiceNowRequest{CustomerID: openrails.CustomerID(fixture.Customer), InvoiceID: fixture.Invoice, PaymentMethodID: uuid.UUID(fixture.Method), IdempotencyKey: "after-restart-" + uuid.NewString()[:8]})
			requireRefusal(t, err, openrails.ErrConflict, openrails.CodeInvoiceRetryOutcomeUnknown)
			...
			settled, err := client.PayInvoiceNow(ctx, openrails.PayInvoiceNowRequest{CustomerID: openrails.CustomerID(fixture.Customer), InvoiceID: fixture.Invoice, PaymentMethodID: uuid.UUID(fixture.Method), IdempotencyKey: key})
			require.NoError(t, err)
...
func TestCustomerRetryNowAcrossDeployments(t *testing.T) {
	...
	runMoneyDeployments(t, h, gateway, func(t *testing.T, d moneyDeployment) {
		...
		retryNow := func(f SubscriptionFixture, key string) (*openrails.SubscriptionRetryNowResult, error) {
			return client.RetrySubscriptionNow(ctx, openrails.RetrySubscriptionNowRequest{CustomerID: openrails.CustomerID(f.Customer), SubscriptionID: uuid.UUID(f.Subscription), IdempotencyKey: key})
		}
		...
		t.Run("success_renews_and_clears_dunning", func(t *testing.T) {
			...
			sales := gateway.SaleCount()
			fixture := seed()
			before, err := client.GetSubscription(ctx, fixture.Subscription.String())
			require.NoError(t, err)
		...
		t.Run("contradicting_receipt_stays_unknown", func(t *testing.T) {
			...
			sub, err := client.GetSubscription(ctx, fixture.Subscription.String())
			require.NoError(t, err)
		...
		t.Run("idempotency_key_bound_to_request", func(t *testing.T) {
			...
			method := uuid.UUID(other.Method) // the payer's, not this subscription's
			_, err = client.RetrySubscriptionNow(ctx, openrails.RetrySubscriptionNowRequest{CustomerID: openrails.CustomerID(fixture.Customer), SubscriptionID: uuid.UUID(fixture.Subscription), PaymentMethodID: &method, IdempotencyKey: key})
			requireRefusal(t, err, openrails.ErrSubscriptionRetryIdempotencyConflict, openrails.CodeSubscriptionRetryIdempotencyConflict)
		...
		t.Run("other_customer_refused", func(t *testing.T) {
			...
			fixture := seed()
			other := seed()
			_, err := client.RetrySubscriptionNow(ctx, openrails.RetrySubscriptionNowRequest{CustomerID: openrails.CustomerID(other.Customer), SubscriptionID: uuid.UUID(fixture.Subscription), IdempotencyKey: "cross-" + uuid.NewString()[:8]})
			require.ErrorIs(t, err, openrails.ErrNotFound)
			foreign := uuid.UUID(other.Method)
			_, err = client.RetrySubscriptionNow(ctx, openrails.RetrySubscriptionNowRequest{CustomerID: openrails.CustomerID(fixture.Customer), SubscriptionID: uuid.UUID(fixture.Subscription), PaymentMethodID: &foreign, IdempotencyKey: "foreign-" + uuid.NewString()[:8]})
			require.ErrorIs(t, err, openrails.ErrCollectionPaymentMethodInvalid)
			require.Equal(t, sales, gateway.SaleCount())
		...
		t.Run("lost_response_converges_after_restart", func(t *testing.T) {
			...
			d.start()
			client = d.client()
			_, err = client.RetrySubscriptionNow(ctx, openrails.RetrySubscriptionNowRequest{CustomerID: openrails.CustomerID(fixture.Customer), SubscriptionID: uuid.UUID(fixture.Subscription), IdempotencyKey: "after-restart-" + uuid.NewString()[:8]})
			requireRefusal(t, err, openrails.ErrSubscriptionRetryOutcomeUnknown, openrails.CodeSubscriptionRetryOutcomeUnknown)
			require.Equal(t, sales+1, gateway.SaleCount(), "the restarted deployment does not resend")
			...
			settled, err := client.RetrySubscriptionNow(ctx, openrails.RetrySubscriptionNowRequest{CustomerID: openrails.CustomerID(fixture.Customer), SubscriptionID: uuid.UUID(fixture.Subscription), IdempotencyKey: key})
			require.NoError(t, err)
			require.True(t, settled.Replayed)
			require.Equal(t, "succeeded", settled.Operation.Status)
