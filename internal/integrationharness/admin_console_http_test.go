...
func TestAdminCustomersSearch(t *testing.T) {
	...
	productID := uuid.New()
	_, err := pool.Exec(ctx,
		`INSERT INTO billing.products (id, key, display_name, merchant_id) VALUES ($1, $2, 'Cust Search Product', $3)`,
	...
	_, err = pool.Exec(ctx,
		`INSERT INTO billing.subscriptions (product_id, status, rail, user_email, merchant_id, customer_id, psp_id, cancelled_at, cancel_type)
	...
	// The detail profile returns the latest historical email and lists the
	// cancelled subscription with its status.
	status, body := requestJSON(t, http.MethodGet,
	...
	require.Len(t, profile.Subscriptions, 1)
	require.Equal(t, "cancelled", profile.Subscriptions[0].Status)
