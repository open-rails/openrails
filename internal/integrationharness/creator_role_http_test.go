...
func TestCreatorRoleThroughRealAuthKitAndHTTP(t *testing.T) {
	...
	product, err := creator.CreateProduct(ctx, openrails.CreateProductRequest{Key: "creator-product", DisplayName: "Creator product"})
	require.NoError(t, err)
	require.Equal(t, catalog.ID, product.CatalogID)
	_, err = creator.CreatePrice(ctx, openrails.CreatePriceRequest{ProductID: product.ID, Key: "creator-usd", UnitAmount: 1_000_000, Currency: "USD"})
	require.NoError(t, err)
	title := "Creator update"
	updated, err := creator.UpdateProduct(ctx, product.ID, openrails.UpdateProductRequest{DisplayName: &title})
	require.NoError(t, err)
	require.Equal(t, title, updated.DisplayName)
	products, err := creator.ListProducts(ctx, openrails.ProductFilter{})
	require.NoError(t, err)
	...
	merchantProduct, err := admin.CreateProduct(ctx, openrails.CreateProductRequest{Key: "merchant-product", DisplayName: "Merchant product"})
	require.NoError(t, err)
	_, err = creator.GetProduct(ctx, merchantProduct.ID)
	...
	_, err = wide.CreateProduct(ctx, openrails.CreateProductRequest{Key: "forbidden-wide-create", DisplayName: "Forbidden"})
	require.ErrorIs(t, err, openrails.ErrDenied)
	_, err = wide.ListProducts(ctx, openrails.ProductFilter{})
	require.ErrorIs(t, err, openrails.ErrDenied)
	...
	_, err = creator.GetProduct(ctx, product.ID)
