...
func TestSubscriptionRecoveryReceiptRegressions(t *testing.T) {
	...
	t.Run("body_bound_key_and_same_PSP_are_enforced", func(t *testing.T) {
		...
		method := uuid.UUID(other.Method)
		_, err = client.RetrySubscriptionNow(ctx, openrails.RetrySubscriptionNowRequest{CustomerID: openrails.CustomerID(f.Customer), SubscriptionID: uuid.UUID(f.Subscription), PaymentMethodID: &method, IdempotencyKey: key})
		require.ErrorIs(t, err, openrails.ErrSubscriptionRetryIdempotencyConflict)
