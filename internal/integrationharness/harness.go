...
func (r trustingResolver) ResolveAPIKey(context.Context, string) (*controlplane.ResolvedServiceCredential, error) {
	return &controlplane.ResolvedServiceCredential{
		...
		Permissions: []string{string(controlplane.MerchantType.OwnerGrant())},
	}, nil
}
...
// SetMerchantHostConfig sets a merchant's #734 canonical API host through the
// REAL merchants.Service the running standalone server itself resolves
// Host->merchant from (server.go wires Runtime.Merchants to the exact same
// *merchants.Service instance) — a live DB write, so the very next request
// against the new host resolves immediately on this SAME running server,
// with no restart (the #734 multi-node/no-boot-mount proof). Browser CORS is
// NOT configured here (#765: it's a static per-route-tier policy, not
// per-merchant).
func (s *Surface) SetMerchantHostConfig(mid merchant.ID, apiHost string) {
	h := s.h
	h.t.Helper()
	...
	require.NoError(h.t, s.app.Runtime.Merchants.SetHostConfig(h.ctx, mid, apiHost), "set merchant host config")
}
...
func (s *Surface) RegisterRemoteApplication(slug, ownerMerchantSlug string, role authkit.Role) RemoteAppCaller {
...
func (s *Surface) RegisterRemoteApplicationWithPermissionsClaim(slug, ownerMerchantSlug string, role authkit.Role, permissions []string) RemoteAppCaller {
	perms := append([]string(nil), permissions...)
	return s.registerRemoteApplication(slug, ownerMerchantSlug, role, perms)
}

func (s *Surface) registerRemoteApplication(slug, ownerMerchantSlug string, role authkit.Role, permissionsClaim []string) RemoteAppCaller {
	h := s.h
	...
	if role != "" {
		require.NoError(h.t, core.OperatorAssignGroupRole(h.ctx, controlplane.MerchantGroup(ownerMerchantSlug), authkit.RemoteAppSubject(ra.ID), role),
...
func (s *Surface) RegisterDelegatedIssuer(slug, ownerMerchantSlug string) *DelegatedIssuer {
	...
	require.NoError(h.t, core.OperatorAssignGroupRole(h.ctx, controlplane.MerchantGroup(ownerMerchantSlug), authkit.RemoteAppSubject(ra.ID), controlplane.MerchantRoleOwner),
...
func (s *Surface) MintAPIKey(merchantSlug, name string, permissions []string) string {
	...
	_, secret, err := cp.Core().MintAPIKeyWithOptions(h.ctx, controlplane.MerchantGroup(merchantSlug), authkit.APIKeyMintOptions{
		Name:      name,
		Role:      authkit.Role(roleForPerms(permissions)),
		CreatedBy: createdBy,
	})
	...
	for _, gt := range controlplane.Groups() {
		if gt.Name == controlplane.MerchantType {
			for _, r := range gt.Roles {
				out[string(r.Name)] = r.Permissions
			}
		}
	}
	return out
...
func (h *Harness) mintFreshAPIKey(a *app.App) string {
	...
	_, secret, err := cp.Core().MintAPIKeyWithOptions(h.ctx, controlplane.MerchantGroup(dbtest.TestMerchantSlug), authkit.APIKeyMintOptions{
...
func (h *Harness) ensureAPIKeyActor(cp *controlplane.ControlPlane, merchantSlug string) string {
	...
	require.NoError(h.t, cp.Core().OperatorAssignGroupRole(h.ctx, controlplane.MerchantGroup(merchantSlug), authkit.UserSubject(user.ID), controlplane.MerchantRoleOwner),
