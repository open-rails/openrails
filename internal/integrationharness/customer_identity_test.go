...
func TestOneSubjectHasIndependentMerchantBillingThroughBothClients(t *testing.T) {
	...
	for _, side := range []struct {
		...
		t.Run(side.feature, func(t *testing.T) {
			...
			require.Equal(t, subject, first.CustomerID)
	...
	for _, side := range observations {
		for name, client := range side.clients {
			t.Run(side.feature+"_"+name, func(t *testing.T) {
				...
				receipt, err := client.GetDeposit(ctx, subject.String(), sourceID)
				require.NoError(t, err)
				require.Equal(t, side.receiptID, receipt.ID)
				require.Equal(t, side.amount, receipt.Amount)
				allowed, err := client.HasEntitlement(ctx, subject.String(), side.feature, time.Time{})
				require.NoError(t, err)
				require.True(t, allowed)
				unknown := uuid.NewString()
				upper := strings.ToUpper(subject.String())
				records, err := client.ListActiveEntitlements(ctx, []string{subject.String(), upper, unknown}, time.Time{})
				require.NoError(t, err)
				require.Len(t, records[subject.String()], 1)
				require.Len(t, records[upper], 1)
				require.Empty(t, records[unknown])
				_, err = client.ListActiveEntitlements(ctx, []string{"not-a-uuid"}, time.Time{})
				require.Error(t, err)
				foreignFeature := "access-a"
				if side.feature == foreignFeature {
					foreignFeature = "access-b"
				}
				allowed, err = client.HasEntitlement(ctx, subject.String(), foreignFeature, time.Time{})
				require.NoError(t, err)
