...
func TestDelegatedAdmissionSeam_LivenessAndDBBackedGrant(t *testing.T) {
	...
	handler, err := httptesthost.Handler(rt, httptesthost.Options{HTTP: embed.HTTPConfig{MerchantAPI: true, CustomerRoutes: []embed.CustomerRoutesConfig{{Treasury:true}}, Gate: httproutes.NewGate(httproutes.GateOptions{DelegatedAuthenticator: authn})}, Prefix: "/billing", DelegatedAuthenticator: authn})
	require.NoError(t, err)
