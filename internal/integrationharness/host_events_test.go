...
func TestHostEventsReplayAcrossEmbeddedAndHTTPClients(t *testing.T) {
	...
	require.Equal(t, payer, first[0].Payment.CustomerID, "the settled event names its payer")
	require.Equal(t, price, first[0].Payment.PriceID, "the settled event names its price")
	...
	require.Contains(t, string(body), `"price_id":"`+price.String()+`"`)
