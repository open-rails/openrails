...
func TestSpendCredits_PrepaidFloorsAtBalance(t *testing.T) {
	...
	err = svc.SpendCredits(ctx, money.SpendParams{Payer: &payer, Invoker: "u", Currency: money.DefaultCurrency, Amount: 1500, Key: money.MustIdempotencyKey(money.OpSpend, "s", "x1")})
	...
	err = svc.SpendCredits(ctx, money.SpendParams{Payer: &payer, Invoker: "u", Currency: money.DefaultCurrency, Amount: 600, Key: money.MustIdempotencyKey(money.OpSpend, "s", "x2")})
	require.NoError(t, err)
	bal, _ = svc.GetBalanceForCustomer(ctx, payer, cur)
...
func TestSpendCredits_HybridSpansBalanceThenOwed(t *testing.T) {
	...
	err = svc.SpendCredits(ctx, money.SpendParams{Payer: &payer, Invoker: "u", Currency: money.DefaultCurrency, Amount: 2000, Key: money.MustIdempotencyKey(money.OpSpend, "s", "x1")})
	require.NoError(t, err)
	bal, _ := svc.GetBalanceForCustomer(ctx, payer, cur)
...
func TestSpendCredits_RespectsCreditLimit(t *testing.T) {
	...
	err = svc.SpendCredits(ctx, money.SpendParams{Payer: &payer, Invoker: "u", Currency: money.DefaultCurrency, Amount: 1000, Key: money.MustIdempotencyKey(money.OpSpend, "s", "x2")})
	require.NoError(t, err)
	owed, _ := svc.GetOutstandingOwed(ctx, payer, cur)
	require.Equal(t, int64(1000), owed)

	err = svc.SpendCredits(ctx, money.SpendParams{Payer: &payer, Invoker: "u", Currency: money.DefaultCurrency, Amount: 1, Key: money.MustIdempotencyKey(money.OpSpend, "s", "x3")})
