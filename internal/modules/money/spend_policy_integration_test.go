...
func TestSpendPolicy_DefaultsAllow(t *testing.T) {
	...
	s, err := svc.GetAccountSettings(ctx, payer, money.DefaultCurrency)
	require.NoError(t, err)
	...
	dec, err := svc.CheckSpendAllowed(ctx, payer, money.DefaultCurrency, "user:alice", 999_999)
	require.NoError(t, err)
...
func TestSpendPolicy_DailyCap(t *testing.T) {
	...
	deny, err := svc.CheckSpendAllowed(ctx, payer, money.DefaultCurrency, "user:a", 600) // 500+600 > 1000
	require.NoError(t, err)
	...
	allow2, err := svc.CheckSpendAllowed(ctx, payer, money.DefaultCurrency, "user:a", 200) // 800+200 == 1000
	require.NoError(t, err)
...
func TestSpendPolicy_PerInvokerCap(t *testing.T) {
	svc, _, payer, _, ctx := spendTestEnv(t)
	day := int64(100)
	_, err := svc.SetSpendLimit(ctx, payer, money.DefaultCurrency, "user:alice", &day, nil)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.False(t, deny.Allowed)
	require.Equal(t, money.DenyInvokerDailyCap, deny.DenyCode)

	allow, err := svc.CheckSpendAllowed(ctx, payer, money.DefaultCurrency, "user:alice", 20) // 80+20 == 100
	require.NoError(t, err)
	require.True(t, allow.Allowed)

	// bob has no per-invoker limit and there's no org cap -> allowed even for a big estimate.
	bob, err := svc.CheckSpendAllowed(ctx, payer, money.DefaultCurrency, "user:bob", 100_000)
	require.NoError(t, err)
...
func TestSpendPolicy_OutstandingCeiling(t *testing.T) {
	...
	allow, err := svc.CheckSpendAllowed(ctx, payer, money.DefaultCurrency, "user:a", 200) // 800+200 == 1000
	require.NoError(t, err)
	require.True(t, allow.Allowed)
}
...
func TestSpendPolicy_SettingsRoundTrip(t *testing.T) {
	...
	thr := int64(2000)
	pct := 90
	out, err := svc.UpsertAccountSettings(ctx, payer, money.DefaultCurrency, money.AccountSettingsInput{
		BillingMode: strptr(money.BillingModeArrears), MaxSpendPerDay: &day,
		MaxSpendPerMonth: &mon, LowBalanceThreshold: &thr, AlertThresholdPct: &pct,
	})
	require.NoError(t, err)
	require.Equal(t, money.BillingModeArrears, out.BillingMode)
	require.Equal(t, int64(5000), *out.MaxSpendPerDay)
	require.Equal(t, 90, out.AlertThresholdPct)

	got, err := svc.GetAccountSettings(ctx, payer, money.DefaultCurrency)
	require.NoError(t, err)
	require.Equal(t, int64(100000), *got.MaxSpendPerMonth)
	require.Equal(t, int64(2000), *got.LowBalanceThreshold)

	// Update one field; others persist.
	day2 := int64(7000)
	_, err = svc.UpsertAccountSettings(ctx, payer, money.DefaultCurrency, money.AccountSettingsInput{MaxSpendPerDay: &day2})
	require.NoError(t, err)
	got2, err := svc.GetAccountSettings(ctx, payer, money.DefaultCurrency)
	require.NoError(t, err)
	require.Equal(t, int64(7000), *got2.MaxSpendPerDay)
	require.Equal(t, money.BillingModeArrears, got2.BillingMode, "unset field must persist")

	// Invalid billing mode rejected.
	_, err = svc.UpsertAccountSettings(ctx, payer, money.DefaultCurrency, money.AccountSettingsInput{BillingMode: strptr("weird")})
	require.Error(t, err)
}
