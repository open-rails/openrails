...
func TestRecordUsage_DebitsAndAggregates(t *testing.T) {
	...
	_, err := svc.Deposit(ctx, money.DepositParams{
		TenantSubjectID: &payer, Actor: payer.UUID().String(), Amount: 100_000, Source: "seed",
	})
	...
	bal, err := svc.GetBalanceForTenantSubject(ctx, payer, cur)
	require.NoError(t, err)
...
func TestRecordUsage_Idempotent(t *testing.T) {
	...
	_, err := svc.Deposit(ctx, money.DepositParams{
		TenantSubjectID: &payer, Actor: payer.UUID().String(), Amount: 10_000, Source: "seed",
	})
	require.NoError(t, err)
...
func TestRecordUsage_ZeroCostNoDebit(t *testing.T) {
	...
	_, err := svc.Deposit(ctx, money.DepositParams{
		TenantSubjectID: &payer, Actor: payer.UUID().String(), Amount: 5_000, Source: "seed",
	})
	require.NoError(t, err)
