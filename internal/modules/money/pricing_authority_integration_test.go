...
func TestPricingAuthoritySeparatesFinalCapturesFromCatalogInputs(t *testing.T) {
	...
	t.Cleanup(func() {
		...
		_, _ = pool.Exec(ctx, `DELETE FROM billing.products WHERE merchant_id=$1 AND id=$2`, merchantID, productID)
	})
	require.NoError(t, pool.QueryRow(ctx, `SELECT 1`).Scan(new(int)))
	...
	_, err = pool.Exec(ctx, `INSERT INTO billing.products (id,key,display_name,merchant_id) VALUES ($1,$2,'Pricing authority',$3)`, productID, "authority-product-"+uuid.NewString(), merchantID)
	require.NoError(t, err)
	_, err = pool.Exec(ctx, `INSERT INTO billing.catalog_meters (merchant_id,key,event_type,value_property,aggregation) VALUES ($1,$2,$2,$2,'sum')`, merchantID, meterKey)
	require.NoError(t, err)
	_, err = pool.Exec(ctx, `INSERT INTO billing.catalog_rate_cards (merchant_id,product_id,ordinal,meter_key,payment_term,price) VALUES ($1,$2,1,$3,'in_arrears','{"model":"per_unit","currency":"USD","per_unit":{"unit_amount":"100","divide_by":1}}'::jsonb)`, merchantID, productID, meterKey)
	require.NoError(t, err)
	...
	rows, err := pool.Query(ctx, `SELECT pricing_authority FROM billing.usage_events WHERE customer_id=$1 AND event_type=$2 ORDER BY created_at`, payer.UUID(), meterKey)
	require.NoError(t, err)
	for rows.Next() {
