...
func windowEnv(t *testing.T, depositMicros int64) (context.Context, *pgxpool.Pool, *credits.MoneyService, identity.TenantSubjectID, string) {
	t.Helper()
	...
	svc := credits.NewMoneyService(dbi)
...
func requireBalance(t *testing.T, ctx context.Context, svc *credits.MoneyService, payer identity.TenantSubjectID, ctName string, balance, held int64) {
	t.Helper()
