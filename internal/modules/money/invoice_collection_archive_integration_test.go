...
func testInvoiceCollectionArchive(t *testing.T, resolution string) {
	...
	_, err := database.Pool().Exec(ctx, `INSERT INTO openrails.merchants(id,slug) VALUES($1,$2)`, mid.UUID(), "archive-"+mid.String())
	require.NoError(t, err)
	payer, method, psp := identity.CustomerIDFromString(uuid.NewString()), uuid.New(), uuid.New()
	account := "archive-" + uuid.NewString()
	_, err = database.Pool().Exec(ctx, `INSERT INTO openrails.customers(merchant_id,id) VALUES($1,$2)`, mid.UUID(), payer.UUID())
	require.NoError(t, err)
