...
func TestSweepUsage_AppRoleMerchantScoped_RatesNonZero(t *testing.T) {
	...
	t.Cleanup(func() {
		_, _ = superPool.Exec(ctx, "DELETE FROM billing.invoice_items WHERE customer_id = $1", payer.UUID())
		_, _ = superPool.Exec(ctx, "DELETE FROM billing.metered_rating_watermarks WHERE customer_id = $1", payer.UUID())
		_, _ = superPool.Exec(ctx, "DELETE FROM billing.ledger_transfers WHERE customer_id = $1", payer.UUID())
		_, _ = superPool.Exec(ctx, "DELETE FROM billing.usage_events WHERE customer_id = $1", payer.UUID())
		_, _ = superPool.Exec(ctx, "DELETE FROM billing.money_settings WHERE customer_id = $1", payer.UUID())
		_, _ = superPool.Exec(ctx, "DELETE FROM billing.catalog_rate_cards WHERE merchant_id = $1 AND product_id = $2", merchantID, productID)
		_, _ = superPool.Exec(ctx, "DELETE FROM billing.catalog_meters WHERE merchant_id = $1 AND key = $2", merchantID, meterKey)
		_, _ = superPool.Exec(ctx, "DELETE FROM billing.products WHERE id = $1", productID)
		_, _ = superPool.Exec(ctx, "DELETE FROM billing.customers WHERE id = $1", payer.UUID())
	})

	// Seed catalog + usage with the RLS-bypassing super role.
	_, err = superPool.Exec(ctx, `INSERT INTO billing.customers (id, merchant_id) VALUES ($1, $2)`, payer.UUID(), merchantID)
	require.NoError(t, err)
	_, err = superPool.Exec(ctx, `INSERT INTO billing.products (id, key, display_name, merchant_id) VALUES ($1, $2, 'RLS Rating', $3)`,
		productID, "rls-rating-"+uuid.NewString(), merchantID)
	require.NoError(t, err)
	_, err = superPool.Exec(ctx, `
INSERT INTO billing.catalog_meters (merchant_id, key, event_type, value_property, aggregation, unit)
...
VALUES ($1, $2, 1, $3, 'in_arrears', '{"model":"per_unit","currency":"USD","per_unit":{"unit_amount":"10000"}}'::jsonb)`,
	...
	_, err = superPool.Exec(ctx, `
INSERT INTO billing.usage_events (merchant_id, customer_id, invoker_id, currency, event_type, dimensions, amount, source, source_id, pricing_authority, occurred_at)
