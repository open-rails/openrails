...
func seedSpendable(t *testing.T, ctx context.Context, svc *credits.MoneyService, creditType string, userID string, amount int64) {
	t.Helper()
	src := uuid.New()
...
func TestReserveCapturePartial_ConservesTotal(t *testing.T) {
	dbi, _, ctx := startOwnerTenantPostgres(t)
	svc := credits.NewMoneyService(dbi, clockwork.NewRealClock())
