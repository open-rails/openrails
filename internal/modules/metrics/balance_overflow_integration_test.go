...
func TestMetrics_BalanceSeriesRefusesOverflow(t *testing.T) {
	...
	merchantID := uuid.New()
	exec(ctx, t, pool, `INSERT INTO billing.merchants (id, slug, status) VALUES ($1, $2, 'active')`, merchantID, "metrics-overflow-"+merchantID.String())
	for i, source := range []string{"processor_clearing", "world"} {
		customer, debit, credit := uuid.New(), uuid.New(), uuid.New()
		exec(ctx, t, pool, `INSERT INTO billing.customers (id, merchant_id) VALUES ($1, $2)`, customer, merchantID)
		exec(ctx, t, pool, `INSERT INTO billing.ledger_accounts (id, merchant_id, account_type, currency) VALUES ($1, $2, $3, 'USD')`, debit, merchantID, source)
		exec(ctx, t, pool, `INSERT INTO billing.ledger_accounts (id, merchant_id, customer_id, account_type, currency, debits_must_not_exceed_credits) VALUES ($1, $2, $3, 'customer_balance', 'USD', true)`, credit, merchantID, customer)
		id := uuid.New()
		exec(ctx, t, pool, `INSERT INTO billing.ledger_transfers (id, merchant_id, debit_account_id, credit_account_id, amount, currency, transfer_type, customer_id, created_at, operation, source, source_id) VALUES ($1, $2, $3, $4, $5, 'USD', 'deposit', $6, $7, 'deposit', 'metrics-overflow', $1::uuid::text)`, id, merchantID, debit, credit, int64(math.MaxInt64/2+1), customer, time.Date(2026, time.Month(5+i), 15, 0, 0, 0, 0, time.UTC))
	}
