...
var Measures = []Measure{
	...
	{Name: "payment_failures", Class: ClassAdditive, Family: FamPayments, Unit: "count",
		...
		Dims:        []string{"currency", "rail", "rail_account", "stream", "product_id", "price_id", "card_brand", "attempt_kind", "failure_reason"}},
	...
	{Name: "payment_attempts", Class: ClassAdditive, Family: FamPayments, Unit: "count", Internal: true,
		...
		Dims: []string{"currency", "rail", "rail_account", "stream", "product_id", "price_id", "card_brand", "attempt_kind"}},
	// --- payments: ratios ------------------------------------------------------
	{Name: "approval_rate", Class: ClassRatio, Unit: "ratio", Num: "payment_count", Den: "payment_attempts",
		Description: "settled payments / (settled + failed) attempts",
...
var Caveats = []string{
	...
	"failure_reason/attempt_kind exist from instrumentation onward; older and imported rows read 'unknown'. CCBill renewal declines depend on CCBill webhook delivery; Stripe renewal-decline attempts are visible via subscription status transitions but may lack per-attempt failed payment rows; Solana has no card-decline taxonomy (on-chain failure codes are recorded verbatim)",
