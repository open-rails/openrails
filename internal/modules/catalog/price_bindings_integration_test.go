...
func TestPriceBindingsKeepPSPIdentityThroughCollisionRenameArchive(t *testing.T) {
	...
	_, err := database.Qx(ctx).Exec(ctx, `INSERT INTO openrails.products (id, merchant_id, key, display_name) VALUES ($1,$2,$3,'Bindings')`, productID, dbtest.TestMerchantID.UUID(), uuid.NewString())
	require.NoError(t, err)
