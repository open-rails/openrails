...
func TestFinalizeInvoice_PrepaidStatement(t *testing.T) {
	...
	t.Cleanup(func() {
		...
		_, _ = bunDB.NewDelete().Model((*models.Invoice)(nil)).Where("owner_id = ?", owner.UUID()).Exec(ctx)
	})

	_, err := svc.Deposit(ctx, credits.CreditDepositParams{OwnerID: &owner, UserID: owner.UUID().String(), CreditType: ct, Amount: 100_000, Source: "purchase"})
	require.NoError(t, err)
	rec := func(et string, amt int64, dims map[string]int64, sid string) {
		_, e := svc.RecordUsage(ctx, credits.RecordUsageParams{Owner: &owner, UserID: "u", CreditType: ct, EventType: et, Dimensions: dims, Amount: amt, Source: "req", SourceID: sid})
		require.NoError(t, e)
	}
	...
	inv, err := svc.FinalizeInvoice(ctx, owner, ct, from, to)
	require.NoError(t, err)
	...
	// Idempotent.
	inv2, err := svc.FinalizeInvoice(ctx, owner, ct, from, to)
	require.NoError(t, err)
...
func TestFinalizeInvoice_ArrearsOwed(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = svc.Deposit(ctx, credits.CreditDepositParams{OwnerID: &owner, UserID: owner.UUID().String(), CreditType: ct, Amount: 1_000, Source: "seed"})
	require.NoError(t, err)
	_, err = svc.RecordUsage(ctx, credits.RecordUsageParams{Owner: &owner, UserID: "u", CreditType: ct, EventType: "gpt-4o", Amount: 1_500, Source: "req", SourceID: "r1"})
	require.NoError(t, err)
...
func TestFinalizeDueInvoices_EnumeratesAccount(t *testing.T) {
	...
	_, err := svc.Deposit(ctx, credits.CreditDepositParams{OwnerID: &owner, UserID: owner.UUID().String(), CreditType: ct, Amount: 5_000, Source: "seed"})
	require.NoError(t, err)
	_, err = svc.RecordUsage(ctx, credits.RecordUsageParams{Owner: &owner, UserID: "u", CreditType: ct, EventType: "gpt-4o", Amount: 2_000, Source: "req", SourceID: "r1"})
	require.NoError(t, err)
	...
	// This owner's invoice exists with the right usage total (bunDB bypasses RLS).
	inv := new(models.Invoice)
	require.NoError(t, bunDB.NewSelect().Model(inv).Where("owner_id = ?", owner.UUID()).Limit(1).Scan(ctx))
