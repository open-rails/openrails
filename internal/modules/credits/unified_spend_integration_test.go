...
func TestSpendCredits_PrepaidFloorsAtBalance(t *testing.T) {
	...
	err = svc.SpendCredits(ctx, credits.SpendParams{Payer: &payer, Actor: "u", CreditType: ct, Amount: 600, Source: "s", SourceID: "x2"})
	require.NoError(t, err)
	bal, _ = svc.GetBalanceForTenantSubject(ctx, payer, ct)
...
func TestSpendCredits_HybridSpansBalanceThenOwed(t *testing.T) {
	...
	err = svc.SpendCredits(ctx, credits.SpendParams{Payer: &payer, Actor: "u", CreditType: ct, Amount: 2000, Source: "s", SourceID: "x1"})
	require.NoError(t, err)
	bal, _ := svc.GetBalanceForTenantSubject(ctx, payer, ct)
...
func TestSpendCredits_RespectsCreditLimit(t *testing.T) {
	...
	err = svc.SpendCredits(ctx, credits.SpendParams{Payer: &payer, Actor: "u", CreditType: ct, Amount: 1000, Source: "s", SourceID: "x2"})
	require.NoError(t, err)
	owed, _ := svc.GetOutstandingOwed(ctx, payer, ct)
	require.Equal(t, int64(1000), owed)

	err = svc.SpendCredits(ctx, credits.SpendParams{Payer: &payer, Actor: "u", CreditType: ct, Amount: 1, Source: "s", SourceID: "x3"})
...
func TestSpendCredits_Idempotent(t *testing.T) {
	...
	_, err := svc.Deposit(ctx, credits.CreditDepositParams{TenantSubjectID: &payer, Actor: payer.UUID().String(), CreditType: ct, Amount: 1000, Source: "seed"})
	require.NoError(t, err)
	p := credits.SpendParams{Payer: &payer, Actor: "u", CreditType: ct, Amount: 300, Source: "s", SourceID: "dup"}
...
func TestRecordUsage_ArrearsDrawsThenAccrues(t *testing.T) {
	...
	_, err = svc.RecordUsage(ctx, credits.RecordUsageParams{
		Payer: &payer, Actor: "u", CreditType: ct, EventType: "gpt-4o",
