...
func TestGraduateTier_ByCumulativePaid(t *testing.T) {
	...
	dep := func(amt int64) {
		_, e := svc.Deposit(ctx, credits.CreditDepositParams{OwnerID: &owner, UserID: owner.UUID().String(), CreditType: ct, Amount: amt, Source: "pay"})
		require.NoError(t, e)
	}

	// No paid spend -> free.
	tier, err := svc.GraduateTier(ctx, owner, ct, ladder)
	...
	// Persisted + readable.
	got, err := svc.GetTier(ctx, owner, ct)
	require.NoError(t, err)
