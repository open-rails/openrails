...
func TestSuspensionAndVerificationState(t *testing.T) {
	...
	require.NoError(t, svc.Resume(ctx, owner, ct))
	suspended, err = svc.IsSuspended(ctx, owner, ct)
	require.NoError(t, err)
