...
func TestConfirmTierChange_RollsBackPartialMirrorAndRetries(t *testing.T) {
	...
	t.Cleanup(func() {
		_, _ = dbi.Pool().Exec(ctx, `DELETE FROM billing.notifications WHERE customer_id=$1`, customerID)
		_, _ = dbi.Pool().Exec(ctx, `DELETE FROM billing.entitlements WHERE customer_id=$1`, customerID)
		_, _ = dbi.Pool().Exec(ctx, `DELETE FROM billing.grants WHERE customer_id=$1`, customerID)
		_, _ = dbi.Pool().Exec(ctx, `DELETE FROM billing.payments WHERE customer_id=$1`, customerID)
		_, _ = dbi.Pool().Exec(ctx, `DELETE FROM billing.solana_subscriptions WHERE subscription_id IN (SELECT id FROM billing.subscriptions WHERE customer_id=$1)`, customerID)
		_, _ = dbi.Pool().Exec(ctx, `DELETE FROM billing.subscriptions WHERE customer_id=$1`, customerID)
		_, _ = dbi.Pool().Exec(ctx, `DELETE FROM billing.prices WHERE id=ANY($1)`, []uuid.UUID{oldPriceID, newPriceID})
		_, _ = dbi.Pool().Exec(ctx, `DELETE FROM billing.products WHERE id=ANY($1)`, []uuid.UUID{oldProductID, newProductID})
	})
	...
	var queued int
	require.NoError(t, dbi.Pool().QueryRow(ctx, `SELECT count(*) FROM billing.notifications WHERE customer_id=$1`, customerID).Scan(&queued))
...
func assertTierChangeState(t *testing.T, ctx context.Context, dbi *db.DB, customerID uuid.UUID, tierGroup string, oldSubID, newSubID uuid.UUID, wantLive int, wantOldStatus models.SubscriptionStatus) {
	...
	require.NoError(t, dbi.Pool().QueryRow(ctx, `SELECT status FROM billing.subscriptions WHERE id=$1`, oldSubID).Scan(&oldStatus))
	require.Equal(t, wantOldStatus, oldStatus)
	if newSubID != uuid.Nil {
		var newStatus models.SubscriptionStatus
		require.NoError(t, dbi.Pool().QueryRow(ctx, `SELECT status FROM billing.subscriptions WHERE id=$1`, newSubID).Scan(&newStatus))
