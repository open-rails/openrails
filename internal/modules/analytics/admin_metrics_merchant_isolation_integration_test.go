...
// TestAdminMetricsCrossMerchantIsolation proves the issue #232 fix end to end:
// with daily_metrics rows seeded for two distinct merchants, every
// AdminMetricsService read scoped to merchant A returns ONLY merchant A's data and
// never merchant B's, while the explicit platform-superadmin cross-merchant path
// sees both. This is the regression guard for the original leak where a merchant
...
func TestAdminMetricsCrossMerchantIsolation(t *testing.T) {
	...
	ctxA := merchant.WithID(ctx, merchantA)
	...
	if summary[0].NewSubscriptions != 2 {
		t.Fatalf("GetSummary(A) new_subscriptions = %d, want 2 (merchant B leaked)", summary[0].NewSubscriptions)
	}
	...
	if revSub != 1000 {
		t.Fatalf("GetRevenueSeries(A) subscription revenue = %d, want 1000 (merchant B leaked)", revSub)
	}
	...
	for _, c := range churn {
		for _, rc := range c.CancellationReasons {
			if rc.Reason == "user" && rc.Count != 1 {
				t.Fatalf("GetChurn(A) user cancellations = %d, want 1 (merchant B's 3 leaked)", rc.Count)
			}
		}
	}

	// --- Merchant B sees only its own data (symmetry) ---
	ctxB := merchant.WithID(ctx, merchantB)
...
// merchant_id. We use a plain MergeTree (no Keeper) so the test runs against a
...
func seedDailyMetric(t *testing.T, ctx context.Context, conn driver.Conn, day time.Time, currency string, merchantID merchant.ID, s dailySeed) {
	...
	ent := int64(0)
	if err := batch.Append(
		day, currency, merchantID.String(),
