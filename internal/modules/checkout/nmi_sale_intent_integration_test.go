...
func TestNMISaleIntent_DelayedReceiptNeverResubmits(t *testing.T) {
	...
	require.NoError(t, err)
	retained, err := intents.NewStore(fx.db).Get(fx.ctx, intent.ID)
	require.NoError(t, err)
	require.Equal(t, intents.StatusUnknownNeedsVerify, retained.Status, "other pending merchant work must not make this sale executable")
	require.EqualValues(t, 1, retained.Attempts)
	replay := fx.enqueueAndExecute(t, key)
	require.Equal(t, intent.ID, replay.ID)
	require.Equal(t, intents.StatusUnknownNeedsVerify, replay.Status)
