...
func TestDurableAdmissionIdentityCaptureAndWindows(t *testing.T) {
	...
	captured, err := f.money.CaptureAdmission(f.ctx, "first", 500, nil)
	require.NoError(t, err)
	require.NotNil(t, captured.LedgerTransferID)
	require.EqualValues(t, 500, f.used(t))
	replay, err := f.money.CaptureAdmission(f.ctx, "first", 500, nil)
	require.NoError(t, err)
	require.True(t, replay.Replayed)
	require.Equal(t, captured.LedgerTransferID, replay.LedgerTransferID)
	_, err = f.money.CaptureAdmission(f.ctx, "first", 501, nil)
	...
	require.EqualValues(t, 500, f.used(t))
	_, err = f.money.CaptureAdmission(f.ctx, "late", 50, nil)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.True(t, decision.Allowed)
	_, err = f.money.CaptureAdmission(f.ctx, zero.RequestID, 20, nil)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.True(t, decision.Allowed)
	free, err := f.money.CaptureAdmission(f.ctx, zero.RequestID, 0, nil)
	require.NoError(t, err)
	require.Nil(t, free.LedgerTransferID)
	require.Zero(t, free.Amount)
	require.EqualValues(t, 570, f.used(t))
	_, err = f.money.CaptureAdmission(f.ctx, "never-admitted", 10, nil)
