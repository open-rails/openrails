...
func TestAdmit_ThroughputDeny(t *testing.T) {
	...
	req := admission.AdmitRequest{CustomerID: payer, Actor: "user:a", Tier: "free", Resource: "gpt-4o",
...
func TestAdmit_Allow(t *testing.T) {
	...
	require.NoError(t, store.UpsertTierPolicy(ctx, payer, "free",
		[]models.ThroughputWindow{{Unit: "request", WindowSeconds: 60, Max: 100}}))
	_, err := cs.Deposit(ctx, money.DepositParams{CustomerID: &payer, Actor: payer.UUID().String(), Amount: 10_000, Source: "seed"})
	require.NoError(t, err)
...
func TestAdmit_EndpointGating(t *testing.T) {
	...
	d, err := adm.Admit(ctx, admission.AdmitRequest{CustomerID: payer, Actor: "user:a", Tier: "free",
...
func TestAdmit_BudgetDeny(t *testing.T) {
	...
	_, err := cs.Deposit(ctx, money.DepositParams{CustomerID: &payer, Actor: payer.UUID().String(), Amount: 100_000, Source: "seed"})
	require.NoError(t, err)

	// Budget windows reserve the estimate 1:1 in micros (#337/#463).
	// First request: 400 against the 500/hour budget -> allowed.
	d, err := adm.Admit(ctx, admission.AdmitRequest{CustomerID: payer, Actor: "user:a", Tier: "free", Resource: "gpt-4o",
...
func TestAdmit_BudgetReservedEqualsEstimate(t *testing.T) {
	adm, cs, store, payer, _, ctx, _ := admitEnv(t)
	_, err := cs.Deposit(ctx, money.DepositParams{CustomerID: &payer, Actor: payer.UUID().String(), Amount: 10_000_000, Source: "seed"}) // $10
	require.NoError(t, err)
	...
	dec, err := adm.Admit(ctx, admission.AdmitRequest{
		CustomerID: payer, Actor: "actor-parity", Tier: "paid",
