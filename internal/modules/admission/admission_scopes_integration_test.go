...
// plus the budget-policy store + tier-policy store, a fresh payer, and a funded
// credit type so the money axis never blocks the budget-scope tests.
func scopeEnv(t *testing.T) (*admission.Admitter, *admission.BudgetPolicyStore, *admission.TierPolicyStore, identity.TenantSubjectID, string, context.Context) {
	t.Helper()
	...
	ctName := "scope_" + uuid.NewString()
	ctID := uuid.New()
	_, err := gen.New(pool).CreateCreditType(ctx, gen.CreateCreditTypeParams{
		ID: ctID, Name: ctName, DisplayName: "Scope Test", Unit: "cents", DecimalPlaces: 2, IsActive: true, CreatedAt: now,
	})
	...
	_, err = cs.Deposit(ctx, credits.CreditDepositParams{TenantSubjectID: &payer, Actor: payer.UUID().String(), CreditType: ctName, Amount: 1_000_000_000, Source: "seed"})
	require.NoError(t, err)
	...
	return adm, bpStore, tierStore, payer, ctName, ctx
}

func mustReq(payer identity.TenantSubjectID, ct, actor, srcID string, roles []uuid.UUID, micros int64) admission.AdmitRequest {
	return admission.AdmitRequest{
		TenantSubjectID: payer, Actor: actor, Tier: "free", Resource: "gpt-4o",
		Roles: roles, Amounts: map[string]int64{"request": 1}, CreditType: ct,
...
func TestScope_PlatformCapDeniesEvenWhenRoleAndInvokerUnder(t *testing.T) {
	adm, bp, tier, payer, ct, ctx := scopeEnv(t)
	role := uuid.New()
	...
	d, err := adm.Admit(ctx, mustReq(payer, ct, "user:a", "s1", []uuid.UUID{role}, 300))
	require.NoError(t, err)
	require.True(t, d.Allowed)

	// Second request 200 micros: platform window 300+200=500 > 400 -> deny on platform.
	d, err = adm.Admit(ctx, mustReq(payer, ct, "user:a", "s2", []uuid.UUID{role}, 200))
	require.NoError(t, err)
...
func TestScope_RolePoolSharedAcrossTwoUsers(t *testing.T) {
	...
	d, err := adm.Admit(ctx, mustReq(payer, ct, "user:a", "r1", []uuid.UUID{role}, 300))
	require.NoError(t, err)
	require.True(t, d.Allowed)

	// user:b (same role) spends 200 -> pool now 500, still allowed.
	d, err = adm.Admit(ctx, mustReq(payer, ct, "user:b", "r2", []uuid.UUID{role}, 200))
	require.NoError(t, err)
	require.True(t, d.Allowed)

	// user:b again, 100 -> pool 600 > 500 -> deny (shared across both users).
	d, err = adm.Admit(ctx, mustReq(payer, ct, "user:b", "r3", []uuid.UUID{role}, 100))
	require.NoError(t, err)
	require.False(t, d.Allowed)
	require.Equal(t, "budget", d.BlockedBy)
}
...
func TestScope_InvokerCapIndependent(t *testing.T) {
	adm, _, tier, payer, ct, ctx := scopeEnv(t)
	...
	d, err := adm.Admit(ctx, mustReq(payer, ct, "user:a", "i1", nil, 400))
	require.NoError(t, err)
	require.True(t, d.Allowed)
	d, err = adm.Admit(ctx, mustReq(payer, ct, "user:a", "i2", nil, 100))
	require.NoError(t, err)
	require.False(t, d.Allowed, "user:a is over its own cap")

	// user:b unaffected — independent (subject, actor) bucket.
	d, err = adm.Admit(ctx, mustReq(payer, ct, "user:b", "i3", nil, 400))
	require.NoError(t, err)
...
func TestScope_OverSubscriptionFlooredByBalanceNotBudget(t *testing.T) {
	...
	d, err := adm.Admit(ctx, mustReq(payer, ct, "user:a", "o1", []uuid.UUID{role}, 500_000))
	require.NoError(t, err)
	require.True(t, d.Allowed)
...
func TestThroughput_PayerAggregatesAcrossInvokers(t *testing.T) {
	...
	req := func(actor, srcID string) admission.AdmitRequest {
		r := mustReq(payer, ct, actor, srcID, nil, 0)
		r.Resource = "rel-A"
		return r
	}
...
func TestThroughput_PerReleaseWindows(t *testing.T) {
	...
	small := func() admission.AdmitRequest { r := mustReq(payer, ct, "user:a", "x", nil, 0); r.Resource = "rel-small"; return r }
	big := func() admission.AdmitRequest { r := mustReq(payer, ct, "user:a", "x", nil, 0); r.Resource = "rel-big"; return r }
...
func TestQueue_AdmitDeniesOnPoolOverflow(t *testing.T) {
	...
	req := func(srcID string) admission.AdmitRequest {
		r := mustReq(payer, ct, "user:a", srcID, nil, 100) // money axis funded
		r.Resource = "rel-q"
		return r
	}
