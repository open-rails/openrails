...
// max_single_charge_micros is rejected up front with the generic deny code.
func TestAdmit_SingleChargeCapDeny(t *testing.T) {
	...
	d, err := adm.Admit(ctx, admission.AdmitRequest{
		CustomerID: payer, Actor: "user:a", Tier: "free", Resource: "gpt-4o",
		Amounts: map[string]int64{"request": 1}, EstimateMicros: 1_001,
...
// plus the new estimate may not exceed the tier's max_concurrent_held_micros.
func TestAdmit_ConcurrentHeldCapDeny(t *testing.T) {
	adm, cs, store, payer, _, ctx, _ := admitEnv(t)
	require.NoError(t, store.UpsertTierPolicyFull(ctx, payer, "free", models.ThroughputPolicy{
		Windows:                 []models.ThroughputWindow{{Unit: "request", WindowSeconds: 60, Max: 100}},
		MaxConcurrentHeldMicros: 1_500,
	}))
	_, err := cs.Deposit(ctx, money.DepositParams{CustomerID: &payer, Actor: payer.UUID().String(), Amount: 1_000_000, Source: "seed"})
	require.NoError(t, err)
	...
	require.Equal(t, int64(1_500), d1.MaxConcurrentHeldMicros)
	require.Equal(t, int64(1_000), d1.HeldMicros, "allowed verdict includes the hold just placed")
	...
	require.Equal(t, int64(1_500), d2.MaxConcurrentHeldMicros)
	require.Equal(t, int64(1_000), d2.HeldMicros, "deny surfaces the pre-existing held sum")
...
func TestAdmit_NoAdmitLimits_Unchanged(t *testing.T) {
	...
	d, err := adm.Admit(ctx, admission.AdmitRequest{
		CustomerID: payer, Actor: "user:a", Tier: "free", Resource: "gpt-4o",
		Amounts: map[string]int64{"request": 1}, EstimateMicros: 900_000,
		Source: "usage", SourceID: "big", ExpiresAt: time.Now().Add(time.Hour),
	})
	require.NoError(t, err)
	require.True(t, d.Allowed)
	require.Equal(t, int64(0), d.MaxSingleChargeMicros)
	require.Equal(t, int64(0), d.MaxConcurrentHeldMicros)
}
