...
func (r *UserSubscriptionResponse) View() openrails.Subscription {
	...
	out.ScheduledProduct = subscriptionProductView(r.ScheduledProduct)
	out.Access = r.Access
	return out
}
...
func SubscriptionView(sub *models.Subscription, price *models.Price, now time.Time) openrails.Subscription {
	out := openrails.Subscription{
		...
		Price:           subscriptionPriceView(price), Product: subscriptionProductView(sub.Product), Card: subscriptionCardView(sub.PaymentMethod),
	}
...
func subscriptionPriceView(p *models.Price) *openrails.SubscriptionPrice {
	...
	return &openrails.SubscriptionPrice{ID: openrails.PriceID(p.ID), Key: p.Key, ProductID: openrails.ProductID(p.ProductID), UnitAmount: p.Amount, Currency: p.Currency, AutoRenew: p.AutoRenew, AccessDurationHours: p.AccessDurationHours, Archived: p.Archived}
}

func subscriptionProductView(p *models.Product) *openrails.SubscriptionProduct {
	if p == nil {
		return nil
	}
	return &openrails.SubscriptionProduct{ID: openrails.ProductID(p.ID), Key: p.Key, DisplayName: p.DisplayName, Description: p.Description, TierGroup: p.TierGroup, TierRank: p.TierRank, Archived: p.Archived}
}
