...
func TestFreezeRebillPinsTheRailAmount(t *testing.T) {
	...
	sub := func(amount int64, currency string) *models.Subscription {
		return &models.Subscription{
			...
			PaymentMethod: &models.PaymentMethod{ID: uuid.New(), PspID: uuid.New(), Custodian: models.CustodianPSP, RailCustomerRef: " vault-1 ", RailMethodRef: "bill-1"},
...
// submitted card data. Recurring rebills require provider-owned custody.
func TestFreezeRebillRefusesCustodianMethod(t *testing.T) {
	...
	require.ErrorIs(t, err, ErrNothingToFreeze)
	require.Empty(t, p.Instrument.RailCustomerRef)
}
