...
func seedMerchant(t *testing.T, pool *pgxpool.Pool, mid uuid.UUID) {
	...
	t.Cleanup(func() {
		...
		for _, tbl := range []string{"merchant_notifications", "alert_rules", "merchant_webhooks", "payments", "prices", "products", "customers", "rail_merchant_accounts", "merchant_configurations"} {
