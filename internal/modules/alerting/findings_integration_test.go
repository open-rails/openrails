...
func TestFindingNotificationPersistenceFailureAndConcurrentRetry(t *testing.T) {
	...
	exec(t, pool, `CREATE FUNCTION billing.reject_test_notification() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN IF NEW.merchant_id = TG_ARGV[0]::uuid THEN RAISE EXCEPTION 'injected notification failure'; END IF; RETURN NEW; END $$`)
	exec(t, pool, `CREATE TRIGGER reject_test_notification BEFORE INSERT ON billing.notifications FOR EACH ROW EXECUTE FUNCTION billing.reject_test_notification('`+mid.String()+`')`)
	t.Cleanup(func() {
		_, _ = pool.Exec(context.Background(), `DROP TRIGGER IF EXISTS reject_test_notification ON billing.notifications; DROP FUNCTION IF EXISTS billing.reject_test_notification()`)
	})
	inConn(t, appDB, mid, func(ctx context.Context) {
	...
	exec(t, pool, `DROP TRIGGER reject_test_notification ON billing.notifications; DROP FUNCTION billing.reject_test_notification()`)
