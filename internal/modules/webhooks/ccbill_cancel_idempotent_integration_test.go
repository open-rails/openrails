...
func TestCCBillCancellationWebhookAfterLocalCancelIsNoOp(t *testing.T) {
	...
	t.Cleanup(func() {
		...
		_, _ = pool.Exec(ctx, "DELETE FROM billing.entitlements WHERE customer_id = $1", custID)
	...
	require.NoError(t, pool.QueryRow(ctx,
		`SELECT status, cancel_type, cancel_feedback, cancelled_at, current_period_ends_at
		   FROM billing.subscriptions WHERE id = $1`, subID).
	...
	require.NoError(t, pool.QueryRow(ctx,
		`SELECT end_at, revoked_at FROM billing.entitlements WHERE id = $1`, entID).Scan(&endAt, &revokedAt))
	...
	require.NoError(t, pool.QueryRow(ctx,
		`SELECT count(*) FROM billing.notifications WHERE customer_id = $1`, custID).Scan(&notifications))
