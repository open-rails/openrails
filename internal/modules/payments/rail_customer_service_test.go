...
// #635: rail_customer_accounts is materialized only for rails with a real card-independent
// remote customer object (Stripe cus_*, NMI customer_vault_id).
func TestRailHasRemoteCustomer(t *testing.T) {
	cases := map[string]bool{
		// #682: NMI vault ids are per-card instrument containers, not persons —
