...
func TestHyperSwitchNMIChargeBoundary(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "unqualified error cannot decline", body: `{"response":{"response":"3","response_code":"400","responsetext":"RAW_PROVIDER_SENTINEL"},"status_code":200,"response_headers":{}}`, wantError: provider.ErrUnknown, wantPosts: 1},
		{name: "missing refusal code", body: `{"response":{"response":"2"},"status_code":200,"response_headers":{}}`, wantError: provider.ErrUnknown, wantPosts: 1},
		{name: "contradictory refusal", body: `{"response":{"response":"2","response_code":"100"},"status_code":200,"response_headers":{}}`, wantError: provider.ErrUnknown, wantPosts: 1},
