...
func (w CCBillReconcileWorker) Work(ctx context.Context, job *river.Job[CCBillReconcileArgs]) error {
	...
	for _, sub := range localActive {
		...
		if _, ok := remoteActive[processorSubID]; !ok {
			missingRemote++
			if err := w.recordDataLinkRepairAlert(ctx, "ccbill_datalink_missing_remote", &sub.ID, sub.CustomerID.String(), processorSubID, nil, nil); err != nil {
	...
	for processorSubID, record := range remoteActive {
		...
		if !ok {
			if alertErr := w.recordDataLinkRepairAlert(ctx, "ccbill_datalink_missing_paid_through", &existing.ID, existing.CustomerID.String(), processorSubID, &record, nil); alertErr != nil {
		...
		if _, err := lifecycleService.ReactivateMembership(ctx, &subscriptions.ReactivateMembershipParams{
			...
			if subscriptions.IsTerminalTransitionBlocked(err) {
				if alertErr := w.recordDataLinkRepairAlert(ctx, "ccbill_datalink_terminal_reactivation_blocked", &existing.ID, existing.CustomerID.String(), processorSubID, &record, err); alertErr != nil {
