...
func NewDBSecretStore(pool *db.Pool) (TenantSecretStore, error) {
	if pool == nil {
...
func (d *dbSecretStore) Put(ctx context.Context, tenantID merchant.ID, name, value string) (Secret, error) {
	...
	var s Secret
	err := d.pool.QueryRow(ctx, `
		INSERT INTO openrails.tenant_secrets (tenant_id, name, value, version)
		VALUES ($1::uuid, $2, $3, 1)
		ON CONFLICT (tenant_id, name) DO UPDATE
		   SET value      = EXCLUDED.value,
		       version    = CASE WHEN openrails.tenant_secrets.value = EXCLUDED.value
		                         THEN openrails.tenant_secrets.version
		                         ELSE openrails.tenant_secrets.version + 1 END,
...
func (d *dbSecretStore) Delete(ctx context.Context, tenantID merchant.ID, name string) error {
	...
	_, err := d.pool.Exec(ctx, `
		DELETE FROM openrails.tenant_secrets WHERE tenant_id = $1::uuid AND name = $2
...
func (d *dbSecretStore) List(ctx context.Context, tenantID merchant.ID) ([]string, error) {
	...
	rows, err := d.pool.Query(ctx, `
		SELECT name FROM openrails.tenant_secrets WHERE tenant_id = $1::uuid ORDER BY name
	`, tenantID.String())
	if err != nil {
