...
//   - a payment-source update naming it in ANY unresolved state, matched on
...
func pinnedBy(ctx context.Context, q *gen.Queries, merchantID, methodID uuid.UUID) (string, error) {
	...
	swaps, err := q.CountUnresolvedPaymentSourceUpdatesForPaymentMethod(ctx, gen.CountUnresolvedPaymentSourceUpdatesForPaymentMethodParams{
		MerchantID: merchantID, PaymentMethodID: methodID,
	})
	if err != nil && !errors.Is(err, pgx.ErrNoRows) {
		return "", fmt.Errorf("count unresolved payment-source updates for %s: %w", methodID, err)
	}
	if swaps > 0 {
