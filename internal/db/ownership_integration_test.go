...
func TestOwnedRowsRejectForeignParentsAndPreserveDeletionScope(t *testing.T) {
	...
	cases := []struct {
		...
		{"subscription_payer", "subscriptions_payment_method_id_fkey", `INSERT INTO openrails.subscriptions(merchant_id,customer_id,product_id,price_id,psp_id,payment_method_id,rail,status,cancelled_at,cancel_type) VALUES($1,$2,$3,$4,$5,$6,'nmi','cancelled',now(),'user')`, []any{a.merchant, payer, a.product, a.price, a.psp, a.method}, 5, a.extraMethod},
		...
		{"usage_unit", "usage_events_ledger_transfer_fk", `INSERT INTO openrails.usage_events(merchant_id,customer_id,currency,ledger_transfer_id,invoker_id,event_type,amount,source,source_id,pricing_authority) VALUES($1,$2,$3,$4,'invoker','usage',1000,'ownership','event','host')`, []any{a.merchant, payer, "USD", deposit.ID}, 2, "EUR"},
