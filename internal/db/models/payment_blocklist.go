...
//
// An entry is either tenant-wide (CustomerID nil/zero) — applying to every payer
// org in the tenant — or scoped to a single tenant subject (CustomerID set, issue #221).
...
type PaymentBlocklistEntry struct {
	...
	// CustomerID, when set, scopes the block to a single tenant subject (issue #221).
