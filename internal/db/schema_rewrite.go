...
// schemaDBTX wraps a gen.DBTX (pool / tenant connection / tx) and rewrites the
...
// ---- pgx.Tx wrapper (covers raw queries inside RunInTx / TenantTx / Pool.Begin) ----
...
func WrapPool(raw *pgxpool.Pool, schema string) *Pool {
	...
	return &Pool{raw: raw, rw: newSchemaRewriter(schema)}
}
