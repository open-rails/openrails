// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.31.1
// source: usdc_funding_sessions.sql

package gen

import (
	"context"
	"time"

	"github.com/google/uuid"
)

const createUSDCFundingSession = `-- name: CreateUSDCFundingSession :exec

INSERT INTO openrails.usdc_funding_sessions (
    id, merchant_id, customer_id, checkout_session_id, provider,
    wallet_address, asset, network, requested_amount, provider_session_id,
    provider_url, status, return_url, idempotency_key, metadata,
    last_checked_at, expires_at, created_at, updated_at
) VALUES (
    $1,
    $10::uuid,
    $2, $11, $3, $4, $5, $6, $7,
    $12, $8, $9, $13,
    $14, $15,
    $16, $17,
    COALESCE(NULLIF($18::timestamptz, '0001-01-01 00:00:00+00'::timestamptz), now()),
    COALESCE(NULLIF($19::timestamptz, '0001-01-01 00:00:00+00'::timestamptz), now())
)
`

type CreateUSDCFundingSessionParams struct {
	ID                uuid.UUID
	CustomerID        uuid.UUID
	Provider          string
	WalletAddress     string
	Asset             string
	Network           string
	RequestedAmount   string
	ProviderUrl       string
	Status            string
	MerchantID        uuid.UUID
	CheckoutSessionID *uuid.UUID
	ProviderSessionID *string
	ReturnUrl         *string
	IdempotencyKey    *string
	Metadata          []byte
	LastCheckedAt     *time.Time
	ExpiresAt         *time.Time
	CreatedAt         time.Time
	UpdatedAt         time.Time
}

// openrails.usdc_funding_sessions.
func (q *Queries) CreateUSDCFundingSession(ctx context.Context, arg CreateUSDCFundingSessionParams) error {
	_, err := q.db.Exec(ctx, createUSDCFundingSession,
		arg.ID,
		arg.CustomerID,
		arg.Provider,
		arg.WalletAddress,
		arg.Asset,
		arg.Network,
		arg.RequestedAmount,
		arg.ProviderUrl,
		arg.Status,
		arg.MerchantID,
		arg.CheckoutSessionID,
		arg.ProviderSessionID,
		arg.ReturnUrl,
		arg.IdempotencyKey,
		arg.Metadata,
		arg.LastCheckedAt,
		arg.ExpiresAt,
		arg.CreatedAt,
		arg.UpdatedAt,
	)
	return err
}

const getUSDCFundingSessionByID = `-- name: GetUSDCFundingSessionByID :one
SELECT id, merchant_id, customer_id, checkout_session_id, provider, wallet_address, asset, network, requested_amount, provider_session_id, provider_url, status, return_url, idempotency_key, metadata, last_checked_at, expires_at, created_at, updated_at FROM openrails.usdc_funding_sessions ufs
WHERE ufs.id = $1
LIMIT 1
`

func (q *Queries) GetUSDCFundingSessionByID(ctx context.Context, id uuid.UUID) (OpenrailsUsdcFundingSession, error) {
	row := q.db.QueryRow(ctx, getUSDCFundingSessionByID, id)
	var i OpenrailsUsdcFundingSession
	err := row.Scan(
		&i.ID,
		&i.MerchantID,
		&i.CustomerID,
		&i.CheckoutSessionID,
		&i.Provider,
		&i.WalletAddress,
		&i.Asset,
		&i.Network,
		&i.RequestedAmount,
		&i.ProviderSessionID,
		&i.ProviderUrl,
		&i.Status,
		&i.ReturnUrl,
		&i.IdempotencyKey,
		&i.Metadata,
		&i.LastCheckedAt,
		&i.ExpiresAt,
		&i.CreatedAt,
		&i.UpdatedAt,
	)
	return i, err
}

const getUSDCFundingSessionByIDForCustomer = `-- name: GetUSDCFundingSessionByIDForCustomer :one
SELECT id, merchant_id, customer_id, checkout_session_id, provider, wallet_address, asset, network, requested_amount, provider_session_id, provider_url, status, return_url, idempotency_key, metadata, last_checked_at, expires_at, created_at, updated_at FROM openrails.usdc_funding_sessions ufs
WHERE ufs.id = $1 AND ufs.customer_id = $2
LIMIT 1
`

type GetUSDCFundingSessionByIDForCustomerParams struct {
	ID         uuid.UUID
	CustomerID uuid.UUID
}

func (q *Queries) GetUSDCFundingSessionByIDForCustomer(ctx context.Context, arg GetUSDCFundingSessionByIDForCustomerParams) (OpenrailsUsdcFundingSession, error) {
	row := q.db.QueryRow(ctx, getUSDCFundingSessionByIDForCustomer, arg.ID, arg.CustomerID)
	var i OpenrailsUsdcFundingSession
	err := row.Scan(
		&i.ID,
		&i.MerchantID,
		&i.CustomerID,
		&i.CheckoutSessionID,
		&i.Provider,
		&i.WalletAddress,
		&i.Asset,
		&i.Network,
		&i.RequestedAmount,
		&i.ProviderSessionID,
		&i.ProviderUrl,
		&i.Status,
		&i.ReturnUrl,
		&i.IdempotencyKey,
		&i.Metadata,
		&i.LastCheckedAt,
		&i.ExpiresAt,
		&i.CreatedAt,
		&i.UpdatedAt,
	)
	return i, err
}

const getUSDCFundingSessionByIdempotencyKey = `-- name: GetUSDCFundingSessionByIdempotencyKey :one
SELECT id, merchant_id, customer_id, checkout_session_id, provider, wallet_address, asset, network, requested_amount, provider_session_id, provider_url, status, return_url, idempotency_key, metadata, last_checked_at, expires_at, created_at, updated_at FROM openrails.usdc_funding_sessions ufs
WHERE ufs.customer_id = $1 AND ufs.idempotency_key = $2
LIMIT 1
`

type GetUSDCFundingSessionByIdempotencyKeyParams struct {
	CustomerID     uuid.UUID
	IdempotencyKey *string
}

func (q *Queries) GetUSDCFundingSessionByIdempotencyKey(ctx context.Context, arg GetUSDCFundingSessionByIdempotencyKeyParams) (OpenrailsUsdcFundingSession, error) {
	row := q.db.QueryRow(ctx, getUSDCFundingSessionByIdempotencyKey, arg.CustomerID, arg.IdempotencyKey)
	var i OpenrailsUsdcFundingSession
	err := row.Scan(
		&i.ID,
		&i.MerchantID,
		&i.CustomerID,
		&i.CheckoutSessionID,
		&i.Provider,
		&i.WalletAddress,
		&i.Asset,
		&i.Network,
		&i.RequestedAmount,
		&i.ProviderSessionID,
		&i.ProviderUrl,
		&i.Status,
		&i.ReturnUrl,
		&i.IdempotencyKey,
		&i.Metadata,
		&i.LastCheckedAt,
		&i.ExpiresAt,
		&i.CreatedAt,
		&i.UpdatedAt,
	)
	return i, err
}

const updateUSDCFundingSessionStatus = `-- name: UpdateUSDCFundingSessionStatus :exec
UPDATE openrails.usdc_funding_sessions SET
    status = $2,
    last_checked_at = $3,
    metadata = $4,
    updated_at = $5
WHERE id = $1
`

type UpdateUSDCFundingSessionStatusParams struct {
	ID            uuid.UUID
	Status        string
	LastCheckedAt *time.Time
	Metadata      []byte
	UpdatedAt     time.Time
}

func (q *Queries) UpdateUSDCFundingSessionStatus(ctx context.Context, arg UpdateUSDCFundingSessionStatusParams) error {
	_, err := q.db.Exec(ctx, updateUSDCFundingSessionStatus,
		arg.ID,
		arg.Status,
		arg.LastCheckedAt,
		arg.Metadata,
		arg.UpdatedAt,
	)
	return err
}
