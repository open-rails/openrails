...
const auditEntitlementGrantMissingEntitlements = `-- name: AuditEntitlementGrantMissingEntitlements :many
...
type AuditEntitlementGrantMissingEntitlementsRow struct {
	GrantID      uuid.UUID
	UserID       string
	Reason       string
	DurationDays *int32
...
func (q *Queries) AuditEntitlementGrantMissingEntitlements(ctx context.Context) ([]AuditEntitlementGrantMissingEntitlementsRow, error) {
	rows, err := q.db.Query(ctx, auditEntitlementGrantMissingEntitlements)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []AuditEntitlementGrantMissingEntitlementsRow
	for rows.Next() {
		var i AuditEntitlementGrantMissingEntitlementsRow
		if err := rows.Scan(
			&i.GrantID,
			&i.UserID,
			&i.Reason,
...
type AuditExpiredEntitlementGrantActiveEntitlementRow struct {
	EntID       uuid.UUID
	UserID      string
	Entitlement string
	GrantID     uuid.UUID
...
func (q *Queries) AuditExpiredEntitlementGrantActiveEntitlement(ctx context.Context) ([]AuditExpiredEntitlementGrantActiveEntitlementRow, error) {
	rows, err := q.db.Query(ctx, auditExpiredEntitlementGrantActiveEntitlement)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []AuditExpiredEntitlementGrantActiveEntitlementRow
	for rows.Next() {
		var i AuditExpiredEntitlementGrantActiveEntitlementRow
		if err := rows.Scan(
			&i.EntID,
			&i.UserID,
			&i.Entitlement,
