// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.31.1
// source: payment_methods.sql

package gen

import (
	"context"
	"time"

	"github.com/google/uuid"
)

const countPaymentMethodForUser = `-- name: CountPaymentMethodForUser :one
SELECT count(*) FROM billing.payment_methods pm
WHERE pm.id = $1 AND pm.tenant_subject_id = $2
`

type CountPaymentMethodForUserParams struct {
	ID              uuid.UUID
	TenantSubjectID uuid.UUID
}

func (q *Queries) CountPaymentMethodForUser(ctx context.Context, arg CountPaymentMethodForUserParams) (int64, error) {
	row := q.db.QueryRow(ctx, countPaymentMethodForUser, arg.ID, arg.TenantSubjectID)
	var count int64
	err := row.Scan(&count)
	return count, err
}

const countPaymentMethodsByTenantSubject = `-- name: CountPaymentMethodsByTenantSubject :one
SELECT count(*) FROM billing.payment_methods pm
WHERE pm.tenant_subject_id = $1
`

func (q *Queries) CountPaymentMethodsByTenantSubject(ctx context.Context, tenantSubjectID uuid.UUID) (int64, error) {
	row := q.db.QueryRow(ctx, countPaymentMethodsByTenantSubject, tenantSubjectID)
	var count int64
	err := row.Scan(&count)
	return count, err
}

const createPaymentMethod = `-- name: CreatePaymentMethod :execrows

INSERT INTO billing.payment_methods (
    id, tenant_subject_id, processor, vault_id, billing_id,
    initial_transaction_id, last_four, card_type, expiry_date,
    failure_reason, metadata, created_at, updated_at
) VALUES (
    $1, $2, $3, $4, $6,
    $5, $7, $8, $9,
    $10, $11,
    COALESCE(NULLIF($12::timestamptz, '0001-01-01 00:00:00+00'::timestamptz), now()),
    COALESCE(NULLIF($13::timestamptz, '0001-01-01 00:00:00+00'::timestamptz), now())
)
`

type CreatePaymentMethodParams struct {
	ID                   uuid.UUID
	TenantSubjectID      uuid.UUID
	Processor            string
	VaultID              string
	InitialTransactionID string
	BillingID            *string
	LastFour             *string
	CardType             *string
	ExpiryDate           *string
	FailureReason        *string
	Metadata             []byte
	CreatedAt            time.Time
	UpdatedAt            time.Time
}

// billing.payment_methods.
func (q *Queries) CreatePaymentMethod(ctx context.Context, arg CreatePaymentMethodParams) (int64, error) {
	result, err := q.db.Exec(ctx, createPaymentMethod,
		arg.ID,
		arg.TenantSubjectID,
		arg.Processor,
		arg.VaultID,
		arg.InitialTransactionID,
		arg.BillingID,
		arg.LastFour,
		arg.CardType,
		arg.ExpiryDate,
		arg.FailureReason,
		arg.Metadata,
		arg.CreatedAt,
		arg.UpdatedAt,
	)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected(), nil
}

const deletePaymentMethod = `-- name: DeletePaymentMethod :execrows
DELETE FROM billing.payment_methods WHERE id = $1
`

func (q *Queries) DeletePaymentMethod(ctx context.Context, id uuid.UUID) (int64, error) {
	result, err := q.db.Exec(ctx, deletePaymentMethod, id)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected(), nil
}

const getPaymentMethodByID = `-- name: GetPaymentMethodByID :one
SELECT id, processor, vault_id, billing_id, initial_transaction_id, last_four, card_type, expiry_date, failure_reason, metadata, created_at, updated_at, tenant_id, tenant_subject_id FROM billing.payment_methods WHERE id = $1
`

func (q *Queries) GetPaymentMethodByID(ctx context.Context, id uuid.UUID) (BillingPaymentMethod, error) {
	row := q.db.QueryRow(ctx, getPaymentMethodByID, id)
	var i BillingPaymentMethod
	err := row.Scan(
		&i.ID,
		&i.Processor,
		&i.VaultID,
		&i.BillingID,
		&i.InitialTransactionID,
		&i.LastFour,
		&i.CardType,
		&i.ExpiryDate,
		&i.FailureReason,
		&i.Metadata,
		&i.CreatedAt,
		&i.UpdatedAt,
		&i.TenantID,
		&i.TenantSubjectID,
	)
	return i, err
}

const getPaymentMethodByInitialTransactionID = `-- name: GetPaymentMethodByInitialTransactionID :one
SELECT id, processor, vault_id, billing_id, initial_transaction_id, last_four, card_type, expiry_date, failure_reason, metadata, created_at, updated_at, tenant_id, tenant_subject_id FROM billing.payment_methods pm
WHERE pm.processor = $1 AND pm.initial_transaction_id = $2
LIMIT 1
`

type GetPaymentMethodByInitialTransactionIDParams struct {
	Processor            string
	InitialTransactionID string
}

func (q *Queries) GetPaymentMethodByInitialTransactionID(ctx context.Context, arg GetPaymentMethodByInitialTransactionIDParams) (BillingPaymentMethod, error) {
	row := q.db.QueryRow(ctx, getPaymentMethodByInitialTransactionID, arg.Processor, arg.InitialTransactionID)
	var i BillingPaymentMethod
	err := row.Scan(
		&i.ID,
		&i.Processor,
		&i.VaultID,
		&i.BillingID,
		&i.InitialTransactionID,
		&i.LastFour,
		&i.CardType,
		&i.ExpiryDate,
		&i.FailureReason,
		&i.Metadata,
		&i.CreatedAt,
		&i.UpdatedAt,
		&i.TenantID,
		&i.TenantSubjectID,
	)
	return i, err
}

const getPaymentMethodByVaultID = `-- name: GetPaymentMethodByVaultID :one
SELECT id, processor, vault_id, billing_id, initial_transaction_id, last_four, card_type, expiry_date, failure_reason, metadata, created_at, updated_at, tenant_id, tenant_subject_id FROM billing.payment_methods pm
WHERE pm.processor = $1 AND pm.vault_id = $2
LIMIT 1
`

type GetPaymentMethodByVaultIDParams struct {
	Processor string
	VaultID   string
}

func (q *Queries) GetPaymentMethodByVaultID(ctx context.Context, arg GetPaymentMethodByVaultIDParams) (BillingPaymentMethod, error) {
	row := q.db.QueryRow(ctx, getPaymentMethodByVaultID, arg.Processor, arg.VaultID)
	var i BillingPaymentMethod
	err := row.Scan(
		&i.ID,
		&i.Processor,
		&i.VaultID,
		&i.BillingID,
		&i.InitialTransactionID,
		&i.LastFour,
		&i.CardType,
		&i.ExpiryDate,
		&i.FailureReason,
		&i.Metadata,
		&i.CreatedAt,
		&i.UpdatedAt,
		&i.TenantID,
		&i.TenantSubjectID,
	)
	return i, err
}

const listPaymentMethodsByIDs = `-- name: ListPaymentMethodsByIDs :many
SELECT id, processor, vault_id, billing_id, initial_transaction_id, last_four, card_type, expiry_date, failure_reason, metadata, created_at, updated_at, tenant_id, tenant_subject_id FROM billing.payment_methods WHERE id = ANY($1::uuid[])
`

func (q *Queries) ListPaymentMethodsByIDs(ctx context.Context, ids []uuid.UUID) ([]BillingPaymentMethod, error) {
	rows, err := q.db.Query(ctx, listPaymentMethodsByIDs, ids)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []BillingPaymentMethod
	for rows.Next() {
		var i BillingPaymentMethod
		if err := rows.Scan(
			&i.ID,
			&i.Processor,
			&i.VaultID,
			&i.BillingID,
			&i.InitialTransactionID,
			&i.LastFour,
			&i.CardType,
			&i.ExpiryDate,
			&i.FailureReason,
			&i.Metadata,
			&i.CreatedAt,
			&i.UpdatedAt,
			&i.TenantID,
			&i.TenantSubjectID,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const listPaymentMethodsByProcessor = `-- name: ListPaymentMethodsByProcessor :many
SELECT id, processor, vault_id, billing_id, initial_transaction_id, last_four, card_type, expiry_date, failure_reason, metadata, created_at, updated_at, tenant_id, tenant_subject_id FROM billing.payment_methods pm
WHERE pm.processor = $1
ORDER BY pm.created_at DESC
`

func (q *Queries) ListPaymentMethodsByProcessor(ctx context.Context, processor string) ([]BillingPaymentMethod, error) {
	rows, err := q.db.Query(ctx, listPaymentMethodsByProcessor, processor)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []BillingPaymentMethod
	for rows.Next() {
		var i BillingPaymentMethod
		if err := rows.Scan(
			&i.ID,
			&i.Processor,
			&i.VaultID,
			&i.BillingID,
			&i.InitialTransactionID,
			&i.LastFour,
			&i.CardType,
			&i.ExpiryDate,
			&i.FailureReason,
			&i.Metadata,
			&i.CreatedAt,
			&i.UpdatedAt,
			&i.TenantID,
			&i.TenantSubjectID,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const listPaymentMethodsByProcessors = `-- name: ListPaymentMethodsByProcessors :many
SELECT id, processor, vault_id, billing_id, initial_transaction_id, last_four, card_type, expiry_date, failure_reason, metadata, created_at, updated_at, tenant_id, tenant_subject_id FROM billing.payment_methods pm
WHERE pm.processor = ANY($1::text[])
ORDER BY pm.created_at DESC
`

func (q *Queries) ListPaymentMethodsByProcessors(ctx context.Context, processors []string) ([]BillingPaymentMethod, error) {
	rows, err := q.db.Query(ctx, listPaymentMethodsByProcessors, processors)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []BillingPaymentMethod
	for rows.Next() {
		var i BillingPaymentMethod
		if err := rows.Scan(
			&i.ID,
			&i.Processor,
			&i.VaultID,
			&i.BillingID,
			&i.InitialTransactionID,
			&i.LastFour,
			&i.CardType,
			&i.ExpiryDate,
			&i.FailureReason,
			&i.Metadata,
			&i.CreatedAt,
			&i.UpdatedAt,
			&i.TenantID,
			&i.TenantSubjectID,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const listPaymentMethodsByTenantSubject = `-- name: ListPaymentMethodsByTenantSubject :many
SELECT id, processor, vault_id, billing_id, initial_transaction_id, last_four, card_type, expiry_date, failure_reason, metadata, created_at, updated_at, tenant_id, tenant_subject_id FROM billing.payment_methods pm
WHERE pm.tenant_subject_id = $1
ORDER BY pm.created_at DESC
`

func (q *Queries) ListPaymentMethodsByTenantSubject(ctx context.Context, tenantSubjectID uuid.UUID) ([]BillingPaymentMethod, error) {
	rows, err := q.db.Query(ctx, listPaymentMethodsByTenantSubject, tenantSubjectID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []BillingPaymentMethod
	for rows.Next() {
		var i BillingPaymentMethod
		if err := rows.Scan(
			&i.ID,
			&i.Processor,
			&i.VaultID,
			&i.BillingID,
			&i.InitialTransactionID,
			&i.LastFour,
			&i.CardType,
			&i.ExpiryDate,
			&i.FailureReason,
			&i.Metadata,
			&i.CreatedAt,
			&i.UpdatedAt,
			&i.TenantID,
			&i.TenantSubjectID,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const listPaymentMethodsByTenantSubjectPaged = `-- name: ListPaymentMethodsByTenantSubjectPaged :many
SELECT id, processor, vault_id, billing_id, initial_transaction_id, last_four, card_type, expiry_date, failure_reason, metadata, created_at, updated_at, tenant_id, tenant_subject_id FROM billing.payment_methods pm
WHERE pm.tenant_subject_id = $1
ORDER BY pm.created_at DESC
LIMIT NULLIF($3::int, 0) OFFSET $2::int
`

type ListPaymentMethodsByTenantSubjectPagedParams struct {
	TenantSubjectID uuid.UUID
	PageOffset      int32
	PageLimit       int32
}

func (q *Queries) ListPaymentMethodsByTenantSubjectPaged(ctx context.Context, arg ListPaymentMethodsByTenantSubjectPagedParams) ([]BillingPaymentMethod, error) {
	rows, err := q.db.Query(ctx, listPaymentMethodsByTenantSubjectPaged, arg.TenantSubjectID, arg.PageOffset, arg.PageLimit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []BillingPaymentMethod
	for rows.Next() {
		var i BillingPaymentMethod
		if err := rows.Scan(
			&i.ID,
			&i.Processor,
			&i.VaultID,
			&i.BillingID,
			&i.InitialTransactionID,
			&i.LastFour,
			&i.CardType,
			&i.ExpiryDate,
			&i.FailureReason,
			&i.Metadata,
			&i.CreatedAt,
			&i.UpdatedAt,
			&i.TenantID,
			&i.TenantSubjectID,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const listPaymentMethodsByTenantSubjectProcessors = `-- name: ListPaymentMethodsByTenantSubjectProcessors :many
SELECT id, processor, vault_id, billing_id, initial_transaction_id, last_four, card_type, expiry_date, failure_reason, metadata, created_at, updated_at, tenant_id, tenant_subject_id FROM billing.payment_methods pm
WHERE pm.tenant_subject_id = $1
  AND pm.processor = ANY($2::text[])
ORDER BY pm.created_at DESC
`

type ListPaymentMethodsByTenantSubjectProcessorsParams struct {
	TenantSubjectID uuid.UUID
	Processors      []string
}

func (q *Queries) ListPaymentMethodsByTenantSubjectProcessors(ctx context.Context, arg ListPaymentMethodsByTenantSubjectProcessorsParams) ([]BillingPaymentMethod, error) {
	rows, err := q.db.Query(ctx, listPaymentMethodsByTenantSubjectProcessors, arg.TenantSubjectID, arg.Processors)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []BillingPaymentMethod
	for rows.Next() {
		var i BillingPaymentMethod
		if err := rows.Scan(
			&i.ID,
			&i.Processor,
			&i.VaultID,
			&i.BillingID,
			&i.InitialTransactionID,
			&i.LastFour,
			&i.CardType,
			&i.ExpiryDate,
			&i.FailureReason,
			&i.Metadata,
			&i.CreatedAt,
			&i.UpdatedAt,
			&i.TenantID,
			&i.TenantSubjectID,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const updatePaymentMethod = `-- name: UpdatePaymentMethod :execrows
UPDATE billing.payment_methods SET
    tenant_subject_id = $2,
    processor = $3,
    vault_id = $4,
    billing_id = $6,
    initial_transaction_id = $5,
    last_four = $7,
    card_type = $8,
    expiry_date = $9,
    failure_reason = $10,
    metadata = $11,
    updated_at = $12
WHERE id = $1
`

type UpdatePaymentMethodParams struct {
	ID                   uuid.UUID
	TenantSubjectID      uuid.UUID
	Processor            string
	VaultID              string
	InitialTransactionID string
	BillingID            *string
	LastFour             *string
	CardType             *string
	ExpiryDate           *string
	FailureReason        *string
	Metadata             []byte
	UpdatedAt            time.Time
}

func (q *Queries) UpdatePaymentMethod(ctx context.Context, arg UpdatePaymentMethodParams) (int64, error) {
	result, err := q.db.Exec(ctx, updatePaymentMethod,
		arg.ID,
		arg.TenantSubjectID,
		arg.Processor,
		arg.VaultID,
		arg.InitialTransactionID,
		arg.BillingID,
		arg.LastFour,
		arg.CardType,
		arg.ExpiryDate,
		arg.FailureReason,
		arg.Metadata,
		arg.UpdatedAt,
	)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected(), nil
}
