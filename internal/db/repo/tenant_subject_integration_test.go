...
func TestEnsureTenantSubjectID_UUIDReusesExistingPayableID(t *testing.T) {
	...
	t.Cleanup(func() {
		_, _ = pool.Exec(ctx, `DELETE FROM openrails.tenant_subjects WHERE id = $1`, userID)
	})
	...
	require.NoError(t, pool.QueryRow(ctx,
		`SELECT issuer, last_seen_at FROM openrails.tenant_subjects WHERE id = $1`, userID,
...
func TestEnsureTenantSubjectID_RejectsNonUUIDSubject(t *testing.T) {
	...
	_, err := EnsureTenantSubjectID(ctx, pool, dbtest.TestTenantID.UUID(), "legacy-user-123")
	require.ErrorContains(t, err, "UUID-only")

	id, err := EnsureTenantSubjectID(ctx, pool, dbtest.TestTenantID.UUID(), "")
	require.NoError(t, err)
