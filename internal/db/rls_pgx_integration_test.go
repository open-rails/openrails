...
func pgxProbeVals(ctx context.Context, t *testing.T, d *DB) []string {
	t.Helper()
	rows, err := d.Qx(ctx).Query(ctx, `SELECT val FROM billing.rls_probe ORDER BY val`)
...
func TestRLSEnforcement_PgxSide(t *testing.T) {
	...
	_, err = super.Qx(ctx).Exec(ctx, `
		INSERT INTO billing.rls_probe (id, tenant_id, val) VALUES
	...
	require.NoError(t, app.RunInTx(pinnedCtx, func(ctx context.Context, tx pgx.Tx) error {
		var n int
		if err := tx.QueryRow(ctx, `SELECT count(*) FROM billing.rls_probe`).Scan(&n); err != nil {
			return err
		}
	...
	require.NoError(t, app.TenantTx(ctxA, func(ctx context.Context, tx pgx.Tx) error {
		...
		require.Equal(t, 1, n)
		tag, derr := tx.Exec(ctx, `DELETE FROM billing.rls_probe WHERE val = 'b-row'`)
	...
	err = app.TenantTx(ctxA, func(ctx context.Context, tx pgx.Tx) error {
		_, ierr := tx.Exec(ctx, `
			INSERT INTO billing.rls_probe (id, tenant_id, val)
