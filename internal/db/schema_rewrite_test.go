...
func TestSchemaRewriter(t *testing.T) {
	...
	t.Run("does not touch unrelated identifiers", func(t *testing.T) {
		...
		const sql = "SELECT public.gen_random_uuid(), p.id FROM profiles.users p JOIN openrails.customers c ON c.id = p.id"
		want := "SELECT public.gen_random_uuid(), p.id FROM profiles.users p JOIN shop.customers c ON c.id = p.id"
