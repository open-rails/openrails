...
func ValidateValues(p Profile, values []*string) error {
	...
	if p.Name == "rail_intents" {
		...
		if payload != nil && *payload != "null" && *payload != "{}" && (typ == nil || (*typ != intents.TypeNMIPaymentMethodDelete && *typ != intents.TypeHyperSwitchMethodDelete && *typ != "nmi_refund" && *typ != "stripe_refund" && *typ != "ccbill_refund" && *typ != "invoice_collection" && *typ != "nmi_sale" && *typ != "nmi_subscription_create" && *typ != "manual_rebill" && *typ != "nmi_provider_cutover")) {
...
func validateRetainedPayment(p Profile, values []*string) (bool, error) {
	...
	if typ != intents.TypeNMIPaymentMethodDelete && typ != intents.TypeHyperSwitchMethodDelete && typ != "invoice_collection" && typ != subscriptions.TypeManualRebill && typ != payments.TypeNMISale && typ != subscriptions.TypeNMIInitialEnrollment {
		return false, nil
	}
