...
func testPurchaseWorkflowArchive(t *testing.T, recurring bool) {
	...
	client, err := nmi.NewClient("writer-account", &config.NMIProviderSettings{SecurityKey: "archive-fixture-key", WebhookSecret: "archive-fixture-webhook"}, true)
	require.NoError(t, err)
	...
	if recurring {
		...
		subscriptionID = uuid.UUID(*first.SubscriptionID)
	}
	...
	require.Contains(t, []string{"", "null", "{}"}, string(operation.Payload), "ordinary success must prune the submission payload")
	...
	for _, tc := range []struct{ name, table, column, value string }{
		...
		{"unpruned NMI payload", "rail_intents", "payload", `{"customer_vault_id":"archive-vault"}`},
