...
// server-to-server surface is API key-authenticated and lives on the
// PUBLIC engine under /v1/service/*; the always-present control plane (#469)
// resolves API keys. This test asserts the API-key gate without
// requiring a live control plane.

func TestRegisterServiceRoutes_APIKeyAuthGatesMountedSurface(t *testing.T) {
	gin.SetMode(gin.TestMode)

	// Mount the service surface with an API key middleware that always rejects
	// (modelling "no valid API key presented"). This proves the public service surface
	// is API key-gated rather than an open or certificate-trusted listener. We mount
	...
	// Use the same registration path as the server, with a rejecting API key gate.
