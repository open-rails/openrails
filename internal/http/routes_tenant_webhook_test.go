...
// verifyStripeTenant is the trust boundary for the tenant webhook surface: it
...
func TestVerifyStripeTenant_TrustBoundary(t *testing.T) {
	...
	tenantSecret := "whsec_tenant_acme"
	otherSecret := "whsec_other_tenant"
	header := stripeSig(tenantSecret, ts, body)

	// The resolved tenant's secret verifies the signature.
	prepared, err := verifyStripeTenant(body, []string{tenantSecret}, header)
	if err != nil {
	...
	if _, err := verifyStripeTenant(body, []string{otherSecret}, header); err == nil {
		t.Fatal("foreign tenant secret must not verify the signature")
	}

	// Multi-secret (snapshot + thin): first non-matching, second matching.
	if _, err := verifyStripeTenant(body, []string{otherSecret, tenantSecret}, header); err != nil {
	...
	if _, err := verifyStripeTenant(body, []string{tenantSecret}, ""); err == nil {
