...
func TestMerchantWebhookRouteHTTPResolvesMerchantBeforeVerifyingStripe(t *testing.T) {
	...
	rt.SetConfiguredMerchant(acme)
	httproutes.RegisterWebhookRoutes(router.NewMux(mux, "/v1/webhooks", rt), rt)
	server := httptest.NewServer(mux)
	t.Cleanup(server.Close)

	body := []byte(`{"id":"evt_1","type":"checkout.session.completed"}`)
	ts := strconv.FormatInt(time.Now().Unix(), 10)
	for _, path := range []string{"/global/stripe", "/global/nmi", "/global/ccbill", "/global/basistheory", "/v1/webhooks/stripe"} {
	...
	require.Equal(t, http.StatusUnauthorized, postMerchantWebhook(t, server.URL+"/v1/webhooks/stripe/"+acctAcmeTest, body, stripeSig("whsec_evil", ts, body)))
	// test_mode posture resolves the environment=test account's secret (#681);
	// the live account's secret no longer verifies.
	require.Equal(t, http.StatusUnauthorized, postMerchantWebhook(t, server.URL+"/v1/webhooks/stripe/"+acctAcmeTest, body, stripeSig("whsec_acme", ts, body)))
	require.Equal(t, http.StatusInternalServerError, postMerchantWebhook(t, server.URL+"/v1/webhooks/stripe/"+acctAcmeTest, body, stripeSig("whsec_acme_test", ts, body)))
	foreignStripe := []byte(`{"id":"evt_wrong_account","type":"checkout.session.completed","account":"` + acctEvil + `"}`)
	require.Equal(t, http.StatusBadRequest, postMerchantWebhook(t, server.URL+"/v1/webhooks/stripe/"+acctAcmeTest, foreignStripe, stripeSig("whsec_acme_test", ts, foreignStripe)))
	require.Equal(t, http.StatusNotFound, postMerchantWebhook(t, server.URL+"/v1/webhooks/stripe/"+acctEvil, body, stripeSig("whsec_evil", ts, body)))
	...
	require.Equal(t, http.StatusBadRequest, postMerchantWebhookWithHeader(t, server.URL+"/v1/webhooks/nmi/"+nmiAcmeTest, wrongNMI, "Webhook-Signature", nmiSig("nmi_acme_test", ts, wrongNMI)))
	...
	require.Equal(t, http.StatusUnauthorized, postMerchantWebhookWithHeader(t, server.URL+"/v1/webhooks/nmi/"+nmiAcmeTest, nmiBody, "Webhook-Signature", nmiSig("nmi_evil", ts, nmiBody)))
	require.Equal(t, http.StatusUnauthorized, postMerchantWebhookWithHeader(t, server.URL+"/v1/webhooks/nmi/"+nmiAcmeTest, nmiBody, "Webhook-Signature", nmiSig("nmi_acme", ts, nmiBody)))
	require.Equal(t, http.StatusInternalServerError, postMerchantWebhookWithHeader(t, server.URL+"/v1/webhooks/nmi/"+nmiAcmeTest, nmiBody, "Webhook-Signature", nmiSig("nmi_acme_test", ts, nmiBody)))
	...
	// secret is loaded.
	status, refusal := postMerchantWebhookBody(t, server.URL+"/v1/webhooks/mobius/"+nmiAcmeTest, nmiBody, "Webhook-Signature", nmiSig("nmi_acme_test", ts, nmiBody))
	...
	for _, retired := range []string{"X-Signature", "X-NMI-Signature", "X-Mobius-Signature"} {
		require.Equal(t, http.StatusUnauthorized,
			postMerchantWebhookWithHeader(t, server.URL+"/v1/webhooks/nmi/"+nmiAcmeTest, nmiBody, retired, nmiSig("nmi_acme_test", ts, nmiBody)),
			retired)
	}

	ccbillBody := []byte(`{"eventType":"RenewalSuccess","clientAccnum":"945282","clientSubacc":"0000","subscriptionId":"ccs_1","transactionId":"cct_1"}`)
	require.Equal(t, http.StatusBadRequest, postMerchantWebhookWithHeader(t, server.URL+"/v1/webhooks/ccbill/945282-0001?eventType=RenewalSuccess", ccbillBody, "X-Unused", "unused"))
	...
	require.Equal(t, http.StatusInternalServerError, postMerchantWebhookWithHeader(t, server.URL+"/v1/webhooks/ccbill/945282-0000?eventType=RenewalSuccess", ccbillBody, "X-Unused", "unused"))
