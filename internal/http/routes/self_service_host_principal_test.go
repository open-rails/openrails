...
func newHostPrincipalSelfRouter(t *testing.T, authn billingauth.DelegatedAuthenticator) http.Handler {
	t.Helper()
	...
	RegisterSelfServiceRoutes(router.NewMux(mux, "/v1/me", nil), nil,
		middleware.DelegatedPrincipalRequired(authn), routesurface.AllProviderRoutes())
	return mux
}
...
func doHostSelf(e http.Handler, method, path string) *httptest.ResponseRecorder {
