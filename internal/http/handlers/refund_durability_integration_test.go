...
func TestAdminCCBillRefundRejectedBeforeReservationAndAccessChanges(t *testing.T) {
	for _, amount := range []int64{5_000_000, 10_000_000} {
		t.Run(fmt.Sprint(amount), func(t *testing.T) {
			...
			fx.exec(`UPDATE billing.payments SET rail='ccbill',psp_id=$2 WHERE id=$1`, payID, fx.pspFor("ccbill"))
			...
			req.SetPathValue("id", openrails.PaymentID(payID).String())
			...
			var count int
			require.NoError(t, fx.dbi.Pool().QueryRow(fx.ctx, `SELECT count(*) FROM billing.rail_intents WHERE merchant_id=$1`, fx.merchant).Scan(&count))
			require.Zero(t, count)
			require.NoError(t, fx.dbi.Pool().QueryRow(fx.ctx, `SELECT count(*) FROM billing.payments WHERE refunded_payment_id=$1`, payID).Scan(&count))
			require.Zero(t, count)
...
func TestRefundInterruptedBeforeEnqueueMustRemainRetryable(t *testing.T) {
	...
	require.Error(t, err)
	var n int
	require.NoError(t, fx.dbi.Pool().QueryRow(fx.ctx, "SELECT count(*) FROM billing.rail_intents WHERE payment_id=$1", payID).Scan(&n))
...
func TestRefundEnqueueFailureRollsBackReservation(t *testing.T) {
	...
	_, err := super.Exec(fx.ctx, fmt.Sprintf(`ALTER TABLE billing.rail_intents ADD CONSTRAINT %s CHECK (merchant_id <> '%s')`, constraint, fx.merchant))
	require.NoError(t, err)
	t.Cleanup(func() {
		_, _ = super.Exec(context.Background(), "ALTER TABLE billing.rail_intents DROP CONSTRAINT IF EXISTS "+constraint)
	})
	hr := httprequest.NewHTTP(httptest.NewRecorder(), httptest.NewRequest(http.MethodPost, "/refund", nil).WithContext(fx.ctx), fx.rt)
	...
	_, err = super.Exec(fx.ctx, "ALTER TABLE billing.rail_intents DROP CONSTRAINT "+constraint)
	require.NoError(t, err)
...
func TestRefundFinalizationRecoversReceiptAndRevokesAtomically(t *testing.T) {
	...
	_, err = super.Exec(fx.ctx, fmt.Sprintf(`ALTER TABLE billing.grants ADD CONSTRAINT %s CHECK (merchant_id <> '%s' OR event <> 'revoke')`, constraint, fx.merchant))
	require.NoError(t, err)
	t.Cleanup(func() {
		_, _ = super.Exec(context.Background(), "ALTER TABLE billing.grants DROP CONSTRAINT IF EXISTS "+constraint)
	})
	hr := httprequest.NewHTTP(httptest.NewRecorder(), httptest.NewRequest(http.MethodPost, "/refund", nil).WithContext(fx.ctx), fx.rt)
	...
	require.NoError(t, fx.dbi.Pool().QueryRow(fx.ctx, `SELECT revoked_at FROM billing.entitlements WHERE id=$1`, entitlement.ID).Scan(&revokedAt))
	...
	_, err = super.Exec(fx.ctx, "ALTER TABLE billing.grants DROP CONSTRAINT "+constraint)
	require.NoError(t, err)
	fx.exec(`UPDATE billing.rail_intents SET next_attempt_at=now() WHERE payment_id=$1`, payID)
