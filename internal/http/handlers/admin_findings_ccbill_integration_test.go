...
func TestFindingsQueueApproveCCBillCancelOnly(t *testing.T) {
	...
	require.NoError(t, fx.dbi.Pool().QueryRow(fx.ctx,
		`SELECT status FROM billing.rail_intents WHERE id = $1`, intentID).Scan(&intentStatus))
...
func TestFindingsCCBillRefundRefusesBeforeCancellation(t *testing.T) {
	...
	var status string
	require.NoError(t, fx.dbi.Pool().QueryRow(fx.ctx, `SELECT status FROM billing.subscriptions WHERE merchant_id=$1 AND id=$2`, fx.merchant, subID).Scan(&status))
	...
	require.NoError(t, fx.dbi.Pool().QueryRow(fx.ctx, `SELECT count(*) FROM billing.payments WHERE merchant_id=$1 AND refunded_payment_id=$2`, fx.merchant, paymentID).Scan(&count))
