...
func TestSavedMethodCollectionDefaultsAreCurrencyAndCustomerScoped(t *testing.T) {
	...
	fx.exec(`UPDATE billing.subscriptions SET payment_method_id=$1 WHERE id=$2`, subscriptionMethod.ID, subID)
	fx.exec(`UPDATE billing.payment_methods SET stored_credential_unscheduled_ref='' WHERE id=$1`, usd.ID)
	...
	fx.exec(`UPDATE billing.payment_methods SET stored_credential_unscheduled_ref='approved-unscheduled' WHERE id=$1`, usd.ID)
	...
	fx.exec(`INSERT INTO billing.customers(id,merchant_id) VALUES($1,$2)`, otherCustomer, fx.merchant)
	...
	require.NoError(t, fx.dbi.Pool().QueryRow(fx.ctx, `SELECT payment_method_id FROM billing.subscriptions WHERE id=$1`, subID).Scan(&stillDefault))
