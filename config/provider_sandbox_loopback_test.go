...
func TestProviderSandboxDestinationRefusals(t *testing.T) {
	for name, destination := range map[string]string{
		...
		for key, sandbox := range map[string]ProviderSandboxConfig{"stripe_api_url": {StripeAPIURL: destination}, "nmi_gateway_url": {NMIGatewayURL: destination}} {
			if err := Validate(providerSandboxConfig(CredentialPostureSandbox, sandbox)); !errors.Is(err, errProviderSandboxDestination) {
				t.Errorf("%s %s %q: want errProviderSandboxDestination, got %v", key, name, destination, err)
			}
		}
	}
	for key, sandbox := range map[string]ProviderSandboxConfig{"stripe_api_url": {StripeAPIURL: "http://127.0.0.1:9092"}, "nmi_gateway_url": {NMIGatewayURL: "http://127.0.0.1:9091"}} {
		if err := Validate(providerSandboxConfig(CredentialPostureLive, sandbox)); !errors.Is(err, errProviderSandboxDestination) {
			t.Errorf("%s under live: want errProviderSandboxDestination, got %v", key, err)
		}
	}
}
