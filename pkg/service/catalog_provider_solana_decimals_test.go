...
// UNITS, and the MINT's on-chain decimals is the only thing that converts
// between them. Shipping micros verbatim (the original bug) was a 1000x
// undercharge on a 9-decimal mint.
...
// TestSolanaAdapter_AutoCreateRejectsUnreadableMint: an unreadable mint must
// fail the publish loudly. There is no default decimals to fall back to — a
// guessed 6 would write a permanently wrong on-chain plan amount (#817).
func TestSolanaAdapter_AutoCreateRejectsUnreadableMint(t *testing.T) {
	...
	if _, err := a.AutoCreate(ctx, autoCreateContext{
		...
		t.Fatal("expected AutoCreate to reject a token whose mint cannot be read on-chain")
	}
}
