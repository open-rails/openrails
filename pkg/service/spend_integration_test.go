...
func authzEnv(t *testing.T) (*billingservice.Service, *money.MoneyService, identity.MerchantSubjectID, context.Context) {
	t.Helper()
...
func TestAuthorizeSpend_PrepaidBalanceGate(t *testing.T) {
	...
	_, err := ms.Deposit(ctx, money.DepositParams{MerchantSubjectID: &payer, Actor: payer.UUID().String(), Currency: money.DefaultCurrency, Amount: 1000, Source: "seed"})
	require.NoError(t, err)

	ok, err := svc.AuthorizeSpend(ctx, billingservice.AuthorizeSpendRequest{MerchantSubjectID: payer, Actor: "user:a", EstimateMicros: 500})
	require.NoError(t, err)
	require.True(t, ok.Allowed)
	require.Equal(t, int64(1000), ok.AvailableMicros)

	deny, err := svc.AuthorizeSpend(ctx, billingservice.AuthorizeSpendRequest{MerchantSubjectID: payer, Actor: "user:a", EstimateMicros: 1500})
	require.NoError(t, err)
...
func TestAuthorizeSpend_DailyCapDeny(t *testing.T) {
	svc, ms, payer, ctx := authzEnv(t)
	_, err := ms.Deposit(ctx, money.DepositParams{MerchantSubjectID: &payer, Actor: payer.UUID().String(), Currency: money.DefaultCurrency, Amount: 100000, Source: "seed"})
	require.NoError(t, err)
	cap := int64(1000)
	...
	_, err = ms.Withdraw(ctx, money.WithdrawParams{MerchantSubjectID: &payer, Actor: "user:a", Currency: money.DefaultCurrency, Amount: 800, Source: "usage"})
	require.NoError(t, err)

	deny, err := svc.AuthorizeSpend(ctx, billingservice.AuthorizeSpendRequest{MerchantSubjectID: payer, Actor: "user:a", EstimateMicros: 300})
	require.NoError(t, err)
	require.False(t, deny.Allowed)
	require.Equal(t, money.DenyDailyCap, deny.DenyCode)
	require.Greater(t, deny.RetryAfterSeconds, int64(0))

	ok, err := svc.AuthorizeSpend(ctx, billingservice.AuthorizeSpendRequest{MerchantSubjectID: payer, Actor: "user:a", EstimateMicros: 100})
	require.NoError(t, err)
	require.True(t, ok.Allowed)
}
...
func TestGetCreditAccount_Snapshot(t *testing.T) {
	svc, ms, payer, ctx := authzEnv(t)
	_, err := ms.Deposit(ctx, money.DepositParams{MerchantSubjectID: &payer, Actor: payer.UUID().String(), Currency: money.DefaultCurrency, Amount: 5000, Source: "seed"})
	require.NoError(t, err)
