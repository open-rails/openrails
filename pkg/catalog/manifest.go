...
type Price struct {
	...
	// provider-generated (Stripe price_id, Solana plan_pda). On an EXISTING
	// (financially matched) price, a declared entry the stored link does not
	// already satisfy is ROTATED on apply: merged through UpdatePrice so the
	// adapter validates or publishes it — e.g. moving a Solana plan to another
	// settlement token. Provider-generated stored ids never count as drift.
	// Canonical keys:
	//   psp_links:
	...
	//     solana: {token: DUSD}                                # publish/attach the plan for this settlement token
