...
func TestCleanupExpiredDataWorker(t *testing.T) {
	...
	t.Run("cleans up old seen notifications", func(t *testing.T) {
		...
		count := suite.Count(ctx, "SELECT COUNT(*) FROM billing.notifications WHERE id = $1", notification.ID)
	...
	t.Run("preserves recent data", func(t *testing.T) {
		...
		notifCount := suite.Count(ctx, "SELECT COUNT(*) FROM billing.notifications WHERE id = $1", notification.ID)
		assert.Equal(t, 1, notifCount, "Recent notification should be preserved")

		// Clean up test data
		_, _ = suite.Pool.Exec(ctx, "DELETE FROM billing.notifications WHERE id = $1", notification.ID)
	})
}
