...
func TestCCBillUserReactivation_RestoresEntitlementsAfterExpiration(t *testing.T) {
	runCCBillReactivationRestoresEntitlements(t, "Expiration", "testdata/webhooks/ccbill/expiration.json")
}

func TestCCBillUserReactivation_RestoresEntitlementsAfterChargeback(t *testing.T) {
	runCCBillReactivationRestoresEntitlements(t, "Chargeback", "testdata/webhooks/ccbill/chargeback.json")
}

func runCCBillReactivationRestoresEntitlements(t *testing.T, terminalEventType, terminalFixture string) {
	t.Helper()
	...
	// Seed an active subscription entitlement window so terminal events can revoke it.
	...
	terminalPayload := mustLoadJSONMap(t, terminalFixture)
	terminalPayload["subscriptionId"] = processorSubID
	terminalPayload["timestamp"] = now.Format("2006-01-02 15:04:05")
	terminalPayload["transactionId"] = "ccbill_term_" + uuid.New().String()

	postCCBillWebhook(t, suite.ServerURL, terminalEventType, terminalPayload)
	...
	latestEnt := mustGetSubscriptionEntitlement(t, suite, ctx, userID, created.ID, "premium")
