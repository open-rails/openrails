...
func TestFailMembershipDunningExhaustionSchedulesNMIDelete(t *testing.T) {
	...
	recentPeriodEnd := time.Now().Add(-2 * 24 * time.Hour) // within the 15-day dunning window
	retryAttempts := subscriptions.MaxDunningFailures - 1  // the next failure is the 5th = terminal
...
func TestFailMembershipExhaustionSetsDurableMarkerViaScheduler(t *testing.T) {
	...
	userID := uuid.New().String()
	pastRetry := time.Now().Add(-1 * time.Hour)
	retryAttempts := subscriptions.MaxDunningFailures - 1
