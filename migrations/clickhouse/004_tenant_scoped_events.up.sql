...
-- The materialized view (002) is NOT changed here; it is rebuilt to populate
-- tenant_id in migration 005, which also tenant-filters AdminMetricsService and
-- documents the platform-superadmin cross-tenant control-plane path
-- (AdminMetricsService.*CrossTenant). Until 005 runs, daily_metrics rows default
-- to the single default tenant, which is correct for self-hosted /
