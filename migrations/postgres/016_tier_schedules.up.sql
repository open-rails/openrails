...
--
-- SCOPE: tenant_subject_id NULL = the tenant-wide default schedule (the common
-- case — one ladder for the whole tenant). A non-NULL tenant_subject_id is a
...
-- tenant-wide default (tenant_subject_id IS NULL) coexist. Partial unique
-- indexes because NULL is not equal to NULL in a multi-column UNIQUE.
CREATE UNIQUE INDEX uq_tier_schedules_tenant_default ON openrails.tier_schedules (tenant_id, owner)
    WHERE (tenant_subject_id IS NULL);
CREATE UNIQUE INDEX uq_tier_schedules_tenant_subject ON openrails.tier_schedules (tenant_id, tenant_subject_id, owner)
    WHERE (tenant_subject_id IS NOT NULL);

ALTER TABLE ONLY openrails.tier_schedules
    ADD CONSTRAINT tier_schedules_tenant_subject_fk FOREIGN KEY (tenant_subject_id) REFERENCES openrails.tenant_subjects(id);
...
COMMENT ON COLUMN openrails.tier_schedules.tenant_subject_id IS 'NULL = tenant-wide default schedule; non-NULL = per-subject override taking precedence for that subject.';
...
CREATE POLICY tenant_isolation ON openrails.tier_schedules USING ((tenant_id = (NULLIF(current_setting('app.tenant_id'::text, true), ''::text))::uuid)) WITH CHECK ((tenant_id = (NULLIF(current_setting('app.tenant_id'::text, true), ''::text))::uuid));
