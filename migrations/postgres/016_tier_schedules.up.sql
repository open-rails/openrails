...
--
-- SCOPE: customer_id NULL = the tenant-wide default schedule (the common
-- case — one ladder for the whole tenant). A non-NULL customer_id is a
...
-- tenant-wide default (customer_id IS NULL) coexist. Partial unique
...
CREATE UNIQUE INDEX uq_tier_schedules_customer ON openrails.tier_schedules (merchant_id, customer_id, owner)
...
COMMENT ON COLUMN openrails.tier_schedules.customer_id IS 'NULL = tenant-wide default schedule; non-NULL = per-subject override taking precedence for that subject.';
