...
func TestConsolidatedSchemaIncludesMerchantProvisioningState(t *testing.T) {
	c := loadSchema001(t)
...
func TestConsolidatedSchemaOmitsProvisioningTransitionDDL(t *testing.T) {
	...
	for _, forbidden := range []string{
		...
		if strings.Contains(c, forbidden) {
			t.Errorf("001 schema must not carry merchant provisioning transition DDL %q", forbidden)
		}
	}
}
