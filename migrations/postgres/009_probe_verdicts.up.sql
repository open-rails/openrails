...
--
-- billing.probe_verdicts is keyed by (provider, key_hash) where key_hash is
...
CREATE TABLE billing.probe_verdicts (
...
COMMENT ON TABLE billing.probe_verdicts IS 'Cached NMI test-mode probe verdicts (#348): one row per (provider, sha256(security_key)). Fresh ''live'' refuses boot from cache, fresh ''simulated'' skips the probe, stale/missing re-probes. RLS-exempt by design: instance-level credential state, not tenant data.';
COMMENT ON COLUMN billing.probe_verdicts.key_hash IS 'sha256 hex of the provider security key. A rotated key hashes differently, so the cache never answers for a credential it has not seen.';

GRANT SELECT,INSERT,DELETE,UPDATE ON TABLE billing.probe_verdicts TO openrails_app;
