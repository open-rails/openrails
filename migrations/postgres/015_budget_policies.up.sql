...
-- is already merchant_subject_id). Never a slug/name (#475/#476 convention).
--
...
CREATE TABLE IF NOT EXISTS openrails.budget_policies (
    ...
    merchant_subject_id uuid CONSTRAINT budget_policies_merchant_subject_id_not_null NOT NULL,
    scope text NOT NULL,
    owner text NOT NULL,
    ...
    CONSTRAINT budget_policies_uniq UNIQUE (merchant_id, merchant_subject_id, scope, owner, scope_key)
);

ALTER TABLE ONLY openrails.budget_policies
    ADD CONSTRAINT budget_policies_merchant_subject_fk FOREIGN KEY (merchant_subject_id) REFERENCES openrails.merchant_subjects(id);
