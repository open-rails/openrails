...
func TestStandaloneCustomerExposureUnderConsole(t *testing.T) {
	...
	cfg := &config.Config{TestMode: config.CredentialPostureSandbox, SecretBackend: config.SecretBackendDB, Encryption: &config.EncryptionConfig{MasterKey: "AAECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8="}, DB: &config.DBConfig{URL: dbtest.SharedPostgresDSN(t)}, Auth: &config.AuthConfig{AllowMissingSenders: true, DirectPeerIP: true, Issuer: "https://gin.openrails.test", KeysPath: t.TempDir()}, AdminConsole: &config.AdminConsoleConfig{Enabled: true}}
