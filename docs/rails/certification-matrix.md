...
- **One-off, NMI** — classic Direct Post `type=sale` with mandatory credential-on-file indicators. last-verified: weekly / env: sandbox / how: `TestNMILiveLifecycleE2E` step 5. `TestLiveSandboxStoredCredentialCITThenMIT` is the scheduled sequence-specific proof; promote it as evidence only after a non-skipped passing run.
...
| Flow | NMI | CCBill | Stripe | Solana |
|---|---|---|---|---|
| Refund — full | `modeled` | `unsupported` (guarded) | `modeled` | `unsupported` (guarded) |
| Refund — partial | `modeled` | `unsupported` (guarded) | `modeled` | `unsupported` (guarded) |
...
- **Refund, CCBill** — automatic full and partial refunds are refused before reservation or provider execution. Combined cancel-and-refund requests are refused before cancellation. The unqualified outbound adapter has been removed; see the [documented contract and remaining qualification requirements](ccbill-refund-qualification.md). Local HTTP/PostgreSQL tests prove these guards, not provider money movement.
- **Refund verification, CCBill** — existing pending/unknown operations retain their reservations, payloads, and receipts for operator review. A historical synthetic reference or subscription counter cannot trigger automatic completion. The older July `-7` observation was not a general no-execution guarantee. Completed history and genuine inbound/manual refund accounting are unchanged.
...
- **Price push, Stripe** — find-or-create by content-addressed `lookup_key`. Two tests do reach Stripe test mode (`TestLiveStripeCatalogAutoCreateReusesContentKeys`, `TestLiveStripeExtrasListing`) but **no workflow runs either**, so they are still not standing evidence. Since or#896 they carry the `stripelive` build tag (`go test -tags=stripelive ./internal/service/ -run TestLiveStripe`), so they are absent from the default build rather than silently skipped in it.
