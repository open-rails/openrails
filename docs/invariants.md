...
| LED-8 | ~~An owed accrual is once per …~~ **Superseded by LED-14**, which covers every transfer type rather than this one. | `0042_…` replaced `idx_ledger_transfers_owed_accrual_once` | **DB** | — |
| LED-11 | ~~A balance debit is once per …~~ **Superseded by LED-14.** | `0042_…` replaced `idx_ledger_transfers_credit_spend_once` | **DB** | — |
