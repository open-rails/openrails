...
| DES-5 | No unattended irreversible provider call without provider-confirmed certainty, a blast-radius cap, and an operator kill switch that works without a deploy. | **MOSTLY ENFORCED** (or#836/#837) — DB-backed kill switch (default safe) on converge/refresh/intent-runner, cancellation cap `min(25, max(3, 5%))`, breaker floor removed, first pull advisory. Residual: the gate must be re-added to whatever replaces `jobs_dunning.go`, and or#856's Stripe webhook worker is still ungated. |
