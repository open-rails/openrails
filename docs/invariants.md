...
| LED-16 | **Request idempotency is Postgres, and it is not the charge guarantee.** `internal/modules/idempotency` claims `(merchant, operation, key)` in `openrails.idempotency_keys` (#1099): one owner per key across replicas, a renewed lease, one reclaim after the lease lapses, fenced writes, replayed results, GC by `expires_at`. A reclaimed request reruns, so every provider call it can reach goes through `rail_intents` (unique per merchant on a key derived from the request key), which never executes one twice. | package doc, `internal/modules/idempotency/store.go` | **DB** | `idempotency_keys_pkey`; `ci/greenfield/idempotency`, `TestReplicasCheckoutIdempotency` |
