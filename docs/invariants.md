...
| CUR-8 | Service entry points require a non-**empty** currency. The guard still checks emptiness only — `"XYZ"` passes there — but it is no longer the last line: every off-session charge is registry-validated at `ScopedCharger.ChargeSavedMethod`, and every internal→rail conversion refuses an unregistered currency by construction. | `pkg/service/currency.go:10-19`; `money/collection.go`; `moneyutil.NativeToRailMinor*` | APP (trivial at the entry point, **S** at the charge/convert boundary) | `go test ./internal/modules/money -run RefusesUnestablishedCurrency` |
