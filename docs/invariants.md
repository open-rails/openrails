...
| LED-16 | **Request idempotency is Postgres, and it is not the charge guarantee.** `internal/modules/idempotency` claims `(merchant, operation, key)` in `openrails.idempotency_keys` (#1099) on the database clock: one owner per key across replicas; the lease renews on its own pool, and the owner's context is cancelled before the lease could lapse unconfirmed; a random token fences a superseded owner's writes and, as a commit guard, its webhook transactions. Every provider call a request can reach goes through `rail_intents` (unique per merchant on a key derived from the request key), admitted only under the checkout session's row lock while the session is open. A session is failed only by a definite refusal with no admitted operation, and failed is final for its key. | package doc, `internal/modules/idempotency/store.go`, `checkout/session_refusal.go` | **DB** | `idempotency_keys_pkey`; `ci/greenfield/idempotency`, `TestReplicasCheckoutIdempotency`, `TestReplicasCheckoutLeaseLapse` |
