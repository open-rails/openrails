...
| LED-18 | **A capture's durable coordinate is composed from the caller's request id and engine constants ALONE** — `(op=capture, source="admit", source_id=request_id)`. No volatile state (the Redis hold ref, which the first capture consumes) and no caller-echoed field participates, so capture is idempotent on the caller's key UNCONDITIONALLY (or#907). Before this, `source` was recovered from the hold ref and rebuilt on retry from a caller-echoed `admit_source`; a blank echo landed at a different coordinate and debited again. With the coordinate constant, LED-14's unique index is the once-only fact and LED-15/or#891 give replay reporting + changed-amount refusal. | `pkg/service.CaptureHold` (`captureSourceNamespace`); `idx_ledger_transfers_operation_once` | **APP** + **DB** | `TestCaptureHold_FallbackRetryIsIdempotent` (non-default admit source, echo-free retry); `TestServiceAdmit_HTTP_EndToEnd` 3b/3c on the wire |
...
| FC-17 | **Business posture is a consequence of onboarding, never a settable flag** (or#908). The `customer_business_profiles` row IS the posture — no boolean exists anywhere, so a business posture with no terms acceptance behind it is unrepresentable (**S**). The row is written only by `money.OnboardBusinessCustomer` (refuses a blank `terms_version` / zero `terms_accepted_at`; the DB CHECKs back the terms gate) and deleted only by `money.OffboardBusinessCustomer`, which refuses while `GetOutstandingOwed > 0` — offboarding a debtor would hide the receivable from the arrears cycle (**APP**). | `internal/modules/money/business_profile.go`; `customer_business_profiles_terms_version_chk`; audit: `go test -tags integration ./internal/modules/money -run TestBusinessProfile` |
| FC-18 | **The or#910 dunning cycle recommends, never enforces.** Its last rung writes a `host_lifecycle_events` signal (`business.suspension_recommended`) and a profile watermark — no code path in `internal/modules/business` revokes an entitlement, blocks admission, or deletes data; reinstatement (`business.suspension_cleared`) is emitted automatically when the past-due book settles. The episode edges are CAS updates (`WHERE suspension_recommended_at IS NULL` / `IS NOT NULL`) committed atomically with their signal, so racing evaluators collapse onto one instruction per direction. Budget alerts notify and NEVER cap. | `internal/modules/business/cycle.go`; `RecommendBusinessSuspension` / `ClearBusinessSuspensionRecommendation` (customer_business_profiles.sql); audit: `go test -tags integration ./internal/modules/business -run TestBusinessCycle` |
