...
| FC-6b | Catalog authority is independent: `catalog_source=manifest` refuses catalog API writes, while `catalog_source=api` refuses mutating catalog pushes. Empty follows `merchant_config_source`; provider credential writes remain controlled by merchant source and secret-backend capability. | `config.validateMerchantConfigSource`; `catalogModeWriteGuardMW`; `manifestModeWriteGuardMW`; `hosttools.PushMerchantCatalog` |
