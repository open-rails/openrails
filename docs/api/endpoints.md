...
| GET | `/v1/merchant/billing-analysis` | `merchant:metrics:read` | Read provider-neutral daily billing evidence from retained provider data. Query: `from`, `to` (RFC3339 or `YYYY-MM-DD`, at most 366 calendar days), `timezone` (IANA), optional `provider`. Each day includes counts plus normalized `charges` and `failures` with provider references, source, and raw payload; `unbilled` is the cumulative open roster for that day. |
