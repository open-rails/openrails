...
| `202` | Unresolved: the same result with `operation.status` `pending`, `in_flight`, `failed_retryable` or `unknown_needs_verify`. The provider charge may already have happened; its outcome or local completion is still being reconciled. Under concurrent completion, the attempt may already be `settled` or `failed` while the operation remains unfinished. Use `operation.status` (or `recovery.operation` on reads) to decide whether to keep polling and whether another retry is allowed. Poll `GET /v1/me/invoices/{id}` (+ `/payments`) or `GET /v1/me/subscriptions/{id}`: `recovery.operation` names the operation until the verifier settles it from the exact receipt or an operator resolves it. Nothing is resent |
