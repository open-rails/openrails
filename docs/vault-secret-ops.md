...
|---|---|---|
| `dbSecretStore` + `encryptedSecretStore` | **default / self-hosted** (no Vault config) | `openrails.merchant_secrets`, envelope-encrypted (master key wraps per-merchant DEK in `openrails.merchant_deks`) |
| `vaultSecretStore` (KV-v2) | `vault.enabled: true` | Vault KV-v2 at `secret/openrails/merchants/<merchant-slug>/<name>` |
...
- `merchants.ErrSecretNotFound` — **terminal**: the secret is genuinely absent
  (never configured / merchant deprovisioned). Never retry; **never** treat as
  "verification disabled".
- `merchants.ErrSecretBackendUnavailable` — **retryable**: Vault unreachable /
  ...
  suspend a merchant, or skip a webhook signature check.

Wired examples: the merchant Stripe webhook route returns **503 (retryable,
...
is the trusted broker; merchant isolation is enforced in code by resolving the
merchant id to its stable slug and using the `(merchant_slug, name)` Vault path, not
by per-merchant Vault auth. The token is renewed in the background and re-acquired
on expiry.

A suggested per-merchant policy (future BYO-key self-service) scopes an operator to
only its subtree:

```hcl
path "secret/data/openrails/merchants/<merchant-slug>/*"     { capabilities = ["read"] }
path "secret/metadata/openrails/merchants/<merchant-slug>/*" { capabilities = ["list"] }
```
...
| Secret | Vault path example | Merchant dashboard writable |
|---|---|---|
...
An operator can preprovision provider credentials before a merchant ever opens a
dashboard:
...
2. **Export** existing secrets from `openrails.merchant_secrets`. They are
   envelope-encrypted, so decrypt through OpenRails rather than reading the column
   raw — enumerate with `Service` / `MerchantSecretStore.List(merchant)` then `Get`
...
3. **Write** each into Vault at the canonical slug address:
   `vault kv put secret/openrails/merchants/<merchant-slug>/<name> value=<plaintext>`
   (the store keys the value under `"value"`). Start with the `default` merchant.
4. **Flip** `vault.enabled: true` and restart. The store now resolves from Vault;
   names and callers are unchanged.
5. **Verify** per merchant: `PutCredential`'s `test` action (Stripe balance check)
   confirms the live key resolves from Vault.
6. **Decommission** the DB rows only after verification (keep a backup): the
   envelope-encrypted `openrails.merchant_secrets` / `openrails.merchant_deks` rows can be
   purged once every merchant resolves from Vault.

## Rotation (KV-v2 versions)

- Rotate via the admin credential API (`RotateCredential` → `Put`) or directly:
  `vault kv put secret/openrails/merchants/<merchant-slug>/<name> value=<new>`. KV-v2 keeps the
