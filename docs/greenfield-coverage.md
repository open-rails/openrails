...
| Provider-owned NMI and Stripe schedules | Import/replay, provider renewal and failure notifications, late/duplicate events, cancellation | Account-specific provider discovery |
| Legacy NMI-owned book | One-book import of every status and cadence (daily/monthly/yearly), multi-subscription customers, multi-card vaults, orphans and idempotent re-import; webhook/pull/duplicate/out-of-order/missing-notification mirroring; zero OpenRails charges across due pass, dunning, LIFE, crash and two replicas; member cancel (switch armed/disarmed), card update, refunds, tier-change refusal, account deletion; drift findings; engine takeover (held, abandoned, drift, crash, bulk under the breaker) and coexistence with engine memberships | NMI webhooks carrying only a foreign order id; month-frequency plans in the catalog |
| NMI schedule with OpenRails dunning | NMI schedule import, failed scheduled renewal, timed OpenRails rebill, exact receipt/period/access, replay and conflicting recurring-reference refusal | More decline categories and customer/provider races around the next regular charge |
