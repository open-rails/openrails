...
Use the engine customer UUID as HyperSwitch merchant_reference_id and its authenticated exact lookup endpoint to recover the vendor customer. Create conflicts must read back that exact reference; never select the first returned customer. Freeze the vendor session once the client sees it. An uncertain preparation must not silently retarget an already accepted engine session.
