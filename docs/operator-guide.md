...
| `openrails intents resolve --merchant=… --intent=… [--step=…] (--receipt=<provider id> \| --not-executed) --actor=… --reason=…` | close an `unknown_needs_verify` operation from an exact provider receipt (read back and matched) or provider-confirmed non-execution; never resends ([provider uncertainty](provider-uncertainty.md)) |
