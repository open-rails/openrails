...
func (w *world) legacyOnTier(tp topology, price tier, cents int64, cycle int, left time.Duration) *legacy {
	...
	railSub := w.nmi.legacyScheduleEvery(vault, price.plan, amount, cycle/24, 0, end)
	sale := w.nmi.scheduleSale(railSub, start)
	...
	require.Equal(t, "provider", subs.Data[0].CollectionPolicy)
...
func TestLegacyNMITierUpgradeDeclined(t *testing.T) {
	...
	for _, tp := range []topology{embedded, remote} {
		t.Run(string(tp), func(t *testing.T) {
			...
			require.Equal(t, "9.99", w.nmi.scheduleState(l.railSub).Amount)
			sub := w.subscription(tp, l.sub)
