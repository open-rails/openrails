...
- [ ] Migrate: `product_access_grants` → grants (ownership/entitlement); `entitlements.source_*` → `grant_id`; `money_blocks` lots → credit-kind grants. Post-migration derived grant effects must equal pre-migration state. (CUTOVER — joint with #512's backfill; `entitlements` already accepts `source_type='grant'`.)
...
- **#513 (admission)** balance cache is low-TTL and staleness-tolerant (Paul, 2026-06-17), so a fresh credit deposit becomes spendable within the cache TTL — no explicit deposit-bump from this layer.
...
**Completed:** no
**Status:** IN_PROGRESS 2026-06-17: design from an admission-path trace; two collision-safe simplifications LANDED (whole-repo `go build` + integration-tag `go vet` green). (1) **Dead fairness/throughput surface removed** (protection 2): deleted `ratelimit/concurrency.go` (`AcquireConcurrency`, zero callers) + `ratelimit/units.go` (#472 queue-pool `AcquireUnits`/`AcquireQueue`, zero callers) + both their tests; renamed the misnamed `models.ThroughputPolicy` → `models.TierMoneyPolicy` (it carries NO throughput axis — only $-budget + bad-spend windows). `AdmitRequest` already had no throughput field; `valuewindow.go`/`limiter.go` KEPT (live: wasted-spend + edge rate-limit). (2) **B1 dead spend-limit feature deleted (code-level)**: removed `CheckSpendAllowed`/`evaluateSpend`/`capInput`/`CapResult`/`dayWindow`/`monthWindow`/`SetSpendLimit`/`getSpendLimit`/`spentInWindow`/`activeHoldsTotal`/`ActiveHeldForCurrency` + the cap deny-codes from `spend_policy.go`; `AuthorizeSpend*` + the `SetSpendLimit` wrapper + `service.DenyInsufficientBalance` from `pkg/service/spend.go`; the `MoneySpendLimit` domain model + `spendLimitFromGen`; the `money_spend_limits` merchant-delete cascade entries; 2 dead test files + 2 dead test funcs (`strptr` re-homed to money `main_test.go`). PRESERVED + trimmed: `SpendDecision` → `{Allowed,HardStop,DenyCode}`; `BillingMode*` consts kept in `spend_policy.go`. The `money_spend_limits` TABLE drop (forward migration `005`) + sqlc query removal + regen are now ALSO DONE (the #512/#514 agent finished, unblocking the regen). So **B1 is fully complete** end-to-end. (3) **Phase D primitive BUILT + integration-tested** (8/8 green vs real Redis): the `spendgate` package was rebuilt to the agreed model — anchored estimate-based windows (session|fixed, per-user-anchored #337) + in-memory cached balance (no Redis `:bal`) + atomic admit/capture/release Lua. See the Phase D entry below. (4) **LIVE-PATH WIRE COMPLETE + tested (2026-06-17).** `admitter.Admit` + `Service.{Admit,CaptureHold,ReleaseHold}` now run on the `spendgate` gate: admit = one Lua EVAL (affordability + windows + hold) after an UNLOCKED balance read; capture = `spendgate.Capture` (free held) + durable `money.CaptureAuthorized` (#512 ledger); release = `spendgate.Release`. Capture/release resolve the payer from a Redis `sg:req:<merchant>:<reqID>` pointer (so the by-request-id wire is unchanged). New: `admission.SpendgatePolicyLoader` (tier_policies + budget_policies → scoped windows, FX-normalized to request currency) + `payerCapacity` (unlocked available + arrears credit-line read). DELETED: `internal/modules/holds`, the `budgets` Postgres engine (`budgets_service.go`/`scopes.go` → reduced to `types.go`), the `/credits/authorize` + `/credits/hold` routes/handlers + their `Service.{AuthorizeAndHold,HoldCredits}` methods, the legacy SDK `Authorize`/`HoldCredits`/`BudgetCheck` (client.go/remote.go/embed); dropped `budget_inflight_holds`/`budget_window_state` (migration `007`) + their sqlc queries + orphaned domain models. `BudgetStatus` rewired to read spendgate counters. **Per-user-staggered windows (Paul):** `fixed` resets at `offset + k*dur`, offset = deterministic hash of the customer-scoped key — staggered per payer, NO stored anchor (recomputable / flush-safe). **tensorhub lockstep DONE** (retyped `applyFailPolicy`/reconcile to `AdmitRequest`, deleted dead `AuthorizeHold`; builds green). TESTED: `spendgate` 8/8 + `admitter` 4/4 (incl. staggered-per-payer isolation) on real Postgres+Redis; **HTTP end-to-end** `tests/service_admit_http_integration_test.go` — drives the real routes `POST /v1/service/admit` + `/credits/holds/:id/{capture,release}` through the full route→handler→Service→spendgate stack on the live suite (Postgres+Redis+HTTP server): admit→402-when-insufficient→capture(durable ledger debit 1000→500)→release, all green; whole-repo build + vet + full integration-compile + schema test green; tensorhub builds. (5) **In-memory balance cache DONE + tested (2026-06-17).** `admission.BalanceCache` — process-local, `DefaultBalanceCacheTTL` = **10s**, nil-safe, keyed `(merchant,customer,currency)` → `{available, creditLine}`. Caches the one admit read that's a GROWING aggregation (`ledger.Balance` = SUM over the customer's transfers; the other admit reads are point lookups). Lives on `app.Runtime.AdmissionBalanceCache` (created in build_runtime); the admitter reads through it via a closure over `payerCapacity`. **NO explicit invalidation (Paul, 2026-06-17 — keep it dumb): every balance change just rides the TTL** — a deposit becomes spendable within ≤10s (accepted), a not-yet-refreshed debit is bounded over-admission (accepted, decision 8). The shared Redis `held` gauge (not the cache) carries cross-instance in-flight, so the cache holds only the SETTLED balance and need not change as holds accumulate. TESTED: unit test (hit/miss/TTL-expiry/nil-read-through/error-not-cached; fake clock + counting loader, no DB) + the HTTP e2e runs against the cache live. **PLANNED removal (Paul, 2026-06-17):** this cache is the stopgap for the O(n) balance SUM — once #512 Phase H denormalizes balance to an O(1) row read, delete `admission.BalanceCache` + its wiring (no more staleness window). `money.AuthorizeAndHold` (read-only capacity calc) kept as a money-package helper its tests use. (6) **Policy cache DONE + tested (2026-06-17).** `admission.PolicyCache` — process-local, `DefaultPolicyCacheTTL` = **10 min**, nil-safe, caches the two admit CONFIG reads (`GetTierPolicy` keyed `(merchant,payer,tier)` + `LoadAll`/budget_policies keyed `(merchant,payer)`) via `SpendgatePolicyLoader.WithCache`. Long TTL is safe — policy is read-mostly config (a cap edit applies within the TTL); the live spend COUNTERS stay in Redis. Pure TTL, no invalidation (same "keep it dumb" approach). Lives on `app.Runtime.AdmissionPolicyCache`. NET: a warm-payer admit does **zero Postgres reads** (policy-cache hit + balance cache/O(1) + one Redis EVAL). Unit-tested + exercised live by the HTTP e2e. **#513 is now complete** (read-side fully off Postgres on a warm path). (7) **Spend-cap RENAME DONE (Paul, 2026-06-17 — "Full: SDK + routes").** The two confusingly-named cap tables + their Go/SDK/route surface renamed to one consistent `*SpendCap*` vocabulary. **Tables:** `tier_policies`→`tier_spend_caps`, `budget_policies`→`scoped_spend_caps` (constraints/indexes/RLS/comments followed; **columns KEPT** — `policy`/`policy_version`/`windows`/`scope`/`owner`/`scope_key` read clearly under the new names; renaming them would ripple through sqlc structs for marginal gain). **sqlc queries** renamed (`UpsertTierSpendCap[Default]`, `GetTierSpendCaps`, `Upsert/Delete/List ScopedSpendCap[s][ByOwner]`) + regenerated against live Postgres (gen `OpenrailsTierSpendCap`/`OpenrailsScopedSpendCap`). **Go internal:** `TierPolicyStore`→`TierSpendCapStore`, `BudgetPolicyStore`→`ScopedSpendCapStore`, `BudgetScopePolicy`→`ScopedSpendCap`, `ResolvedPolicy`→`TierSpendCaps`, `GetTierPolicy`→`GetTierSpendCaps`, `PolicyCache.{TierPolicy,BudgetPolicies}`→`{TierSpendCaps,ScopedSpendCaps}`. **SDK (client/remote/embed):** types `TierPolicyInput`→`TierSpendCapInput`, `SubjectBudgetPolicyInput`→`SubjectSpendCapInput`, `PlatformBudgetPolicyInput`→`PlatformSpendCapInput`, `SubjectBudgetPolicy`→`SubjectSpendCap`, `BudgetScopeWindow`→`SpendCapWindow`; methods `SetTierPolicy`→`SetTierSpendCaps`, `SetSubjectBudgetPolicy`→`SetSubjectSpendCaps`, `SetPlatformBudgetPolicy`→`SetPlatformSpendCaps`, `SubjectBudgetPolicies`→`SubjectSpendCaps`; service `Service.Set*`/handlers `ServiceSet*` to match. **Routes:** `PUT /v1/service/tier-policies`→`/tier-spend-caps`; `/budget-policies/{subject,platform}`→`/spend-caps/{subject,platform}`. **KEPT on purpose** (separate concern from cap naming): the window/jsonb primitive — `models.TierPolicy`/`TierMoneyPolicy`/`BudgetWindowPolicy`, SDK `BudgetWindow`/`BudgetWindowInput`, the `budget_windows`/`windows` JSON body keys, and the tier SCHEDULE (#476). Verified: openrails whole-repo build+vet+gofmt, full no-tag unit suite, spendgate 8/8 + admitter 4/4 + HTTP e2e + schema text tests all green. **tensorhub lockstep:** the 7 SDK call-site files renamed in lockstep (`platform_tier_sync`/`platform_budget_sync`/`platformpolicy`/etc.); `go build ./...` verified green against LOCAL openrails (temp `replace`, since tensorhub pins openrails v0.39.0 which still has old names — real build/CI green needs the openrails tag+bump). **TAIL (pre-existing, NOT from this rename — finish to close #513 in tensorhub):** two tensorhub TEST files still reference the #513-DELETED `Authorize`/`HoldCredits` SDK surface and fail test-compile against new openrails — `internal/api/openrails_test.go` (`client.HoldCredits`/`HoldCreditsRequest`, 2 test funcs, need rewiring to the Admit/CaptureHold flow) + `internal/billing/openrailsclient/openrailsclient_test.go` (`fakeUnified.Authorize`/`openrails.AuthorizeRequest`/`AuthorizeResponse` dead stub — delete). The prior session's "tensorhub lockstep DONE/builds green" was `go build`-only and missed these `go test`-compile breaks. Make the admission/holds path SIMPLE and FAST by moving all *accounting* (balance check, budget windows, per-request holds) to Redis + an in-memory balance cache, keeping only *policy* (caps/tiers) as cached config, and removing Postgres from the hot path entirely. Separate from #512 (which owns ledger consistency/auditability); the two meet only at the durable-spend seam (capture enqueues the #512 ledger write off the hot path).
...
1. **Policy is cached config; accounting is Redis counters.** Nothing on the hot path reads or locks Postgres.
...
### C. Cache balance + policy in memory (no hot-path Postgres)
- [ ] Balance cache: in-memory integer per (merchant, customer, currency), refreshed off the #512 ledger (TTL) and bumped/invalidated on deposit/top-up so a funded payer can spend immediately. Per-instance staleness is fine (bounded over-spend).
- [ ] Policy cache: load `tier_policies`/`budget_policies`/tier ladders into memory, invalidate on the existing `policy_version`/`schedule_version`/`config_version` bumps. (These version columns already exist — they were built for this.)
...
- **In-memory balance, not Redis `:bal`.** `Admit` takes the caller's cached balance as `CachedBalance` ARGV; the affordability gate is `cachedBalance − held − cost ≥ −creditLimit`. Only the shared `held` gauge + per-request `hold:<reqID>` + window counters live in Redis. NO `:bal` key, NO balance-refresh job — stale cache → bounded over-admission (accepted). Matches Paul's "cache balance in memory; fine if spend goes slightly negative."
...
Still TODO for the wire: Postgres→`spendgate.Policy` loader (tier_policies + budget_policies → scoped windows, carrying cadence), in-memory balance cache (refreshed off the #512 ledger, invalidated on deposit), `held`-gauge reconciliation sweep, and the admitter/`Service.{Admit,CaptureHold,ReleaseHold}` rewire that deletes `internal/modules/holds` + the `budgets` Postgres machinery + `money.AuthorizeAndHold`'s capacity read.
...
- [ ] Admit = one `EVAL`: check `cached_balance − Σ holds ≥ cost` AND every window `used + reserved + cost ≤ limit`; if all pass, place the request-id hold + bump reserved. One round trip, no Postgres.
...
- [ ] Holds in SHARED Redis keyed by the provider/tensorhub request-id (cross-instance agreement); balance cache may be per-instance.
...
- [ ] Capture deducts from the balance cache, releases the hold, and best-effort (in-memory/async) hands the spend to the #512 ledger writer; a background drainer applies it + refreshes the cache. A process crash may drop not-yet-applied captures — that unbilled spend is part of the same bounded leak (small per-request amounts) and is accepted. No at-least-once outbox, no WAL.
...
- [ ] Balance-cache staleness bound + deposit invalidation (funded payer can immediately spend).
...
**Completed:** yes
**Status:** COMPLETE 2026-06-17 (HARD CUT done; all implementable phases closed). Phases A+B+C+G shipped + tested; D CANCELLED (YAGNI — typed columns beat a `user_data` jsonb); E CLOSED as not-needed (each currency = own wallet); F trigger documented (lock metric deferred — no ops-metrics infra). The live money path is fully on the double-entry ledger: `deriveBalance` returns the customer's `customer_balance` account net (credits − debits over posted+post_pending); `depositTx`/spend/withdraw/owed/invoice all emit immutable transfers (deposit=DR processor_clearing/CR customer_balance; spend=DR customer_balance/CR platform_revenue; owed accrual=DR arrears_liability/CR platform_revenue, settlement=DR processor_clearing/CR arrears_liability; expiry=DR customer_balance/CR expired_credits). Arrears stopped being a hand-maintained scalar — it's the `arrears_liability` account. `balance_after` removed; transaction history (`GetTransactions`/`GetTransactionBySource`) derives from `ledger_transfers`. `migrations/postgres/004_drop_single_entry.up.sql` DROPs `money_blocks` + `money_transactions` (pre-launch hard cut, no backfill). Exit gate met: no source reader of `balance_after`/`money_blocks`/`money_transactions` remains (only explanatory comments); conservation (`Σ ledger == 0`) holds (tested); `go build ./... && go vet ./...` clean. Validated live: `money/ledger` + `grants` + `money` (67/67) + `river` + `admission` + `pkg/service` all green. C (durable-spend writer) is live via `CaptureAuthorized`→`spendBalanceThenOwedTx`; the Redis hot-path/cache is #513. D CANCELLED (`user_data` jsonb is YAGNI — typed columns are indexed + queried). E CLOSED (each currency = own wallet; reopen only for cross-currency settlement). F: TB-adoption trigger documented; the lock-contention metric is deferred to future ops-metrics infra. **Phase H ADDED 2026-06-17 (Paul) — PLANNED, not yet built:** denormalize running counters onto `ledger_accounts` (`{credits,debits}_{posted,pending}`) so `Balance`/`Held`/`checkDebitFloor` become O(1) row reads instead of growing `SUM`s, and available-balance + available-credit come from ONE point-lookup JOIN (arrears = the balance goes negative to `−credit_limit`, so the negative balance IS the used credit — no separate owed subtraction). Conservation re-SUM is the hard-gate invariant test; on landing, the #513 in-memory balance cache is deleted (it was the stopgap for this SUM). See Phase H for the full design. | (Phase A foundation, earlier 2026-06-17) **double-entry foundation BUILT + integration-tested, 5/5 green** (`migrations/postgres/002_ledger.up.sql`, `internal/db/queries/ledger.sql`, `internal/modules/money/ledger` applier; conservation + two-phase + sign-constraint + currency-guard + append-only tests). Original design context follows. — Re-shape the money ledger from a single-entry, mutate-in-place, per-customer-balance store into a TigerBeetle-style **double-entry, append-only ledger** (accounts + immutable transfers, balances derived, holds = one durable two-phase primitive), WITHOUT adopting TigerBeetle-the-database. Today there is NO counter-account anywhere (`grep` for debit/credit/counter-account in `internal/modules/money` returns nothing): a deposit mints a `money_block`, a charge decrements `money_blocks.remaining_amount`, and money is not conserved.
...
The full admission + holds redesign (Redis-only accounting, cached balance/policy, atomic Lua admit/capture, cap-taxonomy collapse, durable-spend seam) lives in **#513**. This ledger issue only consumes its output: the durable spend that #513's capture enqueues is written here as immutable double-entry transfers (Phases A/B), off the hot path.
...
- [ ] If yes: model conversion as two linked transfers through a per-merchant `fx_liquidity` account (DR payer A-balance / CR fx_liquidity-A; DR fx_liquidity-B / CR settle-B), atomic via `flags.linked`; the liquidity account accrues spread / FX P&L.
- [ ] Record the quote (rate, AsOf, provider) on the conversion transfers for audit; needs a REAL `fx.Provider` (the NoOp returns 1.0).
- [ ] Decide explicit user-initiated convert (simpler, auditable — RECOMMENDED) vs implicit cross-currency spend at spot (magical, riskier: rate at spend time, partial draws across currencies).
- [ ] "No cross-currency in one transfer" (Phase A CHECK) is already true by construction today. (custom credits remain no-FX by design — #475)
...
- [ ] Add `credits_posted`/`debits_posted`/`credits_pending`/`debits_pending` (bigint NOT NULL default 0) to `ledger_accounts` (migration 008) + a one-time backfill (`UPDATE … SET credits_posted = (SELECT SUM(amount) FROM ledger_transfers WHERE credit_account_id = a.id AND phase IN ('posted','post_pending')) …` per account).
- [ ] `Apply` updates the counters of the 1–2 affected accounts in the SAME tx as the `INSERT ledger_transfers`, per phase: **posted** → debit.debits_posted += amt, credit.credits_posted += amt; **pending** → debit.debits_pending += amt (+ credit.credits_pending += amt); **post_pending** (capture, may true-up to actual) → move pending→posted on both legs at the resolved amount; **void_pending** (release) → debit.debits_pending −= amt (+ credit.credits_pending −= amt).
- [ ] `Balance` = `credits_posted − debits_posted` (O(1) row read); `Held` = `debits_pending` (O(1)); `checkDebitFloor` reads the row instead of re-summing.
...
- [ ] Conservation stays the HARD-GATE invariant test: `Σ(transfers for account) == stored counters` (drift = wrong money). Extend `TestLedger_Conservation*` to assert the counters equal the re-SUM after a transfer sequence.
...
- [ ] WHEN THIS LANDS: delete the #513 in-memory balance cache (`admission.BalanceCache` + its wiring). It was the stopgap for this O(n) SUM; once balance is an O(1) row read the cache is redundant — removing it drops the only staleness window. (Paul, 2026-06-17.)
...
- A ONE-TIME backfill migration is required (replay/translate existing `money_transactions` + `money_blocks` into accounts + transfers) — that's the only "old data" step. After it: DROP the single-entry tables/columns. Specifically `money_transactions.balance_after` is removed (balances are derived), `money_blocks` mutation/expiry is replaced, and there is NO dual-read (old single-entry + new double-entry) and NO compatibility view kept around.
...
- Consistency-invariant overlap + DEPENDENCY: #511 (unified consistency engine). The conservation check (`Σ ledger == 0`) stays a #512 ledger diagnostic/test, not a #511 finding unless later promoted as a `CON-2` amount-mismatch subtype. #511's money-side consistency checks (`CON-1` duplicate real charge, `CON-2` refund/payment/invoice/credit amount mismatch) must be written against provider-observed payments/refunds plus #512/#514 source events, not the deleted single-entry `money_transactions`.
...
- [ ] **Depends on #514 (grant ledger):** the DERIVE plane derives grant effects from the append-only grant log; #514 owns the `grants` schema, the migration, and derive-1/derive-2 (the sole writers of grants and grant effects). This issue consumes and verifies them — it does not own the grants schema.
- [ ] Extend `reconciliation_findings`: admit `PULL-*` / `DERIVE-*` / `LIFE-*` / `CON-*` `finding_type` codes (replace the old `PS-*` CHECK with the new set / registry validation); allow a `self` value in `provider` for DERIVE/LIFE/CON findings.
...
- [ ] Provider-Pull coverage: charges→payments, subscription observed status/period/next-bill, refunds, chargebacks/disputes, vault→payment_methods, and remote catalog/plan observations as evidence for `CON-2 consistency.amount_mismatch.provider_catalog` only (never as local catalog overwrites).
- [ ] Provider-action preservation in the overwrite: never clobber `deletion_scheduled_at`/`scheduled_price_id`/pending intents; compute divergence against (provider-observed state + scheduled future transitions); treat not-yet-due lag as consistent, retryable lag as provider action state, and abandoned past-due work as `LIFE-5`.
...
- [ ] Plane passes run in order DERIVE -> LIFE -> CON (PULL is the pull); each plane is a module. Provider actions are emitted by those passes when remediation must happen outside OpenRails.
- [ ] derive-1 (event → grant) / derive-2 (grant → grant effect) are provided by #514 (the sole writers of grants and grant effects); the Convergence Engine invokes them and verifies their outputs against the source ledger — it never writes grant effects directly.
- [ ] Historical replay anchored to source-event timestamps, judged against the grant's `*_spec_snapshot` (never live product spec).
- [ ] Converge-not-replay for the LIFE plane; confirmed-absence gate on every EXCESS repair.

### D. Invariant checks + appliers (catalogue, `docs/consistency-invariants.md` §5)
- [ ] Use `docs/consistency-invariants.md` §5's Postgres-enforced vs application-enforced catalogue as the implementation checklist; first prefer schema hardening for same-merchant references and impossible local states. DB-enforced invariants do not need stable finding codes; only the remaining #511 cross-table/cross-time/provider invariants become `reconciliation_findings` codes.
- [ ] DERIVE grant tier: `DERIVE-1` grant.missing (event->grant) / `DERIVE-2` grant.excess (gated) / `DERIVE-3` grant.mismatch — against the #514 grant log; uniform across entitlements/credits/access.
- [ ] DERIVE grant-effect tier: `DERIVE-4` grant_effect.missing (grant->effect) / `DERIVE-5` grant_effect.excess (gated; credit clawback = unspent only) / `DERIVE-6` grant_effect.mismatch (incl. per-renewal credit cadence) — derive-2; credit + product-access grant effects are NEW beyond entitlements. The credit grant effect is a #512 `ledger_transfer`; entitlement + access grant effects are local rows.
- [ ] LIFE subscription period-overdue / dunning-overdue / grace-exhausted / pending-stale (`LIFE-1`..`LIFE-4`), provider-intent-abandoned (`LIFE-5`), checkout-session-stale (`LIFE-6`).
...
- [ ] CON is limited to three generic consistency classes, with specific surfaces carried in subtype slugs/metadata: duplicate (`CON-1`, e.g. `consistency.duplicate.provider_charge` for already-collected duplicate money, `consistency.duplicate.remote_subscription` for extra active billable remote subscriptions before duplicate money is collected, `consistency.duplicate.invoice_usage`), amount/explainability mismatch (`CON-2`, e.g. `consistency.amount_mismatch.refund_math`, `consistency.amount_mismatch.invoice_math`, `consistency.amount_mismatch.credit_lot`, `consistency.amount_mismatch.provider_catalog`; #512 ledger conservation only if promoted from diagnostic/test), and reference resolution (`CON-3`, e.g. `consistency.reference.catalog_scope`, `consistency.reference.ledger_reference`, `consistency.reference.provider_intent_reference`). Subscription price/product coherence is schema-enforced by `subscriptions_price_product_merchant_fkey`, not a CON finding.
