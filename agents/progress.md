...
OpenRails `merchant_cors` should go away. It currently looks per-merchant but is only flattened into `AllowedCORSOrigins()` and passed to generic CORS middleware before auth. In public merchant registration, that global preflight union is not tenant isolation: every registered merchant origin is allowed before the request carries a JWT issuer.
...
**Tasks:**
- [ ] After AuthKit #85 lands, delete `merchant_cors` from `config.Config`, `config.example.yaml`, `.env.example` docs, and validation/tests.
- [ ] Keep only minimal `cors_origins` if still needed for coarse deployment ingress/dev compatibility; do not imply tenant isolation.
- [ ] Replace current flattened CORS use on delegated browser surfaces with AuthKit-provided CORS/delegated middleware or host-provided `DelegatedAuthenticator` integration.
- [ ] Ensure `/v1/self/*` and `/v1/merchant-admin/*` receive an AuthKit-resolved principal; OpenRails should not inspect JWT issuer/origin itself when AuthKit can do it.
- [ ] Do not apply browser origin policy to webhooks or server-to-server service-token routes.
- [ ] Tests: `merchant_cors` config is rejected/absent, stale examples are gone, delegated browser route uses AuthKit integration, and webhook/service-token routes are unaffected.
...
- [x] Account-bind every pull using #518: DONE (#518 provider-accounts). `reconcileProviderAccountBindings` resolves each provider to its `provider_accounts` row via `intents.Store.VerifyOrBindPrimaryProviderAccount`; `--provider-account <uuid>` targets one explicitly (`resolveReconcileProviderAccountTarget`). The engine threads `RunParams.ProviderAccounts map[Provider]ProviderAccountBinding` (`ID` = `provider_accounts.id`, `AccountID` = external id) into every section.
- [x] Abort on merchant/provider account mismatch (#518): DONE. `VerifyOrBindPrimaryProviderAccount` binds the primary account on first use and returns an operator-facing error when the live credentials resolve to a different account than the bound row — the pull aborts before any write/compare for that provider.
- [x] Stamp provider-owned mirror rows/run evidence with #518 `provider_account_id` for new writes: DONE. `bindApplyActions` stamps the resolved `ProviderAccountID` on every `BackfillPayment`/`RecordRefund`/`Materialize` action; `Local.Load`/`PaymentsByTransactionIDs` filter by the bound account (`nullableProviderAccountID`), so comparisons/overwrites require the same account row and legacy null-binding rows are not destructively reconciled.
- [x] Provider-action preservation in the overwrite: DONE by applier construction. `ReconcileAdoptSubscriptionStatus` updates ONLY `status` + period bounds `WHERE status <> 'cancelled'` — it never touches `deletion_scheduled_at`/`scheduled_price_id`/pending `provider_intents`; abandoned past-due provider work surfaces as `life.provider_intent.abandoned` (LIFE pass). Schedule-aware: terminal/scheduled local state is preserved.
- [x] `--since` / date-range flag for historical backfill; default pulls the head. DONE: `--since`/`--until` (`parseReconcileCLITime`) thread to `RunParams.Since/Until`; bare pull = head; a windowed pull does NOT flip the confirmed-absence gate (only a full HEAD pull proves absence).
...
- [ ] DERIVE grant tier: `derive.grant.missing` (event->grant) / `derive.grant.excess` (gated) / `derive.grant.mismatch` — against the #514 grant log; uniform across entitlements/credits/access. **BLOCKED on the grant-ledger WRITE-PATH unification (finding, 2026-06-18):** grant CREATION goes through the ledger (2c), but REVOCATION and EXTENSION still bypass it — `RevokeSourcesForSubscription`/`RevokeExistingEntitlement`/`EndActiveByPayment` revoke entitlement *windows* directly (leaving the grant live-but-effect-revoked), and `ExtendActiveBySubscription` (admin_service + renewal) mutates a window's `end_at` directly. Consequence: a strict `derive.grant.mismatch` / `derive.grant_effect.mismatch` (effect must equal the grant's window/spec) FALSE-POSITIVES on every extended or window-revoked grant, and `derive.grant.excess` would fire on every cancelled-sub grant. Making these precise requires routing revoke + extend THROUGH the ledger (revoke event → derive-2 retracts; extension as a new/superseding grant) AND giving the retraction path an as-of timestamp (converge-not-replay: revoke as-of grace-end, not the event time). That is a careful, high-risk refactor of the access-revocation path (touches lifecycle + 3 webhook services + admin) and should be its OWN reviewed unit — NOT bundled with the rest. NOTE: no re-grant bug today — `EntitlementExistsForGrant` keys on `deleted_at IS NULL` (not `revoked_at`), so the DERIVE sweep treats a revoked window as still-materialized and never re-grants cancelled access (verified by the green LIFE/sweep tests).
...
- [~] Triage report for `held` findings before any destructive repair is approved. PARTIAL 2026-06-18: added `FindingStatusHeld` and included `held` in the `pull-provider report` standing-findings list (`cmd/openrails/reconcile.go`) alongside `open`/`admin_pending`, so gate-withheld destructive EXCESS repairs surface for operator triage. REMAINING: a richer triage view (group by source_domain + reconciled-state, surface the evidence) if needed.
...
**Status:** IN_PROGRESS — **step 1 (ownership) DONE + verified 2026-06-18; step 2 (entitlements) is next.**
**Step 1 (ownership) COMPLETE:** `internal/db/repo/product_access_grant.go` is now backed by the grant ledger (`grants` kind=ownership) — writes via `grants.Grant`/`Revoke`, reads via `ListLiveGrantsByCustomer` + a new `ListOwnershipGrantsWithStatus` (LEFT JOINs each grant to its termination event to reconstruct the legacy `{status,revoked_at,revoke_reason}` shape) + `ListLiveOwnershipGrantIDsByPayment`; maps `gen.OpenrailsGrant`→`models.ProductAccessGrant`. Service + all callers (handlers/product_access, checkout/purchase_service) UNTOUCHED. `product_access_grants` table DROPPED (`migrations/postgres/010_retire_product_access_grants.up.sql`) + its sqlc queries + gen type removed. Verified: all 4 `productaccess` integration tests green (idempotent grant, has+list, revoke-by-payment refund w/ status derivation, revoke-by-id), whole-repo `go build` + `go vet` clean. NOTE: `grants.payment_id` has a real FK to `payments` (the old table had none) — a good constraint; ownership-from-purchase must reference a real payment (test seeds one). The obsolete `TestProductAccessGrants_RealMigration_RLS` (probed the dropped table directly) was deleted — ownership RLS now rides the `grants` table's own FORCE RLS.
**Step 2 (entitlements) — DESIGN IMPROVED 2026-06-18 (grant_id link, NOT a source_type flip):** the mapping found ~15 queries keyed on `entitlement.source_type IN (subscription/one_off/grace)` AND `source_id=<sub/payment>` (revoke/extend/resume/grace/one-off/distinct-names + the CON orphan checks + the reconcile appliers). Flipping `source_type`→`grant` (the #514 `MaterializeGrant` behavior) would break ALL of them → too invasive. INSTEAD: add a dedicated **`entitlements.grant_id`** FK column. `MaterializeGrant` sets `grant_id=grant.id` and PRESERVES the grant's real `source_type` + `source_id` (e.g. `subscription`/`<sub>`), so every source-keyed reader AND the CON check keep working unchanged; the existence/revoke checks (`EntitlementExistsForGrant`, `RevokeEntitlementsByGrant`) key on `grant_id`; DERIVE links via `grant_id`. ~10x less ripple, and "ledger is sole source of truth" still holds (every entitlement is created only by `MaterializeGrant` from a grant; `source_type/source_id` become denormalized projection fields the sole-writer keeps consistent).
Execution order: **2a DONE** — `migrations/postgres/011_entitlements_grant_id.up.sql` adds `entitlements.grant_id` (nullable FK→grants, partial index). **2b DONE + verified** — `MaterializeGrant` now sets `grant_id=g.ID` and PRESERVES the entitlement's semantic source (handling both wrinkles: `entitlementSourceType()` maps `purchase`→`one_off`; `uuid.Parse(g.SourceID)` with fallback to `g.ID`); `CreateEntitlement` gained a `grant_id` column; `EntitlementExistsForGrant`/`RevokeEntitlementsByGrant`/`GrantHasLiveEntitlement` re-keyed from `source_type='grant' AND source_id` → `grant_id`. Verified green: `internal/modules/grants` + `internal/reconcile/converge` suites (updated `entWindows` helper + 2 grants tests + the converge TerminatedNotRetracted test — now grace-sourced so the entitlement isn't also CON-orphan-flagged, which confirmed the source-preservation + CON interaction works as intended). NOTE: 2b is foundation only — no production behavior change yet (nothing creates entitlement-kind grants in prod until 2c). **2c DONE + verified (the access-critical activation)** — `PushNewEntitlement`'s terminal insert now creates an entitlement-kind grant (`grants.Grant`, reverse-mapping `one_off`→`purchase` via `grantSourceType`) + `MaterializeGrant`, keeping its timeline-window computation; returns the projected window via a new `GetEntitlementByGrant` fetch-back (couldn't reuse `GetTimelineCoveringWindow` — it uses strict `start_at < at` so it misses a window starting exactly at `start`). Verified green: `entitlements` + `subscriptions` (lifecycle/grace/renewal) + `grants` + `converge` + `productaccess` suites ALL pass together. Fixed 6 bit-rotted merchant-less ctxs in `lifecycle_grace_integration_test.go` (+ seeded product/price under the test merchant) and 1 in `entitlement_service_integration_test.go` — pre-existing rot (the Service required a merchant via `MerchantTx` long before this; these never ran in CI), surfaced earlier by the new `merchant.Require`. **MILESTONE: all THREE grant kinds (credit/ownership/entitlement) now flow through the ledger — entitlement creation is grant-derived.** **2d DONE + verified (2026-06-18): `entitlement_grants` retired — the grant ledger is the sole source of truth for all three grant kinds.** Both writers stopped creating the table (`GrantAdminEntitlement` handler + `service_admin.EntitlementGrantEntitlement` now mint a uuid `SourceID` and rely on 2c's `PushNewEntitlement(source=admin)` ledger path; removed the now-unused `repo` import + `models.EntitlementGrant`). Removed every reader: the prune `PaymentHasProtectedDependents` clause, the CON `ConOrphanEntitlementAdminSource` query + its conPass loop (admin source has no remote truth — never an orphan), the merchant-delete count/purge + `merchantOwnedTables` entry/cases. Deleted `entitlement_grants.sql` / `admin_grant.go` repo + model + the gen file. Dropped the table via `migrations/postgres/013_retire_entitlement_grants.up.sql` AND from the `001` consolidated baseline (greenfield — no legacy data), plus `product_access_grants` (its step-1 drop) was scrubbed from both schema-test lists. Decision (Paul): **accept the loss of the `granted_by` operator-audit column** — analytics is ClickHouse-fed (`ch:"entitlements_granted"`), not from this table, so no metric breaks. Verified green together: `grants` + `converge` + `productaccess` + `entitlements` + `subscriptions` + `migrations/postgres` suites; whole-repo `go build ./...` + `go vet ./...` clean; zero dangling references to the deleted symbols/table. **STEP 2 COMPLETE — #513 closed.** (Pre-existing unrelated red: `internal/merchants TestCredentialRotation_WritesAudit` is #518 credential-rotation-audit churn, does not touch entitlement_grants.)
**TWO WRINKLES `MaterializeGrant` MUST handle (found while executing 2b — get these wrong → silent wrong entitlement attribution → access bugs):** (i) **source_id typing** — `grants.source_id` is free TEXT (the converge derive test uses `"pay_<rand>"`), but `entitlements.source_id` is `uuid NOT NULL`; so `MaterializeGrant` must `uuid.Parse(g.SourceID)` and, on success, use it as the entitlement's source_id (so `ReconcileRevokeSubscriptionEntitlements WHERE source_id=<sub>` resolves), else fall back to `g.ID`. (ii) **source_type taxonomy mismatch** — grants use `purchase/subscription/admin/grace`; entitlements use `subscription/one_off/admin/grace`; so `purchase`→`one_off`, others pass through. Both are why this is a careful, fully-tested pass, not a quick edit. (Original step-1 ownership recipe above retained.)
...
### J. LIFE repairs → shared lifecycle core (DONE + verified 2026-06-18)
**DONE 2026-06-18:** the LIFE-plane terminal repairs now route through a side-effect-free shared local-state core on `SubscriptionLifecycleService`, so a converged cancel is byte-identical to a user-driven one and the Solana-cascade bug is fixed. What landed:
- **`ApplyLocalCancellation(ctx, dbb, sub, LocalCancellation{...})`** + **`ApplyLocalPastDue(ctx, dbb, sub, graceEndsAt)`** (`internal/modules/subscriptions/lifecycle_service.go`): terminal status flip + ended/cancel fields + `ClearRetrySchedule` + the #264 Solana cranker cascade + as-of entitlement revoke; NO notifications/event-log/provider-intent (those stay in the caller). Operates on the caller-supplied `dbb` so the caller owns atomicity (`MerchantTx` for the user path; the merchant-scoped conn for convergence). `cancelled_at = min(now, ended_at)` so `chk_ended_not_before_cancelled` holds when access ended in the past.
- **`CancelMembership` refactored** to compute its period policy (immediate-revoke truncation vs period-end keep-access; which sources to revoke) then call `ApplyLocalCancellation` — one local-state implementation, side-effects layered after.
- **As-of revoke**: `RevokeExistingEntitlementParams.AsOf *time.Time` + `EntitlementService.RevokeSourcesForSubscriptionAsOf(...)`; `RevokeSourcesForSubscription` delegates with `now` (6 existing callers untouched). grace_exhausted revokes the sub's paid AND grace windows AS-OF grace-end (converge-not-replay), `cancel_type=expired`, reason `dunning_failed`.
- **Engine wiring**: `NewConvergeEngine(db)` builds the lifecycle service from `db` (side-effect deps nil; real clock — the pass passes its own instants in); `converge`→`subscriptions` confirmed acyclic. The LIFE pass's grace_exhausted/pending_stale `Repair` closures `GetByID` the sub then call `ApplyLocalCancellation`; period_overdue calls `ApplyLocalPastDue`. dunning_overdue keeps `SetSubscriptionNextRetry` (a retry re-arm, no cancel/past_due twin).
- **SQL**: `MarkSubscriptionPastDueFromOverdue` deleted (genuinely orphaned) + `sqlc generate` (clean — also synced the stale gen from 2c/2d/#514). **SCOPING NOTE / deviation from the original plan:** `ReconcileCancelSubscriptionLocal` + `ReconcileRevokeSubscriptionEntitlements` are NOT deleted — they are the LIVE **PULL engine's** local writers (`internal/reconcile/appliers.go`, backing the `pull-provider` command), i.e. applying *provider truth*, NOT the LIFE-plane twin. They die only when the PULL engine itself is retired. (Possible follow-on: route the PULL appliers through the same core for total unification; the PULL cancel lacks the Solana cascade too, but Solana is never externally pulled so it can't hit the bug today.)
- **Verified green together**: `subscriptions` (incl. the #264 Solana-cascade cancel tests + lifecycle/grace), `converge` (LIFE grace_exhausted/period_overdue/pending_stale now via the core), `entitlements`, `reconcile` (PULL appliers), `webhooks`, `grants`, `productaccess`, `http/handlers`, `migrations/postgres`. Whole-repo `go build ./...` + vet clean (sole vet error is a concurrent agent's syntax typo in `internal/modules/money/invoice_integration_test.go`, unrelated).

---
_Original plan (for reference):_
### J. LIFE repairs → shared lifecycle core (PLAN, Paul 2026-06-18)
**Why:** the LIFE plane's repairs (`grace_exhausted`, `period_overdue`, `pending_stale`) use reconcile-specific SQL appliers (`ReconcileCancelSubscriptionLocal`, `ReconcileRevokeSubscriptionEntitlements`, `MarkSubscriptionPastDueFromOverdue`) — a SECOND implementation of "cancel a sub" / "enter dunning" distinct from the normal `SubscriptionLifecycleService.CancelMembership`. Real divergence + a live BUG: `CancelMembership` cascades to `solana_subscriptions` (stops the #264 cranker) but the convergence cancel does NOT — so a *converged* Solana cancel leaves the cranker pulling forever. (A LIFE finding fires only when (1) the server was down / restored from an old backup so a scheduled transition never ran, or (2) a lifecycle bug — they're catch-up cranks, not normal operation.)
**The wrinkles that kept them apart (must be honored, not lost):** (a) converge-not-replay — `grace_exhausted` revokes entitlements AS-OF grace-end, not now; (b) NO side effects on convergence — `CancelMembership` sends notifications + writes the event log + (future) enqueues provider intents; convergence must NOT re-fire those for a transition that logically happened in the past. So unification is NOT "call CancelMembership" — it's "extract a shared LOCAL-STATE core."
**Design:** add side-effect-free local-state methods to `SubscriptionLifecycleService` — `ApplyLocalCancellation(subID, asOf, cancelType, reason)` (status flip + clear retry/grace/deletion schedule + **solana cascade** + entitlement revoke with an AS-OF param) and `ApplyLocalPastDue(subID, graceEndsAt)`. `CancelMembership` becomes `ApplyLocalCancellation(now) + side effects (notifications/event-log/provider)`; the convergence repairs call `ApplyLocalCancellation(asOf=grace-end)` / `ApplyLocalPastDue` — one implementation, side effects only on the normal path. (Needs an as-of param added to `entSvc.RevokeSourcesForSubscription`.)
**Wiring (NOT #513-blocked):** `NewConvergeEngine(db)` builds the lifecycle service internally from `db` (all its deps are db-constructable; `converge`→`subscriptions` has no cycle) and the LIFE pass calls the new core methods — so NO caller/handler signature changes (avoids the #513-red `internal/http/handlers`). Then delete the now-unused `Reconcile*` appliers. **Verify:** `subscriptions` (lifecycle/grace) + `converge` suites. Billing/access-critical — execute as a careful, fully-tested unit.
