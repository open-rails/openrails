...
- [ ] **Re-path the tensorhub + doujins wire in lockstep (BREAKING, no alias).** `GET/PUT /v1/orgs/:org_id/spend-delegations` and the embedded `/billing/v1/orgs/*` mount become `/v1/customers/:customer_id/*`. Tensorhub depends on the old path: `openrailsclient` `embeddedOrgsPrefix = "/billing/v1/orgs"` (`embedded.go:24`), `SetOrgSpendDelegations`, and `platform_budget_sync.go` must bump in lockstep — a tensorhub `openrailsclient` BILLING-WIRE bump, SEPARATE from tensorhub #498 (which is tensorhub's own internal RBAC adoption, not its OpenRails client); needs its own tensorhub-side tracking. Doujins' embedded `/billing/v1/orgs/*` mount + clients likewise. Version-bumped hard cut, no alias.
