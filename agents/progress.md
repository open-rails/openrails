...
- [ ] **Collapse the org-treasury surface into the customer surface — the API gets SMALLER (one persona, not org+customer).** Retire `/v1/orgs/:org_id/*` (#566): the customer payer surface is `/v1/me/*` (self) + `/v1/customers/:customer_id/*` (co-managed/shared). The FULL #566 payer subset — balance, transactions, settings, payment-methods, checkout, invoices — PLUS spend-delegations all live on the ONE customer surface as `customer:*` perms; there is NO separate org bucket. Reuse #566's machinery verbatim: `OrgTreasuryScopeRequired` (rename → `CustomerScopeRequired`) already rebinds the acting payer to the resource's payable subject, so the shared `/v1/me` money handlers work unchanged — only the path + scope-param name changes (`:org_id` → `:customer_id`). Addressed by the customer's own id (authkit #111 "reference the resource id, not the group id").
- [ ] **Re-path the tensorhub + doujins wire in lockstep (BREAKING, no alias).** `GET/PUT /v1/orgs/:org_id/spend-delegations` and the embedded `/billing/v1/orgs/*` mount become `/v1/customers/:customer_id/*`. Tensorhub depends on the old path via its embedded mount: `openrailsclient` `embeddedOrgsPrefix = "/billing/v1/orgs"` (`embedded.go:24`, mounted :153) must bump (its `syncBudgetPolicies` spend-delegation sync is currently a no-op, so re-target it to `/v1/customers/:id/spend-delegations` when re-wired). A tensorhub `openrailsclient` BILLING-WIRE bump, SEPARATE from tensorhub #498 (its own internal RBAC adoption) — tracked as **tensorhub #499**. Doujins' embedded `/billing/v1/orgs/*` mount + clients likewise. Version-bumped hard cut, no alias.
...
**Superseded by #567 (2026-06-22):** the implementation (shared `/me` money handlers + the scope-rebind middleware) is REUSED, but re-pathed off `/v1/orgs/:org_id/*` onto the universal `customer` persona surface (`/v1/customers/:id/*`). `org` naming is retired and the payer surface is no longer "org-only" — EVERY customer gets it. The re-path is a tensorhub/doujins wire bump (see #567). The design below is historical (org framing); the handlers/perms survive.
